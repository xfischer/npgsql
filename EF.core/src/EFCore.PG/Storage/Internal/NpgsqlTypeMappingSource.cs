...
namespace EnterpriseDB.EDBClient.EntityFrameworkCore.PostgreSQL.Storage.Internal
{
    public class NpgsqlTypeMappingSource : RelationalTypeMappingSource
    {
        ...
        readonly NpgsqlStringTypeMapping       _xml                = new NpgsqlStringTypeMapping("xml", EDBDbType.Xml);
        readonly NpgsqlStringTypeMapping       _citext             = new NpgsqlStringTypeMapping("citext", EDBDbType.Citext);
        ...
        // uint mappings
        readonly NpgsqlUintTypeMapping         _xid                = new NpgsqlUintTypeMapping("xid", EDBDbType.Xid);
        readonly NpgsqlUintTypeMapping         _oid                = new NpgsqlUintTypeMapping("oid", EDBDbType.Oid);
        readonly NpgsqlUintTypeMapping         _cid                = new NpgsqlUintTypeMapping("cid", EDBDbType.Cid);
        readonly NpgsqlUintTypeMapping         _regtype            = new NpgsqlUintTypeMapping("regtype", EDBDbType.Regtype);
        readonly NpgsqlUintTypeMapping         _lo                 = new NpgsqlUintTypeMapping("lo", EDBDbType.Oid);

        // Full text search mappings
        readonly EDBTsQueryTypeMapping   _tsquery               = new EDBTsQueryTypeMapping();
        readonly EDBTsVectorTypeMapping  _tsvector              = new EDBTsVectorTypeMapping();
        ...
        // Built-in ranges
        readonly EDBRangeTypeMapping        _int4range;
        readonly EDBRangeTypeMapping        _int8range;
        readonly EDBRangeTypeMapping        _numrange;
        readonly EDBRangeTypeMapping        _tsrange;
        readonly EDBRangeTypeMapping        _tstzrange;
        readonly EDBRangeTypeMapping        _daterange;
            ...
            : base(dependencies, relationalDependencies)
        {
            ...
            _daterange = new EDBRangeTypeMapping("daterange", typeof(EDBRange<DateTime>), _timestamptz, sqlGenerationHelper);
            ...
            var clrTypeMappings = new Dictionary<Type, RelationalTypeMapping>
            {
                ...
                { typeof(EDBRange<DateTime>),               _tsrange              },
        ...
        {
            foreach (var adoMapping in EDBConnection.GlobalTypeMapper.Mappings.Where(m => m.TypeHandlerFactory is IEnumTypeHandlerFactory).ToArray())
            {
