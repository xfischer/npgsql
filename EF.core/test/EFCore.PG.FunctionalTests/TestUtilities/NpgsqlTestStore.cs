...
namespace EnterpriseDB.EDBClient.EntityFrameworkCore.PostgreSQL.TestUtilities
{
    public class NpgsqlTestStore : RelationalTestStore
    {
        ...
        static void WaitForExists(EDBConnection connection) => WaitForExistsImplementation(connection);

        static void WaitForExistsImplementation(EDBConnection connection)
        {
            var retryCount = 0;
            while (true)
            {
...
SELECT pg_terminate_backend (pg_stat_activity.pid)
   ...
   WHERE datname = '{name}'";
        ...
        public static string CreateConnectionString(string name)
            => new EDBConnectionStringBuilder(TestEnvironment.DefaultConnection)
            {
                Database = name
            }.ConnectionString;
