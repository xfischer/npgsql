...
{
	/// <summary>
	/// Zusammenfassung fr EDBParameterConverter.
	/// </summary>
	internal class EDBParameterConverter : ExpandableObjectConverter
	{
		public EDBParameterConverter()
		{
			//
		...
		public override object ConvertTo(ITypeDescriptorContext context, System.Globalization.CultureInfo culture, object value, Type destinationType) {
			...
			if(destinationType == typeof(InstanceDescriptor) && value as EDBParameter != null){
				EDBParameter param = (EDBParameter)value;
				...
				if(!(OtherChanged || SizeChanged || SourceColumnChanged || ValueChanged)){
					ConstructorInfo ci = typeof(EDBParameter).GetConstructor(new Type[]{typeof(String), typeof(DbType)});
					if (ci != null){
					...
					ConstructorInfo ci = typeof(EDBParameter).GetConstructor(new Type[]{typeof(String), typeof(DbType), typeof(Int32)});
					if (ci != null){
						return new InstanceDescriptor(ci, new Object[]{param.ParameterName, param.DbType, param.Size});
					}
				}else if(!(OtherChanged || ValueChanged)){
					ConstructorInfo ci = typeof(EDBParameter).GetConstructor(new Type[]{typeof(String), typeof(DbType), typeof(Int32), typeof(String)});
					if (ci != null){
					...
					ConstructorInfo ci = typeof(EDBParameter).GetConstructor(new Type[]{typeof(String), typeof(Object)});
					if (ci != null){
						return new InstanceDescriptor(ci, new Object[]{param.ParameterName, param.Value});
					}
				}else{
					ConstructorInfo ci = typeof(EDBParameter).GetConstructor(new Type[]{typeof(String), typeof(DbType), typeof(Int32), typeof(String), typeof(ParameterDirection), typeof(Boolean), typeof(Byte), typeof(Byte), typeof(DataRowVersion), typeof(Object)});
					if (ci != null){
