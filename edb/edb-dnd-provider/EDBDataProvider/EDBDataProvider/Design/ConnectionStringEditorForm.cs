...
namespace EnterpriseDB.EDBClient.Design {
	public class ConnectionStringEditorForm : System.Windows.Forms.Form {
		...
		private EnterpriseDB.EDBClient.EDBConnection pgconn;
    ...
    {}
		...
		private void InitializeComponent() {
			...
			this.pgconn = new EnterpriseDB.EDBClient.EDBConnection();
		...
		private bool connect(bool fillComboBox) {
			try{
				...
				if(fillComboBox == true){
					...
					EDBCommand com = new EDBCommand("SELECT datname FROM pg_database WHERE datallowconn = 't'", this.pgconn);
					EDBDataReader dr = com.ExecuteReader();
					
					while(dr.Read()){
						//EDB TEST
						//com.Parameters[0].Value = dr.GetValue(0);
						//System.Windows.Forms.MessageBox.Show("IN READER");
						//System.Windows.Forms.MessageBox.Show(com.Parameters[0].Value.ToString());
