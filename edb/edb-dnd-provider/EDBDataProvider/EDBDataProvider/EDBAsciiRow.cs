...
namespace EnterpriseDB.EDBClient
{
    ...
    /// </summary>
    internal sealed class EDBAsciiRow : EDBRow
    {
...
//EDB team Start
		public  void ReadFromStreamParamData(Stream inputStream, Encoding encoding)
		{
			switch (protocol_version)
			{
				case ProtocolVersion.Version2 :
...
//EDB Team End
        ...
        private void ReadFromStream_Ver_2(Stream inputStream, Encoding encoding)
        {
            ...
            for (Int16 field_count = 0; field_count < row_desc.NumFields; field_count++)
            {
                ...
                EDBRowDescriptionFieldData field_descr = row_desc[field_count];
                ...
                data.Add(EDBTypesHelper.ConvertBackendStringToSystemType(field_descr.type_info, result.ToString(), field_descr.type_size, field_descr.type_modifier));
        ...
        private void ReadFromStream_Ver_3(Stream inputStream, Encoding encoding)
        {
            ...
            for (Int16 field_count = 0; field_count < numCols; field_count++)
            {
                ...
                if (row_desc[field_count].format_code == FormatCode.Text)
                {
					...
					//System.Windows.Forms.MessageBox.Show("In data row: result=" + result.ToString());		
                }
                else
                    // FIXME: input_buffer isn't holding all the field value. This code isn't handling binary data correctly.
                    data.Add(EDBTypesHelper.ConvertBackendBytesToSystemType(field_descr.type_info, input_buffer, encoding, field_value_size, field_descr.type_modifier));
            }
