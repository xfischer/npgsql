...
namespace EnterpriseDB.EDBClient
{
    ...
    /// </summary>
    /// <param name="e">A <see cref="EDB.EDBNoticeEventArgs">EDBNoticeEventArgs</see> that contains the event data.</param>
    public delegate void NoticeEventHandler(Object sender, EDBNoticeEventArgs e);

    /// <summary>
    /// Represents the method that handles the <see cref="EDB.EDBConnection.Notification">Notification</see> events.
    /// </summary>
    /// <param name="sender">The source of the event.</param>
    /// <param name="e">A <see cref="EDB.EDBNotificationEventArgs">EDBNotificationEventArgs</see> that contains the event data.</param>
    ...
    #endif
    public sealed class EDBConnection : Component, IDbConnection, ICloneable
    {
        // Logging related values
        private static readonly String CLASSNAME = "EDBConnection";
        private static ResourceManager resman = new System.Resources.ResourceManager(typeof(EDBConnection));
        ...
        private EDBConnectionString                 connection_string;
        ...
        /// <see cref="EDB.EDBConnection">EDBConnection</see> class.
        /// </summary>
        ...
        /// Initializes a new instance of the
        /// <see cref="EDB.EDBConnection">EDBConnection</see> class
        /// and sets the <see cref="EDB.EDBConnection.ConnectionString">ConnectionString</see>.
        /// </summary>
        ...
        public EDBConnection(String ConnectionString)
        {
            ...
            connection_string = EDBConnectionString.ParseConnectionString(ConnectionString);
            LogConnectionString();
        ...
        public String ConnectionString {
            ...
            set
            {
                ...
                connection_string = EDBConnectionString.ParseConnectionString(value);
                LogConnectionString();
            }
        }
	   ...
	   /// <summary>
        ...
        {
            EDBEventLog.LogMethodEnter(LogLevel.Debug, CLASSNAME, "IDbConnection.BeginTransaction");
        ...
        {
            EDBEventLog.LogMethodEnter(LogLevel.Debug, CLASSNAME, "IDbConnection.BeginTransaction", level);
        ...
        /// Begins a database transaction.
        /// </summary>
        /// <returns>A <see cref="EDB.EDBTransaction">EDBTransaction</see>
        /// object representing the new transaction.</returns>
        /// <remarks>
        /// Currently there's no support for nested transactions.
        /// </remarks>
        public EDBTransaction BeginTransaction()
        {
        ...
        /// There's no support for nested transactions.
        /// </remarks>
        public EDBTransaction BeginTransaction(IsolationLevel level)
        {
        ...
        /// <see cref="EDB.EDBConnection.ConnectionString">ConnectionString</see>.
        /// </summary>
        public void Open()
        {
            CheckConnectionClosed();
        ...
        {
            EDBEventLog.LogMethodEnter(LogLevel.Debug, CLASSNAME, "IDbConnection.CreateCommand");
            return (EDBCommand) CreateCommand();
        }
        ...
        public EDBCommand CreateCommand()
        {
            CheckNotDisposed();
        ...
        /// <summary>
        /// Gets the EDBConnectionString containing the parsed connection string values.
        /// </summary>
        internal EDBConnectionString ConnectionStringValues {
            get
            {
        ...
        private void LogConnectionString()
        {
            ...
            {
                EDBEventLog.LogMsg(resman, "Log_ConnectionStringValues", LogLevel.Debug, DE.Key, DE.Value);
            }
        }
        ...
        private DataTable GetSchemaDatabases()
        {
            ...
            conn.Open();
            EDBCommand c = new EDBCommand("SELECT d.datname as \"Name\", u.usename as \"Owner\", pg_catalog.pg_encoding_to_char(d.encoding) as \"Encoding\" FROM pg_catalog.pg_database d LEFT JOIN pg_catalog.pg_user u ON d.datdba = u.usesysid ORDER BY 1;", conn);
