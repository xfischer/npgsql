...
namespace EnterpriseDB.EDBClient
{
	...
	/// <summary>
    /// This class represents the Parse message sent to EnterpriseDB
    /// server.
    /// </summary>
    ///
    internal sealed class EDBParse
    {
        ...
        public EDBParse(String prepareName, String queryString, Int32[] parameterIDs,EDBParameterCollection parameters ,EDBCommand commd )
        {
            _prepareName = prepareName;
            _queryString = queryString;
            _parameterIDs = parameterIDs;
			_parameters = parameters;			
			_commandType = commd.CommandType.ToString();
		...
		public void WriteToStreamParseOut(Stream outputStream, Encoding encoding)
		{
			...
			Int32 messageLength = 4 + encoding.GetByteCount(_prepareName) + 1 + encoding.GetByteCount(_queryString) + 1 + 2 + (_parameters.Count * 6);
			PGUtil.WriteInt32(outputStream, messageLength);
			PGUtil.WriteString(_prepareName, outputStream, encoding);
			PGUtil.WriteString(_queryString, outputStream, encoding);
