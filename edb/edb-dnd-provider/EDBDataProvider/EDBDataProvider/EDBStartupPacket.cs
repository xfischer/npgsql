...
namespace EnterpriseDB.EDBClient
{
    ...
    /// </summary>
    ///
    internal sealed class EDBStartupPacket
    {
        // Logging related values
        private static readonly String CLASSNAME = "EDBStartupPacket";
        ...
        public EDBStartupPacket(Int32 packet_size,
        ...
        private void WriteToStream_Ver_2(Stream output_stream, Encoding encoding)
				{
            EDBEventLog.LogMethodEnter(LogLevel.Debug, CLASSNAME, "WriteToStream_Ver_2");
        ...
        {
            EDBEventLog.LogMethodEnter(LogLevel.Debug, CLASSNAME, "WriteToStream_Ver_3");

            PGUtil.WriteInt32(output_stream, 4 + 4 + 5 + (encoding.GetByteCount(user_name) + 1) + 9 + (encoding.GetByteCount(database_name) + 1) + 10 + 4 + 1);

            PGUtil.WriteInt32(output_stream, EnterpriseDB.EDBClient.PGUtil.ConvertProtocolVersion(this.protocol_version));
