...
You can use the EnterpriseDB Connectors Installation wizard to add the
.NET Connector to your system; the installer is available `here <https://www.enterprisedb.com/advanced-downloads>`_.

After downloading the installer, right-click on the installer icon, and
select ``Run As Administrator`` from the context menu.

When the ``Language Selection`` popup opens, select an installation language
and click ``OK`` to continue to the ``Setup`` window.

.. figure:: images/dotnet_installation_wizard.png
   :alt: The DotNet Connector Installation wizard
   :align: center
   :scale: 60%

   *The DotNet Connector Installation wizard*
...
The following sections describe how to configure the .NET Connector
depending upon the framework version to be used:

-  **Referencing the Library Files.** General configuration information
   applicable to all components.

-  **.NET Framework 4.0.** Instructions for configuring for usage with
   .NET Framework 4.0.

-  **.NET Framework 4.5.** Instructions for configuring for usage with
   .NET Framework 4.5.

-  **.NET Framework 4.5.1.** Instructions for configuring for usage with
   .NET Framework 4.5.1.

-  **.NET Standard 2.0.** Instructions for configuring for usage with
   .NET Standard 2.0.

-  **Entity Framework 5/6.** Instructions for configuring for usage with
   Entity Framework.

-  **EnterpriseDB VSIX.** Instructions for configuring for usage with
   EnterpriseDB VSIX.
...
To reference library files using Microsoft Visual Studio:
...
If you are using Entity Framework 6, the additional namespace is the
following:
...
Depending upon the type of application, you may need to import the namespace into the
...
Depending upon the application type, you may need to import the namespace into the
source code (see :ref:`Referencing the Library Files <referencing_the_library_files>`).
...
To set up the .NET Connector for use with Entity Framework, the data
...
.. note:: Entity Framework can be used with the ``EnterpriseDB.EDBClient.dll`` library file; the file is available in the ``net45`` and ``net451`` subdirectories.

See :ref:`Referencing the Library Files <referencing_the_library_files>` for information about referencing library files.

Add the ``<DbProviderFactories>`` entries for the ADO.NET driver for EDB Postgres in the ``app.config`` file. Include the following entries:
...
.. code-block:: text
  ...
  <add name="EnterpriseDB.EDBClient"
    ...
    type="EnterpriseDB.EDBClient.EDBFactory, EnterpriseDB.EDBClient, Version=4.0.6.1, Culture=neutral, PublicKeyToken=5d8b90d52f46fda7"
    support="FF"/>
...
.. note:: The same entries for <providers> and <DbProviderFactories> are valid for the ``web.config`` file as well as for ``app.config``.

Depending upon the type of application, import the namespace into the
source code (see :ref:`Referencing the Library Files <referencing_the_library_files>`).

For information about using the features of Entity Framework, refer to the Microsoft documentation.

.. raw:: latex

    \newpage

EnterpriseDB VSIX for Visual Studio 2015/2017
---------------------------------------------
...
Studio.  It allows you to connect to Advanced Server from within Visual Studio's
Server Explorer and creating a model from an existing database.  You must use 
EnterpriseDB VSIX to access Advanced Server with Visual Studio functionality.
...
Installation and Configuration for Visual Studio 2015/2017
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Use the following steps to install and configure EnterpriseDB VSIX:

**Step 1:** Install EnterpriseDB VSIX for the desired version of Visual
...
Do not include excessive white-space characters in double-quoted strings within the attribute-value pairs.  Examples shown in this section may be split on multiple lines for clarity, but should actually be configured to occupy a single line.
...
Visual Studio ``EntityFramework Tools\DBGen\`` folder as demonstrated in the following
example:
...
**Step 4:** Add the library files (``EnterpriseDB.EDBClient.dll`` and
