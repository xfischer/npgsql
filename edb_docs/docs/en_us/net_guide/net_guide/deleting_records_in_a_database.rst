...
In the example that follows, the ``DELETE`` command is stored in the
variable ``strDeleteQuery``. The code passes the employee number to the
Delete command (specified by: ``EmpNo``). The command is then executed using
the ``ExecuteNonQuery()`` method. The following example deletes the employee
