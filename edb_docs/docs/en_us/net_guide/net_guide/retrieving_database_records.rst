.. _retrieving_database_records:

************************************
`Retrieving Database Records`:index:
************************************

You can use a ``SELECT`` statement to retrieve records from the database via
a ``SELECT`` command. To execute a ``SELECT`` statement you must:
...
subsequent record in the stream, you must call the ``Read()`` method of the
...
5. Loops through the contents of the ``EDBDataReader`` object to display the
   records returned by the query within a ``WHILE`` loop.

The ``Read()`` method advances to the next record (if a record exists) and
returns ``true`` if a record exists, or ``false`` to indicate that the
...
To exercise the sample code, save the code in your default web root
...
To retrieve a single result from a query, use the ``ExecuteScalar()`` method
of the ``EDBCommand`` object. The ``ExecuteScalar()`` method returns the first
value of the first column of the first row of the ``DataSet`` generated by
the specified query.
...
Please note that the sample includes an explicit conversion of the value
returned by the ``ExecuteScalar()`` method. The ``ExecuteScalar()`` method
returns an object; to view the object, you must convert it into an
integer value by using the ``Convert.ToInt32`` method.
