.. _using_advanced_queueing:

********************************
`Using Advanced Queueing`:index:
********************************
...
queue table. You should first create a queue table before creating a
queue that is dependent on it.
...
start the queue on the database server. Invoke EDB-PSQL and connect to
...
The ``EDBAQDequeueOptions`` class lists the options available when dequeuing
a message.
...
The ``EDBAQEnqueueOptions`` class lists the options available when enqueuing
a message.
...
The ``EDBAQMessageState`` class represents the state of the message during
dequeue.
...
The ``EDBAQNavigationMode`` class represents the different types of
...
The ``EDBAQQueue`` class represents a SQL statement to execute ``DMBS_AQ``
