...
public class AuthenticationGSSContinueMessage : AuthenticationGenericMessage
{

    public AuthenticationGSSContinueMessage(char messageCode, int len, int intData, byte[] bytes) : base(messageCode, len, intData, "AuthenticationGSSContinue")
    {
        this.AuthData = bytes;
    }
    ...
    internal override PostgresMessageBase ReadResponseMessage(char code, PcapBinaryReader reader)
    {
        throw new NotImplementedException();
    }
}

public class AuthenticationSASLContinueMessage : AuthenticationGenericMessage
{

    public AuthenticationSASLContinueMessage(char messageCode, int len, int intData, byte[] bytes) : base(messageCode, len, intData, "AuthenticationSASLContinue")
    {
        this.SASLData = bytes;
    }

    public byte[] SASLData { get; }

    internal override PostgresMessageBase ReadResponseMessage(char code, PcapBinaryReader reader) => SASLResponseMessage.Read(code, reader);
}

public class AuthenticationSASLFinalMessage : AuthenticationGenericMessage
{

    public AuthenticationSASLFinalMessage(char messageCode, int len, int intData, byte[] bytes) : base(messageCode, len, intData, "AuthenticationSASLFinal")
    {
        this.SASLOutcome = bytes;
    }

    public byte[] SASLOutcome { get; }

    internal override PostgresMessageBase ReadResponseMessage(char code, PcapBinaryReader reader) => throw new NotImplementedException();
...
public class SASLResponseMessage(char code, int length) : PostgresMessageBase(code, length)
{
    ...
    internal static SASLResponseMessage Read(char messageCode, PcapBinaryReader reader)
    {
        ...
        packet.AuthData = reader.ReadBytes(len - 4);

        return packet;
    }
}

public class AuthenticationGenericMessage : AuthenticationMessage
{
    public AuthenticationGenericMessage(char code, int length, int data, string commonName) : base(code, length)
    {
        this.Data = data;
        this.AuthenticationName = commonName;
    }

    internal virtual PostgresMessageBase ReadResponseMessage(char code, PcapBinaryReader reader) { throw new NotImplementedException(); }
...
public class AuthenticationMD5PasswordMessage(char code, int length) : AuthenticationGenericMessage(code, length, 0, "AuthenticationMD5Password")
{
    internal static AuthenticationMD5PasswordMessage Read(char messageCode, int length, PcapBinaryReader reader)
    {
        ...
        packet.MD5Check = reader.ReadInt32();
...
{
    public List<string> Mechanisms { get; } = new();
    ...
    internal override PostgresMessageBase ReadResponseMessage(char code, PcapBinaryReader reader) => SASLInitialResponseMessage.Read(code, reader);
}
public class SASLInitialResponseMessage(char code, int length) : PostgresMessageBase(code, length)
{
    ...
    internal static SASLInitialResponseMessage Read(char messageCode, PcapBinaryReader reader)
    {
        ...
        packet.Mechanism = reader.ReadNullTerminatedString(len);
        packet.InitialResponseLength = reader.ReadInt32();
