...
public sealed class PcapToLatexService(ILogger<PcapToLatexService> logger, IOptions<PcapToLatexOptions> pcapPostgresOptions) : IPcapToLatexService
{
    ...
    bool ProcessPostGresMessage(PostgresMessageBase message, GenerationState state, StringBuilder latexBuilder, Action<StringBuilder, GenerationState>? messageReadyAction = null)
    {
        ...
        ITextTransformer? textTransformer = options.CustomTemplateProvider?.Invoke(message) ?? FindTextTransformer(message);
    ...
    void WriteTextTransformation(GenerationState state, StringBuilder latexBuilder, ITextTransformer textTransformer)
    {
