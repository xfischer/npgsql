...
public class SpectreConsoleLogger(string name, LogLevel minLevel, bool includePrefix = true, bool includeCategory = false, bool includeEventId = false) : ILogger
{
    ...
    public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter)
    {
        ...
        AnsiConsole.MarkupLine(string.Concat(prefix, categoryStr, formatter(state, exception)));
        //AnsiConsole.MarkupLine(prefix + categoryStr);
        //AnsiConsole.MarkupLine(string.Empty.PadRight(6) + formatter(state, exception));
