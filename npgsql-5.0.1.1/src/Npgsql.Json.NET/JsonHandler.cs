...
namespace EnterpriseDB.EDBClient.Json.NET
{
    ...
    class JsonHandler : EnterpriseDB.EDBClient.TypeHandlers.JsonHandler
    {
        readonly JsonSerializerSettings _settings;
        ...
        protected override Task WriteWithLength<T2>(T2 value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
            if (typeof(T2) == typeof(string) ||
                typeof(T2) == typeof(char[]) ||
                typeof(T2) == typeof(ArraySegment<char>) ||
        ...
        protected override Task WriteObjectWithLength(object value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
            if (value is DBNull ||
                value is string ||
                value is char[] ||
