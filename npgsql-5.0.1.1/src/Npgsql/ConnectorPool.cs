...
namespace EnterpriseDB.EDBClient
{
    sealed partial class ConnectorPool
    {
        ...
        static readonly EDBLogger Log = EDBLogManager.CreateLogger(nameof(ConnectorPool));
        ...
        /// </summary>
        readonly EDBConnector?[] _connectors;
        ...
        /// </summary>
        readonly ChannelReader<EDBConnector?> _idleConnectorReader;
        internal ChannelWriter<EDBConnector?> IdleConnectorWriter { get; }

        /// <summary>
        /// Incremented every time this pool is cleared via <see cref="EDBConnection.ClearPool"/> or
        /// <see cref="EDBConnection.ClearAllPools"/>. Allows us to identify connections which were
        ...
        internal ConnectorPool(EDBConnectionStringBuilder settings, string connString)
        {
            ...
            var idleChannel = Channel.CreateUnbounded<EDBConnector?>();
            ...
            _connectors = new EDBConnector[_max];
            ...
            if (Settings.Multiplexing)
            {
                ...
                _ = Task.Run(MultiplexingWriteLoop)
                    .ContinueWith(t =>
                    {
                        ...
                        Log.Error("Exception in multiplexing write loop, this is an EnterpriseDB.EDBClient bug, please file an issue.",
                            t.Exception!);
                    }, TaskContinuationOptions.OnlyOnFaulted);
            }
        }

        internal ValueTask<EDBConnector> Rent(
            EDBConnection conn, EDBTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            return TryGetIdleConnector(out var connector)
                ? new ValueTask<EDBConnector>(AssignConnection(conn, connector))
                : RentAsync();

            async ValueTask<EDBConnector> RentAsync()
            {
                ...
                while (true)
                {
                    ...
                    {
                        throw new EDBException("The connection pool has been shut down.");
                    }
            ...
            static EDBConnector AssignConnection(EDBConnection connection, EDBConnector connector)
            {
        ...
        bool TryGetIdleConnector([NotNullWhen(true)] out EDBConnector? connector)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        bool CheckIdleConnector([NotNullWhen(true)] EDBConnector? connector)
        {
        ...
        async ValueTask<EDBConnector?> OpenNewConnector(
            EDBConnection conn, EDBTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            ...
            for (var numConnectors = _numConnectors; numConnectors < _max; numConnectors = _numConnectors)
            {
                ...
                try
                {
                    ...
                    var connector = new EDBConnector(conn) { ClearCounter = _clearCounter };
                    ...
                    if (i == _max)
                        throw new EDBException($"Could not find free slot in {_connectors} when opening. Please report a bug.");
        ...
        internal void Return(EDBConnector connector)
        {
        ...
        void CloseConnector(EDBConnector connector)
        {
            try
            {
                connector.Close();
            ...
            if (i == _max)
                throw new EDBException($"Could not find free slot in {_connectors} when closing. Please report a bug.");
        ...
        internal bool TryRentEnlistedPending(Transaction transaction, [NotNullWhen(true)] out EDBConnector? connector)
        {
            lock (_pendingEnlistedConnectors)
            {
                if (!_pendingEnlistedConnectors.TryGetValue(transaction, out var list))
