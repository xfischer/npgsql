...
namespace EnterpriseDB.EDBClient
{
    ...
    public sealed class EDBCommand : DbCommand, ICloneable
    {
        ...
        internal readonly List<EDBStatement> _statements;
...
#nullable enable
        ...
        internal Task Write(EDBConnector connector, bool async, CancellationToken cancellationToken = default)
        {
            return (_behavior & CommandBehavior.SchemaOnly) == 0
                ? WriteExecute(connector, async)
                : WriteExecuteSchemaOnly(connector, async);

            async Task WriteExecute(EDBConnector connector, bool async)
            {
            ...
            async Task WriteExecuteSchemaOnly(EDBConnector connector, bool async)
            {
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
        ...
        internal ManualResetValueTaskSource<EDBConnector> ExecutionCompletion { get; }
            = new ManualResetValueTaskSource<EDBConnector>();

        internal async ValueTask<EDBDataReader> ExecuteReader(CommandBehavior behavior, bool async, CancellationToken cancellationToken)
        {
            var conn = CheckAndGetConnection();
            _behavior = behavior;
            ...
            catch (Exception e)
            {
                ...
                if (!(e is EDBOperationInProgressException) && reader != null)
            ...
            async Task NonMultiplexingWriteWrapper(EDBConnector connector, bool async, CancellationToken cancellationToken2)
            {
                BeginSend(connector);
        ...
        void LogCommand()
        {
            ...
            foreach (var s in _statements)
            {
                ...
                if (p.Count > 0 && (EDBLogManager.IsParameterLoggingEnabled || connector.Settings.LogParameters))
                {
