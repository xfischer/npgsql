    ...
    partial class EDBConnector
    {
        ...
        async Task AuthenticateSASL(List<string> mechanisms, string username, bool async, CancellationToken cancellationToken = default)
        {
            ...
            if (!supportsSha256 && !supportsSha256Plus)
                throw new EDBException("No supported SASL mechanism found (only SCRAM-SHA-256 and SCRAM-SHA-256-PLUS are supported for now). " +
        ...
        async Task AuthenticateGSS(bool async)
        {
            if (!IntegratedSecurity)
                throw new EDBException("GSS/SSPI authentication but IntegratedSecurity not enabled");
            ...
            throw new EDBException("NegotiateStream.AuthenticateAsClient completed unexpectedly without signaling success");
        }
        ...
        class GSSPasswordMessageStream : Stream
        {
            ...
            internal GSSPasswordMessageStream(EDBConnector connector)
            ...
            async Task Write(byte[] buffer, int offset, int count, bool async, CancellationToken cancellationToken = default)
            {
                ...
                if (count > _leftToWrite)
                    throw new EDBException($"NegotiateStream trying to write {count} bytes but according to frame header we only have {_leftToWrite} left!");
            ...
            async Task<int> Read(byte[] buffer, int offset, int count, bool async, CancellationToken cancellationToken = default)
            {
                ...
                if (count > _leftToRead)
                    throw new EDBException($"NegotiateStream trying to read {count} bytes but according to frame header we only have {_leftToRead} left!");
        ...
        string? GetPassword(string username)
        {
            ...
            if (ProvidePasswordCallback is { } passwordCallback)
                ...
                {
                    throw new EDBException($"Obtaining password using {nameof(EDBConnection)}.{nameof(ProvidePasswordCallback)} delegate failed", e);
                }
