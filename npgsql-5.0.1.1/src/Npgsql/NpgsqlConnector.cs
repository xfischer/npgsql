...
namespace EnterpriseDB.EDBClient
{
    ...
    sealed partial class EDBConnector : IDisposable
    {
        ...
        internal EDBDatabaseInfo DatabaseInfo { get; private set; } = default!;
        ...
        internal EDBConnector(EDBConnection connection)
        ...
        EDBConnector(EDBConnectionStringBuilder settings, string connectionString)
        {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (forceReload || !EDBDatabaseInfo.Cache.TryGetValue(key, out var database))
            {
                ...
                try
                {
                    ...
                    {
                        EDBDatabaseInfo.Cache[key] = database = await EDBDatabaseInfo.Load(Connection,
                            timeout, async);
                    }
                }
                finally
            ...
            for (var i = 0; i < endpoints.Length; i++)
            {
                ...
                catch (Exception e)
                {
                    ...
                    if (i == endpoints.Length - 1)
                        throw new EDBException("Exception while connecting", e);
                }
            }
        }
        ...
        internal readonly ChannelReader<EDBCommand>? CommandsInFlightReader;
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
        ...
        void ProcessNewTransactionStatus(TransactionStatus newStatus)
        {
            ...
            case TransactionStatus.Pending:
                throw new Exception($"Internal EnterpriseDB.EDBClient bug: invalid TransactionStatus {nameof(TransactionStatus.Pending)} received, should be frontend-only");
            default:
                throw new InvalidOperationException(
                    $"Internal EnterpriseDB.EDBClient bug: unexpected value {newStatus} of enum {nameof(TransactionStatus)}. Please file a bug.");
            }
        }
        ...
        void DoCancelRequest(int backendProcessId, int backendSecretKey)
        {
            ...
            try
            {
                RawOpen(new EDBTimeout(TimeSpan.FromSeconds(ConnectionTimeout)), false, CancellationToken.None)
