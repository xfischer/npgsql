...
FROM pg_constraint c
    ...
    AND n.nspname NOT IN ('pg_catalog', 'pg_toast')");
        ...
        static DataTable GetDataSourceInformation(EDBConnection conn)
        {
            ...
            row["DataSourceProductName"] = "EnterpriseDB.EDBClient";
        ...
        static DataTable GetDataTypes(EDBConnection conn)
        {
            ...
            // EnterpriseDB.EDBClient-specific
            ...
            foreach (var rangeType in connector.DatabaseInfo.RangeTypes)
            {
                ...
                if (elementMapping.ClrTypes.Length > 0)
                    row["DataType"] = typeof(EDBRange<>).MakeGenericType(elementMapping.ClrTypes[0]).FullName;
                if (elementMapping.EDBDbType.HasValue)
                    row["ProviderDbType"] = (int)(elementMapping.EDBDbType.Value | EDBDbType.Range);
            }
