...
namespace EnterpriseDB.EDBClient.TypeHandling
{
    ...
    public abstract class EDBSimpleTypeHandler<TDefault> : EDBTypeHandler<TDefault>, IEDBSimpleTypeHandler<TDefault>
    {
            ...
            : base(postgresType)
        {
            ...
            (_nonGenericValidateAndGetLength, _nonGenericWriteWithLength) =
                NonGenericDelegateCache.GetOrAdd(GetType(), t => (
                    ...
                    GenerateNonGenericWriteMethod(GetType(), typeof(IEDBSimpleTypeHandler<>)))
                );
        }
        ...
        /// <summary>
        /// Reads a value of type <typeparamref name="TDefault"/> with the given length from the provided buffer,
        /// using either sync or async I/O. This method is sealed for <see cref="EDBSimpleTypeHandler{T}"/>,
        /// override <see cref="Read(EnterpriseDB.EDBClient.EDBReadBuffer,int,EnterpriseDB.EDBClient.BackendMessages.FieldDescription)"/>.
        /// </summary>
        /// <param name="buf">The buffer from which to read.</param>
        /// <param name="len">The byte length of the value. The buffer might not contain the full length, requiring I/O to be performed.</param>
        ...
        /// <see cref="Read(EnterpriseDB.EDBClient.EDBReadBuffer,int,EnterpriseDB.EDBClient.BackendMessages.FieldDescription)"/> - but may do
        ...
        public override TAny Read<TAny>(EDBReadBuffer buf, int len, FieldDescription? fieldDescription = null)
        {
            ...
            var asTypedHandler = this as IEDBSimpleTypeHandler<TAny>;
        ...
        /// </summary>
        internal sealed override Task WriteWithLengthInternal<TAny>([AllowNull] TAny value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
            ...
            async Task WriteWithLengthLong()
            {
                ...
                typedHandler = (IEDBSimpleTypeHandler<TAny>)this;
        ...
        /// </summary>
        public sealed override Task Write(TDefault value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
                ...
                ifElseExpression = Expression.IfThenElse(
                    ...
                    Expression.Assign(
                        ...
                        Expression.Call(
                            ...
                            i.GetMethod(nameof(IEDBSimpleTypeHandler<TDefault>.ValidateAndGetLength))!,
