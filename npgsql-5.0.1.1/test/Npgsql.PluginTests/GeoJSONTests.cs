...
namespace EnterpriseDB.EDBClient.PluginTests
{
    public class GeoJSONTests : TestBase
    {
        ...
        public void Read(TestData data)
        {
            ...
            using (var cmd = new EDBCommand($"SELECT {data.CommandText}, st_asgeojson({data.CommandText},options:=1)", conn))
        ...
        public void Write(TestData data)
        {
            ...
            using (var cmd = new EDBCommand($"SELECT st_asewkb(@p) = st_asewkb({data.CommandText})", conn))
            {
        ...
        {
            using (var conn = OpenConnection())
            using (var cmd = new EDBCommand("SELECT st_makepointm(1,1,1)", conn))
            using (var reader = cmd.ExecuteReader())
            {
                Assert.That(reader.Read());
        ...
        public void ReadUnknownCRS()
        {
            ...
            using (var cmd = new EDBCommand("SELECT st_setsrid(st_makepoint(0,0), 1)", conn))
            using (var reader = cmd.ExecuteReader())
            {
                Assert.That(reader.Read());
        ...
        {
            using (var conn = OpenConnection(option: GeoJSONOptions.ShortCRS))
            using (var cmd = new EDBCommand("SELECT st_setsrid(st_makepoint(0,0), 0)", conn))
            using (var reader = cmd.ExecuteReader())
            {
                Assert.That(reader.Read());
        ...
        {
            using (var conn = OpenConnection(option: GeoJSONOptions.ShortCRS))
            using (var cmd = new EDBCommand("SELECT st_setsrid(st_makepoint(0,0), 4326)", conn))
            {
        ...
        {
            using (var conn = OpenConnection())
            using (var cmd = new EDBCommand("SELECT st_srid(@p)", conn))
            {
        ...
        public void RoundtripGeometryGeography()
        {
            ...
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("INSERT INTO data (geom, geog) VALUES (@p, @p)", conn))
                {
                    cmd.Parameters.AddWithValue("p", point);
                    cmd.ExecuteNonQuery();
                }

                using (var cmd = new EDBCommand("SELECT geom, geog FROM data", conn))
                using (var reader = cmd.ExecuteReader())
                {
                    reader.Read();
        ...
        protected EDBConnection OpenConnection(string? connectionString = null, GeoJSONOptions option = GeoJSONOptions.None)
        {
