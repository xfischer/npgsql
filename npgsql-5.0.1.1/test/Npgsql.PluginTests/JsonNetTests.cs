...
{
    /// <summary>
    /// Tests for the EnterpriseDB.EDBClient.Json.NET mapping plugin
    /// </summary>
    [NonParallelizable]
    ...
    public class JsonNetTests : TestBase
    {
        ...
        {
            using (var conn = OpenConnection())
            using (var cmd = new EDBCommand($@"SELECT '[1, 2, 3]'::{_pgTypeName}", conn))
            using (var reader = cmd.ExecuteReader())
            {
                reader.Read();
        ...
        public void RoundtripClrArray()
        {
            ...
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand($@"SELECT @p::{_pgTypeName}", conn))
                {
                    cmd.Parameters.AddWithValue("p", expected);
                    using (var reader = cmd.ExecuteReader())
                    {
        ...
        void RoundtripCustomSerializerSettings(bool asJsonb)
        {
            ...
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand($@"SELECT @p::{_pgTypeName}, @p::text", conn))
                {
                    cmd.Parameters.AddWithValue("p", expected);
                    using (var reader = cmd.ExecuteReader())
                    {
