    ...
    public class NodaTimeTests : TestBase
    {
        ...
        public void Timestamp(LocalDateTime localDateTime)
        {
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("INSERT INTO data VALUES (@p1, @p2, @p3, @p4, @p5, @p6, @p7, @p8)", conn))
                {
                    cmd.Parameters.Add(new EDBParameter("p1", EDBDbType.Timestamp) { Value = instant });
                    cmd.Parameters.Add(new EDBParameter("p2", DbType.DateTime) { Value = instant });
                    cmd.Parameters.Add(new EDBParameter("p3", DbType.DateTime2) { Value = instant });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p4", Value = instant });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p5", Value = localDateTime });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p6", Value = minTimestampPostgres });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p7", Value = maxTimestampPostgres });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p8", Value = dateTime });
                    cmd.ExecuteNonQuery();
                }
                ...
                using (var cmd = new EDBCommand("SELECT d6::TEXT, d7::TEXT, d8::TEXT FROM data", conn))
                using (var reader = cmd.ExecuteReader())
                {
                    reader.Read();
                ...
                using (var cmd = new EDBCommand("SELECT d1, d2, d3, d4, d5 FROM data", conn))
                using (var reader = cmd.ExecuteReader())
                {
                    reader.Read();
        ...
        public void TimestampConvertInfinity()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                using (var cmd = new EDBCommand("INSERT INTO data VALUES (@p1, @p2, @p3, @p4)", conn))
                {
                    cmd.Parameters.AddWithValue("p1", EDBDbType.Timestamp, Instant.MaxValue);
                    cmd.Parameters.AddWithValue("p2", EDBDbType.Timestamp, Instant.MinValue);
                    cmd.Parameters.AddWithValue("p3", EDBDbType.Timestamp, DateTime.MaxValue);
                    cmd.Parameters.AddWithValue("p4", EDBDbType.Timestamp, DateTime.MinValue);
                    cmd.ExecuteNonQuery();
                }

                using (var cmd = new EDBCommand("SELECT d1::TEXT, d2::TEXT, d3::TEXT, d4::TEXT FROM data", conn))
                using (var reader = cmd.ExecuteReader())
                {
                    reader.Read();
        ...
        public void TimestampTz()
        {
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("INSERT INTO data VALUES (@p1, @p2, @p3, @p4, @p5, @p6)", conn))
                {
                    cmd.Parameters.Add(new EDBParameter("p1", EDBDbType.TimestampTz) { Value = instant });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p2", Value = utcZonedDateTime });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p3", Value = localZonedDateTime });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p4", Value = offsetDateTime });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p5", Value = dateTimeOffset });
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p6", Value = dateTime });
                    cmd.ExecuteNonQuery();
                }

                using (var cmd = new EDBCommand("SELECT d1::TEXT, d2::TEXT, d3::TEXT, d4::TEXT, d5::TEXT, d6::TEXT FROM data", conn))
                using (var reader = cmd.ExecuteReader())
                {
                    reader.Read();
        ...
        public void DateConvertInfinity()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                using (var cmd = new EDBCommand("INSERT INTO data VALUES (@p1, @p2, @p3, @p4)", conn))
                {
                    cmd.Parameters.AddWithValue("p1", EDBDbType.Date, LocalDate.MaxIsoValue);
                    cmd.Parameters.AddWithValue("p2", EDBDbType.Date, LocalDate.MinIsoValue);
                    cmd.Parameters.AddWithValue("p3", EDBDbType.Date, DateTime.MaxValue);
                    cmd.Parameters.AddWithValue("p4", EDBDbType.Date, DateTime.MinValue);
                    cmd.ExecuteNonQuery();
                }
        ...
        public void TimeTz()
        {
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("SELECT @p1, @p2, @p3, @p4, @p5, @p6", conn))
                {
                    ...
                    cmd.Parameters.Add(new EDBParameter("p4", dateTimeOffset));
                    cmd.Parameters.Add(new EDBParameter("p5", EDBDbType.TimeTz) { Value = dateTime });
                    cmd.Parameters.Add(new EDBParameter("p6", dateTime));
