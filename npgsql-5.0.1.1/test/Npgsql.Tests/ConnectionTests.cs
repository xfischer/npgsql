...
namespace EnterpriseDB.EDBClient.Tests
{
    public class ConnectionTests : MultiplexingTestBase
    {
        ...
        public void DataSource()
        {
            ...
            var bld = new EDBConnectionStringBuilder(ConnectionString);
            bld.Host = "Otherhost";

            using (var conn = new EDBConnection(bld.ToString()))
                Assert.That(conn.DataSource, Is.EqualTo($"tcp://{conn.Host}:{conn.Port}"));

            bld = new EDBConnectionStringBuilder(ConnectionString);
            bld.Port = 5435;
        ...
        public void EDBErrorRepro2()
        {
            ...
            using (connection = new EDBConnection(ConnectionString))
            {
                connection.Open();
                using (var command = connection.CreateCommand())
                {
