...
#nullable disable
	...
	public class EDBFunctionTests : TestBase
    {
		...
		public void testFunctionWithLongTextAsInInoutOut()
		{
			...
			string strSql ="CREATE OR REPLACE FUNCTION FunctionWithLongText1(p_in in Text,p_inout inout Text,p_out out Text) return Text   IS   BEGIN  p_out:=p_inout; p_inout:=p_in; return p_out;  END;";
			command.CommandText = strSql;
			command.ExecuteNonQuery();
	...
	//	[Test]
		...
		public void testFunctionWithSMALLFLOATAsInInoutOut()
		{
			...
			string strSql ="CREATE OR REPLACE FUNCTION FunctionWithSmallFloat(p_in in Float,p_inout inout Float,p_out out Float) return Float  IS   BEGIN  p_out:=p_inout; p_inout:=p_in; return 10.1111;  END;";
			command.CommandText = strSql;
			command.ExecuteNonQuery();
	...
	//	[Test]
		...
		public void testMaxParametersSupportInFunctionWithTextAsInAndOut()
		{
			...
			try
			{
                ...
                for (int i = 0; i < 127; i++)
                {
                    string expectedValue = (i+1).ToString();
                    if (i % 3 == 2)
