...
namespace EnterpriseDB.EDBClient.Tests
{
    public class ExceptionTests : TestBase
    {
        ...
        public void Basic()
        {
            using var conn = OpenConnection(new EDBConnectionStringBuilder(ConnectionString)
            {
            ...
            Assert.That(exString, Does.StartWith("EnterpriseDB.EDBClient.PostgresException (0x80004005): 12345: testexception"));
...
$$ LANGUAGE 'plpgsql';");
        ...
        {
            var builder = new EDBConnectionStringBuilder(ConnectionString) { IncludeErrorDetails = true };
...
$$ LANGUAGE 'plpgsql';");
        ...
        public void DataTypeNameExceptionFieldIsPopulated()
        {
            ...
            using (var conn = OpenConnection())
            {
                ...
                try
                {
                    var command = new EDBCommand(dropDomain, conn);
                    command.ExecuteNonQuery();

                    command = new EDBCommand(createDomain, conn);
                    command.ExecuteNonQuery();

                    command = new EDBCommand(castStatement, conn);
        ...
        [Test]
        public void EDBExceptionInAsync()
        {
            using (var conn = OpenConnection())
            {
        ...
        [Test]
        public void EDBExceptionTransience()
        {
