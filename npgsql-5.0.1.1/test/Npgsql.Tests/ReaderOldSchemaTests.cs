...
CREATE TABLE {table} (
    ...
    CONSTRAINT data2_pkey PRIMARY KEY (field_pk1, field_pk2)
            ...
            using var command = new EDBCommand($"SELECT * FROM {table}", conn);
...
CREATE OR REPLACE VIEW {view} (id, int2) AS SELECT id, int2 + int2 AS int2 FROM {table}");
            ...
            var command = new EDBCommand($"SELECT * FROM {view}", conn);
...
SELECT 1 AS some_other_column, 2";
        ...
        public async Task BaseColumnName()
        {
            ...
            var cmd = new EDBCommand("SELECT Cod as CodAlias, Descr as DescrAlias, Date FROM data", conn);
        ...
        private async Task<DataTable?> GetSchemaTable(EDBDataReader dr) => IsAsync ? await dr.GetSchemaTableAsync() : dr.GetSchemaTable();
    }
}
