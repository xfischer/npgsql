...
");
        ...
        private async Task<DataTable> GetSchema(EDBConnection conn)
            => IsAsync ? await conn.GetSchemaAsync() : conn.GetSchema();

        private async Task<DataTable> GetSchema(EDBConnection conn, string collectionName)
            => IsAsync ? await conn.GetSchemaAsync(collectionName) : conn.GetSchema(collectionName);

        private async Task<DataTable> GetSchema(EDBConnection conn, string collectionName, string?[] restrictions)
