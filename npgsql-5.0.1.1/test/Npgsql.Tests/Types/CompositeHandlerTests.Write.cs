...
namespace EnterpriseDB.EDBClient.Tests.Types
{
    ...
    {
        void Write<T>(Action<Func<EDBDataReader>, T> assert, string? schema = null)
        ...
        void Write<T>(T composite, Action<Func<EDBDataReader>, T> assert, string? schema = null)
            where T : IComposite
        {
            using var connection = OpenAndMapComposite<T>(composite, schema, nameof(Write), out var _);
            using var command = new EDBCommand("SELECT (@c).*", connection);
