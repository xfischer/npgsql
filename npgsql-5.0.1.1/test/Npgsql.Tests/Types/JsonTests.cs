    ...
    public class JsonTests : MultiplexingTestBase
    {
        ...
        public async Task RoundtripString()
        {
            ...
            using (var cmd = new EDBCommand("SELECT @p1, @p2", conn))
            {
                ...
                cmd.Parameters.Add(new EDBParameter("p1", EDBDbType) { Value = value });
                cmd.Parameters.Add(new EDBParameter<string>("p2", EDBDbType) { TypedValue = value });
        ...
        public async Task RoundtripStringTypes()
        {
            ...
            var expectedString = EDBDbType.Equals(EDBDbType.Jsonb) ? "{\"p\": 1}"
                                    : "{\"p\":1}";

            using var conn = OpenConnection();
            using var cmd = new EDBCommand(@"SELECT @p1, @p2, @p3", conn);

            cmd.Parameters.Add(new EDBParameter<string>("p1", EDBDbType) { Value = expected });
            cmd.Parameters.Add(new EDBParameter<char[]>("p2", EDBDbType) { Value = expected.ToCharArray() });
            cmd.Parameters.Add(new EDBParameter<byte[]>("p3", EDBDbType) { Value = Encoding.ASCII.GetBytes(expected) });
        ...
        [Test, Ignore("IEDBTypeHandler<ArraySegment<char>>.Read currently not yet implemented in TextHandler")]
        public async Task RoundtripArraySegment()
        {
            ...
            cmd.Parameters.Add(new EDBParameter<ArraySegment<char>>("p1", EDBDbType) { Value = new ArraySegment<char>(expected.ToCharArray()) });
        ...
        public async Task ReadJsonDocument()
        {
            ...
            using (var cmd = new EDBCommand("SELECT @p", conn))
            {
                ...
                cmd.Parameters.Add(new EDBParameter("p", EDBDbType) { Value = value });
                using (var reader = await cmd.ExecuteReaderAsync())
                {
                    reader.Read();
        ...
        public async Task WriteJsonDocument()
        {
            ...
            using (var cmd = new EDBCommand("SELECT @p1, @p2", conn))
            {
                ...
                cmd.Parameters.Add(new EDBParameter("p1", EDBDbType) { Value = value });
                cmd.Parameters.Add(new EDBParameter<JsonDocument>("p2", EDBDbType) { TypedValue = value });
                if (IsJsonb)
                {
                    cmd.CommandText += ", @p3";
        ...
        public async Task WriteObject()
        {
            ...
            using (var cmd = new EDBCommand("SELECT @p1, @p2", conn))
            {
                ...
                cmd.Parameters.Add(new EDBParameter("p1", EDBDbType) { Value = value });
                cmd.Parameters.Add(new EDBParameter<WeatherForecast>("p2", EDBDbType) { TypedValue = value });
                using (var reader = await cmd.ExecuteReaderAsync())
                {
                    reader.Read();
                    for (var i = 0; i < 2; i++)
