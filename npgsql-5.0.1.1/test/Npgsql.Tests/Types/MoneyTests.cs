...
namespace EnterpriseDB.EDBClient.Tests.Types
{
    public class MoneyTests : MultiplexingTestBase
    {
        ...
        public async Task Write(string query, decimal expected)
        {
            ...
            {
                cmd.Parameters.Add(new EDBParameter("p", EDBDbType.Money) { Value = expected });
        ...
        public async Task WriteWithLargeScale(string query, decimal parameter, decimal expected)
        {
            ...
            using (var cmd = new EDBCommand("SELECT @p, @p = " + query, conn))
            {
                cmd.Parameters.Add(new EDBParameter("p", EDBDbType.Money) { Value = parameter });
                using (var rdr = await cmd.ExecuteReaderAsync())
                {
                    rdr.Read();
                    Assert.That(decimal.GetBits(rdr.GetFieldValue<decimal>(0)), Is.EqualTo(decimal.GetBits(expected)));
        ...
        public async Task Mapping()
        {
            ...
            {
                cmd.Parameters.Add(new EDBParameter("p1", EDBDbType.Money) { Value = 8M });
                cmd.Parameters.Add(new EDBParameter("p2", DbType.Currency) { Value = 8M });
