    ...
    public class TextTests : MultiplexingTestBase
    {
        ...
        public async Task Roundtrip()
        {
            ...
            using (var cmd = new EDBCommand("SELECT @p1, @p2, @p3, @p4, @p5, @p6, @p7", conn))
            {
                ...
                var p1 = new EDBParameter("p1", EDBDbType.Text);
                var p2 = new EDBParameter("p2", EDBDbType.Varchar);
                var p3 = new EDBParameter("p3", DbType.String);
                var p4 = new EDBParameter { ParameterName = "p4", Value = expected };
                var p5 = new EDBParameter("p5", EDBDbType.Text);
                var p6 = new EDBParameter("p6", EDBDbType.Text);
                var p7 = new EDBParameter("p7", EDBDbType.Text);
                Assert.That(p2.DbType, Is.EqualTo(DbType.String));
                Assert.That(p3.EDBDbType, Is.EqualTo(EDBDbType.Text));
        ...
        public async Task Long([Values(CommandBehavior.Default, CommandBehavior.SequentialAccess)] CommandBehavior behavior)
        {
            using (var conn = await OpenConnectionAsync())
            {
                ...
                using (var cmd = new EDBCommand($"INSERT INTO {table} (name) VALUES (@p)", conn))
                {
                    cmd.Parameters.Add(new EDBParameter("p", expected));
                    await cmd.ExecuteNonQueryAsync();
                }

                using (var cmd = new EDBCommand($"SELECT name, 'foo', name, name, name, name FROM {table}", conn))
                {
        ...
        public async Task NullCharacter()
        {
            ...
            {
                cmd.Parameters.Add(new EDBParameter("p1", "string with \0\0\0 null \0bytes"));
        ...
        public async Task AliasedPgTypes(string typename)
        {
            ...
            using (var conn = await OpenConnectionAsync())
            using (var cmd = new EDBCommand($"SELECT '{expected}'::{typename}", conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                reader.Read();
        ...
        {
            using (var conn = await OpenConnectionAsync())
            using (var command = new EDBCommand("SELECT @p", conn))
            {
        ...
        public async Task Citext()
        {
            using (var conn = await OpenConnectionAsync())
            {
                ...
                var value = "Foo";
                using (var cmd = new EDBCommand("SELECT @p::CITEXT", conn))
                {
                ...
                using (var cmd = new EDBCommand("SELECT @p1::CITEXT = @p2::CITEXT", conn))
                {
                    cmd.Parameters.AddWithValue("p1", EDBDbType.Citext, "abc");
                    cmd.Parameters.AddWithValue("p2", EDBDbType.Citext, "ABC");
        ...
        public async Task Xml()
        {
            ...
            using (var cmd = new EDBCommand("SELECT @p1, @p2", conn))
            {
                ...
                var p1 = new EDBParameter("p1", EDBDbType.Xml);
                var p2 = new EDBParameter("p2", DbType.Xml);
                Assert.That(p1.EDBDbType, Is.EqualTo(EDBDbType.Xml));
