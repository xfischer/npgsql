...
public class NpgsqlTypeMappingSource : RelationalTypeMappingSource
{
    ...
    {
        LegacyTimestampBehavior = AppContext.TryGetSwitch("EnterpriseDB.EDBClient.EnableLegacyTimestampBehavior", out var enabled) && enabled;
    ...
    /// </summary>
    private readonly Dictionary<Type, List<EDBRangeTypeMapping>> _rangeTypeMapings;
    ...
    // Date/Time types
    private readonly EDBDateTypeMapping         _dateDateTime       = new(typeof(DateTime));
    ...
    private readonly EDBDateTypeMapping         _dateDateOnly       = new(typeof(DateOnly));
    ...
    private readonly EDBTsQueryTypeMapping   _tsquery               = new();
    private readonly EDBTsVectorTypeMapping  _tsvector              = new();
    ...
    // Built-in ranges
    private readonly EDBRangeTypeMapping        _int4range;
    private readonly EDBRangeTypeMapping        _int8range;
    private readonly EDBRangeTypeMapping        _numrange;
    private readonly EDBRangeTypeMapping        _tsrange;
    private readonly EDBRangeTypeMapping        _tstzrange;
    private readonly EDBRangeTypeMapping        _dateOnlyDaterange;
    private readonly EDBRangeTypeMapping        _dateTimeDaterange;
    ...
    private readonly EDBTidTypeMapping             _tid             = new();
        ...
        : base(dependencies, relationalDependencies)
    {
        ...
        _int4range         = new EDBRangeTypeMapping("int4range", typeof(EDBRange<int>),      _int4,         sqlGenerationHelper);
        _int8range         = new EDBRangeTypeMapping("int8range", typeof(EDBRange<long>),     _int8,         sqlGenerationHelper);
        _numrange          = new EDBRangeTypeMapping("numrange",  typeof(EDBRange<decimal>),  _numeric,      sqlGenerationHelper);
        _tsrange           = new EDBRangeTypeMapping("tsrange",   typeof(EDBRange<DateTime>), _timestamp,    sqlGenerationHelper);
        _tstzrange         = new EDBRangeTypeMapping("tstzrange", typeof(EDBRange<DateTime>), _timestamptz,  sqlGenerationHelper);
        _dateOnlyDaterange = new EDBRangeTypeMapping("daterange", typeof(EDBRange<DateOnly>), _dateDateOnly, sqlGenerationHelper);
        _dateTimeDaterange = new EDBRangeTypeMapping("daterange", typeof(EDBRange<DateTime>), _dateDateTime, sqlGenerationHelper);
...
// ReSharper restore CoVariantArrayConversion
        ...
        var clrTypeMappings = new Dictionary<Type, RelationalTypeMapping>
        {
            ...
            { typeof(EDBRange<int>[]),                  _int4multirangeArray  },
            { typeof(EDBRange<long>[]),                 _int8multirangeArray  },
            { typeof(EDBRange<decimal>[]),              _nummultirangeArray   },
            { typeof(EDBRange<DateTime>[]),             LegacyTimestampBehavior ? _tsmultirangeArray : _tstzmultirangeArray },
            { typeof(EDBRange<DateOnly>[]),              _dateOnlyDatemultirangeArray },

            { typeof(List<EDBRange<int>>),              _int4multirangeList   },
            { typeof(List<EDBRange<long>>),             _int8multirangeList   },
            { typeof(List<EDBRange<decimal>>),          _nummultirangeList    },
            { typeof(List<EDBRange<DateTime>>),         LegacyTimestampBehavior ? _tsmultirangeList : _tstzmultirangeList },
            { typeof(List<EDBRange<DateOnly>>),          _dateOnlyDatemultirangeList },
    ...
    {
        _adoUserTypeMappingsGetMethodInfo ??= EDBConnection.GlobalTypeMapper.GetType().GetProperty("UserTypeMappings")?.GetMethod;
        ...
        var adoUserTypeMappings = (IDictionary<string, IUserTypeMapping>)_adoUserTypeMappingsGetMethodInfo.Invoke(EDBConnection.GlobalTypeMapper, Array.Empty<object>())!;
    ...
    protected virtual RelationalTypeMapping? FindUserRangeMapping(in RelationalTypeMappingInfo mappingInfo)
    {
        ...
        return new EDBRangeTypeMapping(rangeDefinition.RangeName, rangeDefinition.SchemaName, rangeClrType, subtypeMapping, _sqlGenerationHelper);
    }
