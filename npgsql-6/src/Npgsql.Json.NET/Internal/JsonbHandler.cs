...
{
    class JsonbHandler : EnterpriseDB.EDBClient.Internal.TypeHandlers.JsonHandler
    {
        readonly JsonSerializerSettings _settings;

        public JsonbHandler(PostgresType postgresType, EDBConnector connector, JsonSerializerSettings settings)
            : base(postgresType, connector.TextEncoding, isJsonb: true) => _settings = settings;

        protected override async ValueTask<T> ReadCustom<T>(EDBReadBuffer buf, int len, bool async, FieldDescription? fieldDescription = null)
        {
        ...
        protected override int ValidateAndGetLengthCustom<T2>([DisallowNull] T2 value, ref EDBLengthCache? lengthCache, EDBParameter? parameter)
        {
        ...
        protected override Task WriteWithLengthCustom<T2>([DisallowNull] T2 value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
            if (typeof(T2) == typeof(string) ||
                typeof(T2) == typeof(char[]) ||
                typeof(T2) == typeof(ArraySegment<char>) ||
