...
using (var cmd = new EDBCommand("INSERT INTO data (geom) VALUES (@p)", conn))
{
    cmd.Parameters.AddWithValue("@p", point);
    cmd.ExecuteNonQuery();
}

using (var cmd = new EDBCommand("SELECT geom FROM data", conn))
...
For more information, [visit the NetTopologySuite plugin documentation page](https://www.EnterpriseDB.EDBClient.org/doc/types/nts.html).
