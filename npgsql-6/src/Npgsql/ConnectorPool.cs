...
namespace EnterpriseDB.EDBClient
{
    class ConnectorPool : ConnectorSource
    {
        ...
        /// </summary>
        private protected readonly EDBConnector?[] Connectors;
        ...
        internal ConnectorPool(EDBConnectionStringBuilder settings, string connString, MultiHostConnectorPool? parentPool = null)
            : base(settings, connString)
        {
            ...
            Connectors = new EDBConnector[_max];
        }

        internal sealed override ValueTask<EDBConnector> Get(
            EDBConnection conn, EDBTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            return TryGetIdleConnector(out var connector)
                ? new ValueTask<EDBConnector>(connector)
        ...
        internal sealed override bool TryGetIdleConnector([NotNullWhen(true)] out EDBConnector? connector)
        {
        ...
        internal sealed override async ValueTask<EDBConnector?> OpenNewConnector(
            EDBConnection conn, EDBTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            ...
            for (var numConnectors = _numConnectors; numConnectors < _max; numConnectors = _numConnectors)
            {
                ...
                try
                {
                    ...
                    if (i == _max)
                        throw new EDBException($"Could not find free slot in {Connectors} when opening. Please report a bug.");
        ...
        internal sealed override void Return(EDBConnector connector)
        {
        ...
        void CloseConnector(EDBConnector connector)
        {
            ...
            if (i == _max)
                throw new EDBException($"Could not find free slot in {Connectors} when closing. Please report a bug.");
