    ...
    public static class ClusterStateCache
    {
        ...
        internal static ClusterState UpdateClusterState(string host, int port, ClusterState state, DateTime timeStamp, TimeSpan stateExpiration)
            => Clusters.AddOrUpdate(
                ...
                new ClusterInfo(state, new EDBTimeout(stateExpiration), timeStamp),
                (_, oldInfo) => oldInfo.TimeStamp >= timeStamp ? oldInfo : new ClusterInfo(state, new EDBTimeout(stateExpiration), timeStamp)).State;
#else
        internal static ClusterState UpdateClusterState(string host, int port, ClusterState state, DateTime timeStamp, TimeSpan stateExpiration)
            => Clusters.AddOrUpdate(
                new ClusterIdentifier(host, port),
                ...
                new ClusterInfo(state, new EDBTimeout(stateExpiration), timeStamp)).State;
#endif
        ...
        readonly struct ClusterInfo
        {
            ...
            public ClusterInfo(ClusterState state, EDBTimeout timeout, DateTime timeStamp)
