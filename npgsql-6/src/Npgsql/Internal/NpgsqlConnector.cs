...
namespace EnterpriseDB.EDBClient.Internal
{
    ...
    public sealed partial class EDBConnector : IDisposable
    {
        ...
        internal EDBConnector(ConnectorSource connectorSource, EDBConnection conn)
        ...
        EDBConnector(ConnectorSource connectorSource)
        {
        ...
        internal async Task Open(EDBTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            ...
            static async Task OpenCore(EDBConnector conn, SslMode sslMode, EDBTimeout timeout, bool async, CancellationToken cancellationToken)
            {
        ...
        internal async ValueTask LoadDatabaseInfo(bool forceReload, EDBTimeout timeout, bool async,
            CancellationToken cancellationToken = default)
        {
            ...
            var key = new EDBDatabaseInfoCacheKey(Settings);
            if (forceReload || !EDBDatabaseInfo.Cache.TryGetValue(key, out var database))
            {
                ...
                try
                {
                    if (forceReload || !EDBDatabaseInfo.Cache.TryGetValue(key, out database))
                    {
                        EDBDatabaseInfo.Cache[key] = database = await EDBDatabaseInfo.Load(this, timeout, async);
                    }
                }
                finally
                {
        ...
        async Task ConnectAsync(EDBTimeout timeout, CancellationToken cancellationToken)
        {
            ...
            Task<IPAddress[]> GetHostAddressesAsync(EDBTimeout timeout, CancellationToken cancellationToken)
            {
