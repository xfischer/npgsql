...
namespace EnterpriseDB.EDBClient.Internal
{
    ...
    public abstract class EDBDatabaseInfo
    {
        ...
        internal static readonly ConcurrentDictionary<EDBDatabaseInfoCacheKey, EDBDatabaseInfo> Cache
            = new();

        static volatile IEDBDatabaseInfoFactory[] Factories = new IEDBDatabaseInfoFactory[]
        {
        ...
        /// Initializes the instance of <see cref="EDBDatabaseInfo"/>.
        /// </summary>
        protected EDBDatabaseInfo(string host, int port, string databaseName, Version version, string serverVersion)
        {
            Host = host;
            Port = port;
            Name = databaseName;
        ...
        private protected EDBDatabaseInfo(string host, int port, string databaseName, string serverVersion)
        {
            Host = host;
            Port = port;
            Name = databaseName;
        ...
        /// </summary>
        public static void RegisterFactory(IEDBDatabaseInfoFactory factory)
        {
            if (factory == null)
                throw new ArgumentNullException(nameof(factory));

            var factories = new IEDBDatabaseInfoFactory[Factories.Length + 1];
            factories[0] = factory;
        ...
        internal static async Task<EDBDatabaseInfo> Load(EDBConnector conn, EDBTimeout timeout, bool async)
        {
            ...
            throw new EDBException("No DatabaseInfoFactory could be found for this connection");
        }
