...
namespace EnterpriseDB.EDBClient.Internal.TypeHandlers.DateTimeHandlers
{
    ...
    /// </remarks>
    public partial class TimestampTzHandler : EDBSimpleTypeHandlerWithPsv<DateTime, EDBDateTime>,
        IEDBSimpleTypeHandler<DateTimeOffset>, IEDBSimpleTypeHandler<long>
    {
        ...
        {
            var ts = ReadEDBDateTime(buf, len, fieldDescription);

            if (!ts.IsFinite)
                return ts;

            var npgsqlDateTime = new EDBDateTime(ts.Date, ts.Time, DateTimeKind.Utc);
        ...
        public override int ValidateAndGetLength(EDBDateTime value, EDBParameter? parameter)
            => value.Kind == DateTimeKind.Utc ||
               value == EDBDateTime.Infinity ||
               value == EDBDateTime.NegativeInfinity ||
               LegacyTimestampBehavior
                ? 8
                : throw new InvalidCastException(
                    $"Cannot write DateTime with Kind={value.Kind} to PostgreSQL type 'timestamp with time zone', only UTC is supported. " +
