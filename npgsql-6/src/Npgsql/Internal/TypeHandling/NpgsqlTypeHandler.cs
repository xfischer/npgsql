...
namespace EnterpriseDB.EDBClient.Internal.TypeHandling
{
    ...
    /// Type handler writers shouldn't inherit from this class, inherit <see cref="EDBTypeHandler"/>
    /// or <see cref="EDBSimpleTypeHandler{T}"/> instead.
    /// </summary>
    public abstract class EDBTypeHandler
    {
        ...
        protected internal async ValueTask<TAny> Read<TAny>(EDBReadBuffer buf, int len, bool async, FieldDescription? fieldDescription = null)
        {
            switch (this)
            {
            case IEDBSimpleTypeHandler<TAny> simpleTypeHandler:
                await buf.Ensure(len, async);
                return simpleTypeHandler.Read(buf, len, fieldDescription);
            case IEDBTypeHandler<TAny> typeHandler:
        ...
        /// <summary>
        /// Version of <see cref="Read{TAny}(EDBReadBuffer,int,bool,FieldDescription?)"/> that's called when we know the entire value
        /// is already buffered in memory (i.e. in non-sequential mode).
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public TAny Read<TAny>(EDBReadBuffer buf, int len, FieldDescription? fieldDescription = null)
        {
            Debug.Assert(buf.ReadBytesLeft >= len);

            return this switch
            {
                IEDBSimpleTypeHandler<TAny> simpleTypeHandler => simpleTypeHandler.Read(buf, len, fieldDescription),
                IEDBTypeHandler<TAny> typeHandler => typeHandler.Read(buf, len, async: false, fieldDescription).Result,
                _ => ReadCustom<TAny>(buf, len, async: false, fieldDescription).Result
            };
        }

        protected internal virtual ValueTask<TAny> ReadCustom<TAny>(EDBReadBuffer buf, int len, bool async, FieldDescription? fieldDescription)
        ...
        /// </summary>
        public abstract ValueTask<object> ReadAsObject(EDBReadBuffer buf, int len, bool async, FieldDescription? fieldDescription = null);

        /// <summary>
        /// Version of <see cref="ReadAsObject(EDBReadBuffer,int,bool,FieldDescription?)"/> that's called when we know the entire value
        /// is already buffered in memory (i.e. in non-sequential mode).
        /// </summary>
        internal object ReadAsObject(EDBReadBuffer buf, int len, FieldDescription? fieldDescription = null)
        {
            Debug.Assert(buf.ReadBytesLeft >= len);
        ...
        /// <summary>
        /// Version of <see cref="ReadPsvAsObject(EDBReadBuffer,int,bool,FieldDescription?)"/> that's called when we know the entire value
        /// is already buffered in memory (i.e. in non-sequential mode).
        /// </summary>
        ...
        /// <summary>
        /// <para>Called to validate and get the length of a value of a generic <see cref="EDBParameter{T}"/>.</para>
        ...
        protected internal int ValidateAndGetLength<TAny>(
            [DisallowNull] TAny value, [NotNullIfNotNull("lengthCache")] ref EDBLengthCache? lengthCache, EDBParameter? parameter)
        {
            Debug.Assert(value is not DBNull);

            return this switch
            {
                IEDBSimpleTypeHandler<TAny> simpleTypeHandler => simpleTypeHandler.ValidateAndGetLength(value, parameter),
                IEDBTypeHandler<TAny> typeHandler => typeHandler.ValidateAndGetLength(value, ref lengthCache, parameter),
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public async Task WriteWithLength<TAny>(TAny? value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
        {
        ...
        // Source-generated
        public abstract int ValidateObjectAndGetLength(object value, ref EDBLengthCache? lengthCache, EDBParameter? parameter);
        ...
        /// </param>
        // Source-generated
        public abstract Task WriteObjectWithLength(object? value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default);
        ...
        /// </summary>
        public abstract EDBTypeHandler CreateArrayHandler(PostgresArrayType pgArrayType, ArrayNullabilityMode arrayNullabilityMode);

        /// <summary>
        /// Creates a type handler for ranges of this handler's type.
        /// </summary>
        public abstract EDBTypeHandler CreateRangeHandler(PostgresType pgRangeType);

        /// <summary>
        /// Creates a type handler for multiranges of this handler's type.
        /// </summary>
        public abstract EDBTypeHandler CreateMultirangeHandler(PostgresMultirangeType pgMultirangeType);

        /// <summary>
        /// Used to create an exception when the provided type can be converted and written, but an
        /// instance of <see cref="EDBParameter"/> is required for caching of the converted value
        /// (in <see cref="EDBParameter.ConvertedValue"/>.
        /// </summary>
