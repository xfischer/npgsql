...
namespace EnterpriseDB.EDBClient
{
    sealed class MultiHostConnectorPool : ConnectorSource
    {
        ...
        public MultiHostConnectorPool(EDBConnectionStringBuilder settings, string connString) : base(settings, connString)
        {
        ...
        async ValueTask<EDBConnector?> TryGetIdleOrNew(EDBConnection conn, TimeSpan timeoutPerHost, bool async,
            ...
            for (var i = 0; i < pools.Length; i++)
            {
                ...
                try
                {
                    if (pool.TryGetIdleConnector(out connector))
                    {
                        ...
                        {
                            clusterState = await connector.QueryClusterState(new EDBTimeout(timeoutPerHost), async, cancellationToken);
        ...
        async ValueTask<EDBConnector?> TryGet(EDBConnection conn, TimeSpan timeoutPerHost, bool async, TargetSessionAttributes preferredType,
        ...
        internal override async ValueTask<EDBConnector> Get(EDBConnection conn, EDBTimeout timeout, bool async, CancellationToken cancellationToken)
        {
        ...
        internal override void Return(EDBConnector connector)
            => throw new EDBException("EDB bug: a connector was returned to " + nameof(MultiHostConnectorPool));

        internal override bool TryGetIdleConnector([NotNullWhen(true)] out EDBConnector? connector)
            => throw new EDBException("EDB bug: trying to get an idle connector from " + nameof(MultiHostConnectorPool));
        ...
        internal override ValueTask<EDBConnector?> OpenNewConnector(EDBConnection conn, EDBTimeout timeout, bool async, CancellationToken cancellationToken)
            => throw new EDBException("EDB bug: trying to open a new connector from " + nameof(MultiHostConnectorPool));
