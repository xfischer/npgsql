...
namespace EnterpriseDB.EDBClient
{
    ...
    {
        static readonly EDBLogger Log = EDBLogManager.CreateLogger(nameof(MultiplexingConnectorPool));
        ...
        readonly ChannelReader<EDBCommand> _multiplexCommandReader;
        internal ChannelWriter<EDBCommand> MultiplexCommandWriter { get; }
        ...
        internal MultiplexingConnectorPool(
            EDBConnectionStringBuilder settings, string connString, MultiHostConnectorPool? parentPool = null)
        ...
        /// 2. Cause any connection exceptions (e.g. bad username) to be thrown from EDBConnection.Open
        /// 3. Start the multiplexing write loop after we've made sure that's the exact pool we're going to use
        /// </summary>
        internal async Task BootstrapMultiplexing(EDBConnection conn, EDBTimeout timeout, bool async, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var __ = Task.Run(MultiplexingWriteLoop, CancellationToken.None)
                    .ContinueWith(t =>
                    {
                        ...
                        Log.Error("Exception in multiplexing write loop, this is an EDB bug, please file an issue.",
                                t.Exception!);
        ...
        async Task MultiplexingWriteLoop()
        {
            ...
            bool WriteCommand(EDBConnector connector, EDBCommand command, ref MultiplexingStats stats)
            {
            ...
            void Flush(EDBConnector connector, ref MultiplexingStats stats)
            {
            ...
            void FailWrite(EDBConnector connector, Exception exception)
            {
            ...
            static void CompleteWrite(EDBConnector connector, ref MultiplexingStats stats)
            {
