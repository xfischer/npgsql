...
namespace EnterpriseDB.EDBClient
{
    ...
    /// a PostgreSQL table. Initiated by <see cref="EDBConnection.BeginBinaryExport(string)"/>
    /// </summary>
    public sealed class EDBBinaryExporter : ICancelable
    {
        ...
        EDBTypeHandler?[] _typeHandlerCache;

        static readonly EDBLogger Log = EDBLogManager.CreateLogger(nameof(EDBBinaryExporter));
        ...
        internal EDBBinaryExporter(EDBConnector connector)
        {
        ...
        internal async Task Init(string copyToCommand, bool async, CancellationToken cancellationToken = default)
        {
            ...
            _typeHandlerCache = new EDBTypeHandler[NumColumns];
        ...
        /// corruption will occur. If in doubt, use <see cref="Read{T}(EDBDbType)"/> to manually
        /// specify the type.
        /// </typeparam>
        ...
        /// <returns>The value of the column</returns>
        public T Read<T>(EDBDbType type) => Read<T>(type, false).GetAwaiter().GetResult();
        ...
        /// <typeparam name="T">The .NET type of the column to be read.</typeparam>
        /// <returns>The value of the column</returns>
        public ValueTask<T> ReadAsync<T>(EDBDbType type, CancellationToken cancellationToken = default)
        {
            using (NoSynchronizationContextScope.Enter())
                return Read<T>(type, true, cancellationToken);
        }

        ValueTask<T> Read<T>(EDBDbType type, bool async, CancellationToken cancellationToken = default)
        {
            CheckDisposed();
            if (_column == -1 || _column == NumColumns)
                throw new InvalidOperationException("Not reading a row");
            ...
            var handler = _typeHandlerCache[_column];
            if (handler == null)
                handler = _typeHandlerCache[_column] = _typeMapper.ResolveByEDBDbType(type);

            return DoRead<T>(handler, async, cancellationToken);
        }

        async ValueTask<T> DoRead<T>(EDBTypeHandler handler, bool async, CancellationToken cancellationToken = default)
        {
            try
            {
                using var registration = _connector.StartNestedCancellableOperation(cancellationToken);
...
#pragma warning restore CS8653
        ...
        async ValueTask DisposeAsync(bool async)
        {
            ...
            if (!_isConsumed && !_connector.IsBroken)
            {
                ...
                {
                    Log.Debug($"Caught an exception while disposing the {nameof(EDBBinaryExporter)}, indicating that it was cancelled.", e, _connector.Id);
                }
                catch (Exception e)
                {
                    Log.Error($"Caught an exception while disposing the {nameof(EDBBinaryExporter)}.", e, _connector.Id);
                }
            }
