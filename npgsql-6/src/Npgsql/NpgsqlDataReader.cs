...
#pragma warning restore CA1010
    {
        ...
        /// </summary>
        internal List<(int Offset, int Length)> _columns = new();
        ...
        /// </summary>
        EDBReadBuffer.ColumnStream? _columnStream;
        ...
        static readonly EDBLogger Log = EDBLogManager.CreateLogger(nameof(EDBDataReader));
        ...
        internal void Init(
            EDBCommand command, CommandBehavior behavior, List<EDBBatchCommand> statements, Task? sendTask = null)
        {
            Command = command;
        ...
        async Task<bool> NextResult(bool async, bool isConsuming = false, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                for (StatementIndex++; StatementIndex < _statements.Count; StatementIndex++)
                {
                    ...
                    if (!Command.IsWrappedByBatch && StatementIndex == 0 && (Command.Parameters.HasOutputParameters || Command.Parameters._hasReturnParam))//ZK: Changed EnterpriseDB Team
                    {
                    ...
                    else if (Command.CommandType == CommandType.StoredProcedure && Command.Parameters.Count > 0)  //ZK :Connector._isCallableStmt == true
                    {
                        msg = await Connector.ReadMessage(async);
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
        ...
        public IReadOnlyList<EDBBatchCommand> Statements => _statements.AsReadOnly();
        ...
        public override bool HasRows
            => State switch
            {
                ...
                ReaderState.Disposed => throw new ObjectDisposedException(nameof(EDBDataReader)),
                _ => _hasRows
            };
        ...
        async Task Consume(bool async)
        {
            ...
            else
            {
                ...
                while (true)
                {
                    try
                    {
                        var msg = await Connector.SkipUntil(BackendMessageCode.CommandComplete, BackendMessageCode.ReadyForQuery, async);
                        switch (msg.Code)
                        {
                        case BackendMessageCode.CommandComplete:
                            ProcessMessage(msg);
        ...
        public EDBDate GetDate(int ordinal) => GetFieldValue<EDBDate>(ordinal);
        ...
        [Obsolete(
            ...
            "See https://www.EnterpriseDB.EDBClient.org/doc/types/datetime.html for more information.")]
        public EDBTimeSpan GetInterval(int ordinal) => GetFieldValue<EDBTimeSpan>(ordinal);
        ...
        [Obsolete(
            ...
            "See https://www.EnterpriseDB.EDBClient.org/doc/types/datetime.html for more information.")]
        public EDBDateTime GetTimeStamp(int ordinal) => GetFieldValue<EDBDateTime>(ordinal);
        ...
        public new EDBNestedDataReader GetData(int ordinal)
        {
            ...
            {
                reader = new EDBNestedDataReader(this, null, UniqueRowId, 1, compositeType);
            }
            if (isArray)
                reader.InitArray();
            else
        ...
        ValueTask<Stream> GetStreamInternal(FieldDescription field, int ordinal, bool async, CancellationToken cancellationToken = default)
        {
            ...
            return new ValueTask<Stream>(_columnStream = (EDBReadBuffer.ColumnStream)Buffer.GetStream(ColumnLen, !_isSequential));

            static async Task<Stream> GetStreamLong(EDBDataReader reader, FieldDescription field, Task seekTask, CancellationToken cancellationToken)
            {
                ...
                return reader._columnStream = (EDBReadBuffer.ColumnStream)reader.Buffer.GetStream(reader.ColumnLen, !reader._isSequential);
            }
        }
...
#nullable restore
        ...
        public override int GetProviderSpecificValues(object[] values)
        {
            ...
            if (State != ReaderState.InResult)
            {
                throw State == ReaderState.Disposed
                    ? new ObjectDisposedException(nameof(EDBDataReader))
        ...
        void CheckResultSet()
        {
            ...
            case ReaderState.Disposed:
                throw new ObjectDisposedException(nameof(EDBDataReader));
            default:
                throw new InvalidOperationException("No resultset is currently being traversed");
            }
        }
