...
namespace EnterpriseDB.EDBClient
{
    ...
    /// a PostgreSQL table. Initiated by <see cref="EDBConnection.BeginRawBinaryCopy(string)"/>
    /// </summary>
    /// <remarks>
    ...
    public sealed class EDBRawCopyStream : Stream, ICancelable
    {
        ...
        internal EDBRawCopyStream(EDBConnector connector)
        {
            ...
            try
            {
                ...
                else
                {
                    if (!_isConsumed)
                    {
                        ...
                        {
                            Log.Debug($"Caught an exception while disposing the {nameof(EDBRawCopyStream)}, indicating that it was cancelled.", e, _connector.Id);
                        }
                        catch (Exception e)
                        {
                            Log.Error($"Caught an exception while disposing the {nameof(EDBRawCopyStream)}.", e, _connector.Id);
                        }
                    }
                }
            }
...
#pragma warning restore CS8625
    ...
    /// <summary>
    /// Writer for a text import, initiated by <see cref="EDBConnection.BeginTextImport(string)"/>.
    /// </summary>
    /// <remarks>
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    ...
    {
        internal EDBCopyTextWriter(EDBConnector connector, EDBRawCopyStream underlying) : base(underlying)
        {
        ...
        /// </summary>
        public void Cancel()
            => ((EDBRawCopyStream)BaseStream).Cancel();
        ...
        {
            using (NoSynchronizationContextScope.Enter())
                return ((EDBRawCopyStream)BaseStream).CancelAsync();
        }
    ...
    /// <summary>
    /// Reader for a text export, initiated by <see cref="EDBConnection.BeginTextExport(string)"/>.
    /// </summary>
    /// <remarks>
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    ...
    {
        internal EDBCopyTextReader(EDBConnector connector, EDBRawCopyStream underlying) : base(underlying)
        {
            if (underlying.IsBinary)
