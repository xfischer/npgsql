...
namespace EDBTypes
{
        ...
        IComparer<EDBDate>, IComparer
    {
        ...
        /// </summary>
        public static readonly EDBDate Epoch = new(1970, 1, 1);

        /// <summary>
        /// Represents the date 0001-01-01
        /// </summary>
        public static readonly EDBDate Era = new(0);

        public const int MaxYear = 5874897;
        public const int MinYear = -4714;
        public static readonly EDBDate MaxCalculableValue = new(MaxYear, 12, 31);
        public static readonly EDBDate MinCalculableValue = new(MinYear, 11, 24);

        public static readonly EDBDate Infinity = new(InternalType.Infinity);
        public static readonly EDBDate NegativeInfinity = new(InternalType.NegativeInfinity);
        ...
        public static EDBDate Now => new(DateTime.Now);
        public static EDBDate Today => Now;
        public static EDBDate Yesterday => Now.AddDays(-1);
        public static EDBDate Tomorrow => Now.AddDays(1);
        ...
        public EDBDate Add(in EDBTimeSpan interval)
        {
            switch (_type) {
            case InternalType.Infinity:
                return Infinity;
        ...
        internal EDBDate Add(in EDBTimeSpan interval, int carriedOverflow)
        {
            switch (_type) {
            case InternalType.Infinity:
                return Infinity;
        ...
        public int Compare(EDBDate x, EDBDate y) => x.CompareTo(y);
        ...
        public override bool Equals(object? obj) => obj is EDBDate date && Equals(date);
        ...
        public static bool operator ==(EDBDate x, EDBDate y) => x.Equals(y);
        public static bool operator !=(EDBDate x, EDBDate y) => !(x == y);
        public static bool operator <(EDBDate x, EDBDate y)  => x.CompareTo(y) < 0;
        public static bool operator >(EDBDate x, EDBDate y)  => x.CompareTo(y) > 0;
        public static bool operator <=(EDBDate x, EDBDate y) => x.CompareTo(y) <= 0;
        public static bool operator >=(EDBDate x, EDBDate y) => x.CompareTo(y) >= 0;
        ...
        public static EDBDate ToEDBDate(DateTime date)
            => new((int)(date.Ticks / EDBTimeSpan.TicksPerDay));
        ...
        public EDBDate Subtract(in EDBTimeSpan interval) => Add(-interval);
        ...
        public EDBDate(DateOnly date) : this(date.Year, date.Month, date.Day) {}

        public static DateOnly ToDateOnly(EDBDate date)
        {
            switch (date._type)
            {
            case InternalType.Infinity:
        ...
        public static explicit operator DateOnly(EDBDate date) => ToDateOnly(date);

        public static EDBDate ToEDBDate(DateOnly date)
            => new(date.Year, date.Month, date.Day);

        public static explicit operator EDBDate(DateOnly date) => ToEDBDate(date);
#endif
