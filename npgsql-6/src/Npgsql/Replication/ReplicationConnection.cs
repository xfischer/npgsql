    ...
    public abstract class ReplicationConnection : IAsyncDisposable
    {
            ...
            string command, bool temporarySlot, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                await buf.EnsureAsync(len);
                var consistentPoint   = EDBLogSequenceNumber.Parse(buf.ReadString(len));
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var columnStream = new EDBReadBuffer.ColumnStream(connector, startCancellableOperations: false);
                ...
                while (true)
                {
                    ...
                    case 'k': // Primary keepalive message
                    {
                        ...
                        if (Log.IsEnabled(EDBLogLevel.Trace))
                        {
            ...
            try
            {
                ...
                if (Log.IsEnabled(EDBLogLevel.Trace))
                {
                    Log.Trace("Feedback message sent with " +
                              $"{nameof(LastReceivedLsn)} of {new EDBLogSequenceNumber(unchecked((ulong)lastReceivedLsn))}, " +
                              $"{nameof(LastFlushedLsn)} of {new EDBLogSequenceNumber(unchecked((ulong)lastFlushedLsn))}, " +
                              $"{nameof(LastAppliedLsn)} of {new EDBLogSequenceNumber(unchecked((ulong)lastAppliedLsn))} " +
