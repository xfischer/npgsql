...
namespace EnterpriseDB.EDBClient
{
    static class TaskExtensions
    {
        ...
        /// <param name="timeout">How much time to allow <paramref name="task"/> to complete before throwing a <see cref="TimeoutException"/></param>
        /// <returns>An awaitable task that represents the original task plus the timeout</returns>
        internal static async Task WithTimeout(this Task task, EDBTimeout timeout)
        {
            if (!timeout.IsSet)
            {
                await task;
        ...
        internal static Task WithCancellationAndTimeout(this Task task, EDBTimeout timeout, CancellationToken cancellationToken)
            => task.WithCancellation(cancellationToken).WithTimeout(timeout);
#endif

        internal static async Task ExecuteWithTimeout(Func<CancellationToken, Task> func, EDBTimeout timeout, CancellationToken cancellationToken)
        {
        ...
        internal static async Task<TResult> ExecuteWithTimeout<TResult>(Func<CancellationToken, Task<TResult>> func, EDBTimeout timeout, CancellationToken cancellationToken)
        {
            CancellationTokenSource? combinedCts = null;
            try
            {
        ...
        static CancellationToken GetCombinedCancellationToken(ref CancellationTokenSource? combinedCts, EDBTimeout timeout, CancellationToken cancellationToken)
        {
