...
namespace EnterpriseDB.EDBClient.TypeMapping
{
    class BuiltInTypeHandlerResolver : TypeHandlerResolver
    {
        ...
        static readonly Dictionary<string, TypeMappingInfo> Mappings = new()
        {
            ...
            { "numeric",          new(EDBDbType.Numeric,  "decimal",          typeof(decimal), typeof(BigInteger)) },
            { "decimal",          new(EDBDbType.Numeric,  "decimal",          typeof(decimal), typeof(BigInteger)) },
...
#pragma warning disable 618 // EDBDateTime is obsolete, remove in 7.0
            { "timestamp without time zone", new(EDBDbType.Timestamp,   "timestamp without time zone", typeof(DateTime), typeof(EDBDateTime)) },
            { "timestamp",                   new(EDBDbType.Timestamp,   "timestamp without time zone", typeof(DateTime), typeof(EDBDateTime)) },
#pragma warning disable 618
            ...
            { "date",                        new(EDBDbType.Date,        "date",                        typeof(EDBDate)
#if NET6_0_OR_GREATER
                , typeof(DateOnly)
#endif
            ) },
            { "time without time zone",      new(EDBDbType.Time,        "timeout time zone"
#if NET6_0_OR_GREATER
                , typeof(TimeOnly)
#endif
            ) },
            { "time",                        new(EDBDbType.Time,        "timeout time zone"
#if NET6_0_OR_GREATER
                , typeof(TimeOnly)
#endif
            ) },
            ...
            { "interval",                    new(EDBDbType.Interval,    "interval", typeof(TimeSpan), typeof(EDBTimeSpan)) },
            ...
            { "tsrange",                       new(EDBDbType.Range | EDBDbType.Timestamp,   "tsrange") },
            { "tstzrange",                     new(EDBDbType.Range | EDBDbType.TimestampTz, "tstzrange") },
            { "tsmultirange",                  new(EDBDbType.Multirange | EDBDbType.Timestamp,   "tsmultirange") },
            { "tstzmultirange",                new(EDBDbType.Multirange | EDBDbType.TimestampTz, "tstzmultirange") },
...
#pragma warning disable 618 // EDBDateTime and EDBDate are obsolete, remove in 7.0
                { typeof(EDBDateTime), "timestamp without time zone" },
                { typeof(EDBDate),     "date" },
                { typeof(EDBTimeSpan), "interval" },
#pragma warning restore 618
...
#pragma warning restore 618
        ...
        static DateTimeKind GetMultirangeKind(EDBRange<DateTime>[] multirange)
        {
        ...
        public override EDBTypeHandler? ResolveValueTypeGenerically<T>(T value)
        {
            ...
            if (typeof(T) == typeof(EDBInterval))
                return _intervalHandler;
#pragma warning disable 618 // EDBDate and EDBTimeSpan are obsolete, remove in 7.0
            if (typeof(T) == typeof(EDBDate))
                return _dateHandler;
            if (typeof(T) == typeof(EDBTimeSpan))
                return _intervalHandler;
#pragma warning restore 618
        ...
        EDBTypeHandler UnknownHandler() => _unknownHandler ??= new UnknownTypeHandler(_connector);
