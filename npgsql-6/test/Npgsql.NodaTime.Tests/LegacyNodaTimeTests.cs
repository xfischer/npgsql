...
namespace EnterpriseDB.EDBClient.NodaTime.Tests
{
    ...
    public class LegacyNodaTimeTests : TestBase
    {
        ...
        public async Task Timestamp_read_infinity()
        {
            await using var conn = await OpenConnectionAsync();
            await using var cmd =
                new EDBCommand("SELECT 'infinity'::timestamp without time zone, '-infinity'::timestamp without time zone", conn);
            await using var reader = await cmd.ExecuteReaderAsync();
            await reader.ReadAsync();
        ...
        static EDBParameter[] TimestamptzParameters
        {
            get
            {
                ...
                return new EDBParameter[]
                {
                    ...
                    new() { Value = instance.InZone(DateTimeZoneProviders.Tzdb["America/New_York"]), EDBDbType = EDBDbType.TimestampTz },
                    new() { Value = instance.WithOffset(Offset.FromHours(1)), EDBDbType = EDBDbType.TimestampTz }
                };
            }
        }
...
#else
            Assert.Ignore(
                "Legacy NodaTime tests rely on the EnterpriseDB.EDBClient.EnableLegacyTimestampBehavior AppContext switch and can only be run in DEBUG builds");
#endif
