...
namespace EnterpriseDB.EDBClient.NodaTime.Tests
{
    ...
    public class NodaTimeTests : TestBase
    {
        ...
        public async Task Timestamp_write_values(LocalDateTime localDateTime, string expected)
        {
            ...
            await using var cmd = new EDBCommand("SELECT $1::text", conn)
            {
                Parameters =
                {
                    new() { Value = localDateTime, EDBDbType = EDBDbType.Timestamp }
                }
            };
        ...
        static EDBParameter[] TimestampInvalidParameters
            => new EDBParameter[]
            {
                new() { Value = new LocalDateTime().InUtc().ToInstant(), EDBDbType = EDBDbType.Timestamp },
                new() { Value = new DateTimeOffset(), EDBDbType = EDBDbType.Timestamp },
                new() { Value = DateTime.UtcNow, EDBDbType = EDBDbType.Timestamp }
            };
        ...
        public async Task Timestamptz_write_values(LocalDateTime localDateTime, string expected)
        {
            ...
            await using var cmd = new EDBCommand("SELECT $1::text", conn)
            {
                Parameters = { new() { Value = localDateTime.InUtc().ToInstant(), EDBDbType = EDBDbType.TimestampTz} }
            };
        ...
        static EDBParameter[] TimestamptzParameters
        {
            get
            {
                ...
                {
                    new() { Value = instance },
                    new() { Value = instance, EDBDbType = EDBDbType.TimestampTz },
        ...
        public async Task Timestamptz_resolution_failure(object value)
        {
            ...
            await using var cmd = new EDBCommand("SELECT $1::text", conn)
            {
                Parameters = { new() { EDBDbType = EDBDbType.TimestampTz, Value = value } }
            };
            ...
            get
            {
                ...
                return new[]
                {
                    new TestCaseData(
                            ...
                            new EDBRange<Instant>(start, true, end, false))
                        .SetName("InclusiveExclusive"),

                    new TestCaseData(
                            "(2020-01-01 12:00:00Z,2020-01-05 12:00:00Z]",
                            new Interval(start + Duration.Epsilon, end + Duration.Epsilon),
                            new EDBRange<Instant>(start, false, end, true))
                        .SetName("ExclusiveInclusive"),

                    new TestCaseData(
                            "(,]",
                            new Interval(null, null),
                            new EDBRange<Instant>(default, false, true, default, false, true))
        ...
        public async Task Timestamptz_range_read(string sql, Interval interval, EDBRange<Instant> range)
        {
            ...
            await using var conn = await OpenConnectionAsync();
            await using var cmd = new EDBCommand($"SELECT '{sql}'::tstzrange", conn);
            await using var reader = await cmd.ExecuteReaderAsync();
            await reader.ReadAsync();
            ...
            Assert.That(reader.GetFieldValue<EDBRange<Instant>>(0), Is.EqualTo(range));
            Assert.That(reader.GetFieldValue<EDBRange<ZonedDateTime>>(0), Is.EqualTo(new EDBRange<ZonedDateTime>(
            ...
            Assert.That(reader.GetFieldValue<EDBRange<OffsetDateTime>>(0),
                Is.EqualTo(new EDBRange<OffsetDateTime>(
            ...
            Assert.That(reader.GetFieldValue<EDBRange<DateTime>>(0),
                Is.EqualTo(new EDBRange<DateTime>(
            ...
            Assert.That(reader.GetFieldValue<EDBRange<DateTimeOffset>>(0),
                Is.EqualTo(new EDBRange<DateTimeOffset>(
        ...
        static EDBParameter[] Timestamptz_range_resolution_values
        {
            get
            {
                var (start, end) = (
                ...
                return new EDBParameter[]
                {
                    ...
                    new() { Value = new Interval(start, end), EDBDbType = EDBDbType.TimestampTzRange},
                    new() { Value = new EDBRange<Instant>(start, true, end, false) },
                    new() { Value = new EDBRange<ZonedDateTime>(start.InUtc(), true, end.InUtc(), false) },
                    new() { Value = new EDBRange<OffsetDateTime>(start.WithOffset(Offset.Zero), true, end.WithOffset(Offset.Zero), false) }
                };
            }
        }

        [Test, TestCaseSource(nameof(Timestamptz_range_resolution_values))]
        public async Task Timestamptz_range_resolution(EDBParameter parameter)
        {
            await using var conn = await OpenConnectionAsync();
            conn.TypeMapper.Reset();
            conn.TypeMapper.UseNodaTime();
...
SELECT '{[""2020-01-01 12:00:00Z"",""2020-01-05 12:00:00Z""), (""2020-01-07 12:00:00Z"",""2020-01-10 12:00:00Z""]}'::tstzmultirange", conn);
            ...
            var instantMultirange = new EDBRange<Instant>[]
            {
                new(start1, true, end1, false),
                new(start2, false, end2, true)
            };
            Assert.That(reader.GetFieldValue<EDBRange<Instant>[]>(0), Is.EqualTo(instantMultirange));
            Assert.That(reader.GetFieldValue<List<EDBRange<Instant>>>(0), Is.EqualTo(instantMultirange));

            var zonedDateTimeMultirange = new EDBRange<ZonedDateTime>[]
            {
                new(start1.InUtc(), true, end1.InUtc(), false),
                new(start2.InUtc(), false, end2.InUtc(), true)
            };
            Assert.That(reader.GetFieldValue<EDBRange<ZonedDateTime>[]>(0), Is.EqualTo(zonedDateTimeMultirange));
            Assert.That(reader.GetFieldValue<List<EDBRange<ZonedDateTime>>>(0), Is.EqualTo(zonedDateTimeMultirange));

            var offsetDateTimeMultirange = new EDBRange<OffsetDateTime>[]
            {
            ...
            Assert.That(reader.GetFieldValue<EDBRange<OffsetDateTime>[]>(0), Is.EqualTo(offsetDateTimeMultirange));
            Assert.That(reader.GetFieldValue<List<EDBRange<OffsetDateTime>>>(0), Is.EqualTo(offsetDateTimeMultirange));
        }

        static EDBParameter[] Timestamptz_multirange_resolution_parameters
        {
            get
            {
                var start1 = new LocalDateTime(2020, 1, 1, 12, 0, 0).InUtc().ToInstant();
                ...
                return new EDBParameter[]
                {
                    ...
                    new() { Value = intervalMultirange, EDBDbType = EDBDbType.TimestampTzMultirange },
                    new() { Value = new EDBRange<Instant>[]
                    {
                        new(start1, true, end1, false),
                        new(start2, true, end2, false)
                    }},
                    new() { Value = new EDBRange<ZonedDateTime>[]
                    {
                        new(start1.InUtc(), true, end1.InUtc(), false),
                        new(start2.InUtc(), true, end2.InUtc(), false)
                    }},
                    new() { Value = new EDBRange<OffsetDateTime>[]
                    {
                        new(start1.WithOffset(Offset.Zero), true, end1.WithOffset(Offset.Zero), false),
        ...
        public async Task Timestamptz_multirange_resolution(EDBParameter parameter)
        {
            await using var conn = await OpenConnectionAsync();
            MinimumPgVersion(conn, "14.0", "Multirange types were introduced in PostgreSQL 14");
            conn.TypeMapper.Reset();
...
#pragma warning disable 618 // EDBDate is obsolete, remove in 7.0
        ...
        public async Task Date()
        {
            ...
            using (var cmd = new EDBCommand("CREATE TEMP TABLE data (d1 DATE, d2 DATE, d3 DATE, d4 DATE, d5 DATE)", conn))
                cmd.ExecuteNonQuery();

            using (var cmd = new EDBCommand("INSERT INTO data VALUES (@p1, @p2, @p3, @p4, @p5)", conn))
            {
                cmd.Parameters.Add(new EDBParameter("p1", EDBDbType.Date) { Value = localDate });
                cmd.Parameters.Add(new EDBParameter { ParameterName = "p2", Value = localDate });
                cmd.Parameters.Add(new EDBParameter { ParameterName = "p3", Value = new LocalDate(-5, 3, 3) });
                cmd.Parameters.Add(new EDBParameter { ParameterName = "p4", Value = dateTime });
                cmd.Parameters.Add(new EDBParameter { ParameterName = "p5", Value = dateTime, EDBDbType = EDBDbType.Date });
                cmd.ExecuteNonQuery();
            }

            using (var cmd = new EDBCommand("SELECT d1::TEXT, d2::TEXT, d3::TEXT, d4::TEXT, d5::TEXT FROM data", conn))
            ...
            using (var cmd = new EDBCommand("SELECT * FROM data", conn))
            using (var reader = cmd.ExecuteReader())
            {
                reader.Read();
                ...
                Assert.That(() => reader.GetDate(0), Is.EqualTo(new EDBDate(localDate.Year, localDate.Month, localDate.Day)));
...
#pragma warning restore 618 // EDBDate is obsolete, remove in 7.0
        ...
        public async Task Date_DateOnly()
        {
            ...
            using (var cmd = new EDBCommand("CREATE TEMP TABLE data (d1 DATE)", conn))
                cmd.ExecuteNonQuery();

            using (var cmd = new EDBCommand("INSERT INTO data VALUES (@p1)", conn))
            {
                cmd.Parameters.Add(new EDBParameter { ParameterName = "p1", Value = dateOnly });
                cmd.ExecuteNonQuery();
            }

            using (var cmd = new EDBCommand("SELECT d1::TEXT FROM data", conn))
            using (var reader = cmd.ExecuteReader())
            {
                reader.Read();
                Assert.That(reader.GetValue(0), Is.EqualTo("2002-03-04"));
        ...
        public async Task Time()
        {
            ...
            cmd.Parameters.Add(new EDBParameter("p3", DbType.Time) { Value = timeSpan });
            using var reader = cmd.ExecuteReader();
            reader.Read();
        ...
        public async Task TimeTz()
        {
            ...
            using var cmd = new EDBCommand("SELECT @p1, @p2, @p3", conn);
            cmd.Parameters.Add(new EDBParameter("p1", EDBDbType.TimeTz) { Value = expected });
            cmd.Parameters.Add(new EDBParameter { ParameterName = "p2", Value = expected });
            cmd.Parameters.Add(new EDBParameter("p3", EDBDbType.TimeTz) { Value = dateTimeOffset });
            ...
            cmd.Parameters.Add(new("p", EDBDbType.TimeTz) { Value = DateTime.UtcNow });
            Assert.That(() => cmd.ExecuteReader(), Throws.Exception.TypeOf<InvalidCastException>());

            cmd.Parameters.Clear();
            cmd.Parameters.Add(new("p", EDBDbType.TimeTz) { Value = TimeSpan.Zero });
            Assert.That(() => cmd.ExecuteReader(), Throws.Exception.TypeOf<InvalidCastException>());
        }
        ...
        public async Task Interval_as_Period()
        {
            ...
            cmd.Parameters.Add(new EDBParameter("p1", EDBDbType.Interval) { Value = expectedPeriod });
        ...
        {
            await using var conn = await OpenConnectionAsync();
            using var cmd = new EDBCommand("SELECT make_interval(months => 2)", conn);
            using var reader = cmd.ExecuteReader();
            reader.Read();

            Assert.That(() => reader.GetFieldValue<Duration>(0), Throws.Exception.TypeOf<EDBException>().With.Message.EqualTo(
                "Cannot read PostgreSQL interval with non-zero months to NodaTime Duration. Try reading as a NodaTime Period instead."));
        }

        [Test]
        public async Task Interval_as_EDBInterval()
        {
            var expected = new EDBInterval(0, 1, 7384005000);
            await using var conn = await OpenConnectionAsync();
            using var cmd = new EDBCommand("SELECT @p1, @p2", conn);
            ...
            for (var i = 0; i < 2; i++)
            {
                Assert.That(reader.GetFieldValue<EDBInterval>(i), Is.EqualTo(expected));
            }
        }
        ...
        public async Task Date_range_read()
        {
            ...
            await using var conn = await OpenConnectionAsync();
            await using var cmd = new EDBCommand("SELECT '[2020-01-01,2020-01-05]'::daterange", conn);
            await using var reader = await cmd.ExecuteReaderAsync();
            await reader.ReadAsync();
            ...
            Assert.That(reader.GetFieldValue<EDBRange<LocalDate>>(0),
                Is.EqualTo(new EDBRange<LocalDate>(new(2020, 1, 1), true, new(2020, 1, 6), false)));
#if NET6_0_OR_GREATER
            Assert.That(reader.GetFieldValue<EDBRange<DateOnly>>(0),
                Is.EqualTo(new EDBRange<DateOnly>(new(2020, 1, 1), true, new(2020, 1, 6), false)));
#endif
        }

        static EDBParameter[] DaterangeParameters
        {
            get
            {
                var dateInterval = new DateInterval(new(2020, 1, 1), new(2020, 1, 5));
                var range = new EDBRange<LocalDate>(new(2020, 1, 1), new(2020, 1, 5));

                return new EDBParameter[]
                {
                    new() { Value = dateInterval },
                    new() { Value = range },
                    new() { Value = dateInterval, EDBDbType = EDBDbType.DateRange },
                    new() { Value = range, EDBDbType = EDBDbType.DateRange },
#if NET6_0_OR_GREATER
                    new() { Value = new EDBRange<DateOnly>(new(2020, 1, 1), new(2020, 1, 5)) },
                    new() { Value = new EDBRange<DateOnly>(new(2020, 1, 1), new(2020, 1, 5)), EDBDbType = EDBDbType.DateRange },
#endif
                };
            }
        }
        ...
        public async Task Date_range_resolution(EDBParameter parameter)
        {
            await using var conn = await OpenConnectionAsync();
            conn.TypeMapper.Reset();
            conn.TypeMapper.UseNodaTime();
        ...
        public async Task Date_multirange_read()
        {
            ...
            var rangeMultirange = new EDBRange<LocalDate>[]
            {
                new(new(2020, 1, 1), true, new(2020, 1, 6), false),
                new(new(2020, 1, 7), true, new(2020, 1, 11), false)
            };

            await using var cmd = new EDBCommand("SELECT '{[2020-01-01,2020-01-05], [2020-01-07,2020-01-10]}'::datemultirange", conn);
            await using var reader = await cmd.ExecuteReaderAsync();
            await reader.ReadAsync();
            ...
            Assert.That(reader.GetFieldValue<EDBRange<LocalDate>[]>(0), Is.EqualTo(rangeMultirange));
            Assert.That(reader.GetFieldValue<List<EDBRange<LocalDate>>>(0), Is.EqualTo(rangeMultirange));
        }

        static EDBParameter[] DatemultirangeParameters
        {
            get
            {
                var dateIntervalMultirange = new DateInterval[]
                ...
                return new EDBParameter[]
                {
                    ...
                    new() { Value = dateIntervalMultirange, EDBDbType = EDBDbType.DateMultirange },
                    new() { Value = rangeMultirange, EDBDbType = EDBDbType.DateMultirange }
                };
            }
        }

        [Test, TestCaseSource(nameof(DatemultirangeParameters))]
        public async Task Date_multirange_resolution(EDBParameter parameter)
        {
            await using var conn = await OpenConnectionAsync();
            MinimumPgVersion(conn, "14.0", "Multirange types were introduced in PostgreSQL 14");
            conn.TypeMapper.Reset();
