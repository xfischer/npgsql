    ...
    class CommandBuilderTests : TestBase
    {
        ...
        public async Task DeriveFunctionParameters_Various()
        {
            using (var conn = await OpenConnectionAsync())
            {
                ...
                Assert.That(cmd.Parameters[2].EDBDbType, Is.EqualTo(EDBDbType.Integer));
        ...
        public async Task DeriveFunctionParameters_NonExistingFunction()
        {
            using (var conn = await OpenConnectionAsync())
            {
                var invalidCommandName = new EDBCommand("invalidfunctionname", conn) { CommandType = CommandType.StoredProcedure };
        ...
        public async Task DeriveFunctionParameters_DotCharacterInFunctionName()
        {
            using (var conn = await OpenConnectionAsync())
            {
                ...
                var command = new EDBCommand(@"""My.Dotted.Function""", conn) { CommandType = CommandType.StoredProcedure };
                EDBCommandBuilder.DeriveParameters(command);
                Assert.AreEqual(EDBDbType.Integer, command.Parameters[0].EDBDbType);
                Assert.AreEqual(EDBDbType.Text, command.Parameters[1].EDBDbType);
            }
...
");
                var command = new EDBCommand("redundantfunc", conn) { CommandType = CommandType.StoredProcedure };
                EDBCommandBuilder.DeriveParameters(command);
...
RESET search_path;
");
                var command = new EDBCommand("schema1func", conn) { CommandType = CommandType.StoredProcedure };
                Assert.That(() => EDBCommandBuilder.DeriveParameters(command),
                    Throws.Exception.TypeOf<PostgresException>()
                    .With.Property(nameof(PostgresException.SqlState)).EqualTo("42883"));
            }
        }
...
$$ LANGUAGE SQL;
        ...
        [Test, Ignore(""), Description("Tests parameter derivation for a function that returns TABLE")]
...
#nullable restore
        ...
        public async Task DeriveTextCommandParameters_MultipleParameters()
        {
            using (var conn = await OpenConnectionAsync())
            {
                ...
                Assert.That(cmd.Parameters[0].EDBDbType, Is.EqualTo(EDBDbType.Integer));
                Assert.That(cmd.Parameters[1].EDBDbType, Is.EqualTo(EDBDbType.Double));
                Assert.That(cmd.Parameters[2].EDBDbType, Is.EqualTo(EDBDbType.Numeric));
...
#nullable restore
        ...
        public async Task DeriveTextCommandParameters_Array()
        {
            using (var conn = await OpenConnectionAsync())
            {
                var cmd = new EDBCommand("SELECT :a::integer[]", conn);
                var val = new[] { 7, 42 };

                EDBCommandBuilder.DeriveParameters(cmd);
                Assert.That(cmd.Parameters, Has.Count.EqualTo(1));
                Assert.That(cmd.Parameters[0].ParameterName, Is.EqualTo("a"));
                Assert.That(cmd.Parameters[0].EDBDbType, Is.EqualTo(EDBDbType.Integer | EDBDbType.Array));
        ...
        public async Task DeriveTextCommandParameters_UnmappedEnum()
        {
            using (var conn = await OpenConnectionAsync())
            {
                ...
                var cmd = new EDBCommand("SELECT :x::fruit", conn);
        ...
        public async Task DeriveTextCommandParameters_MappedEnum()
        {
            using (var conn = await OpenConnectionAsync())
            {
                ...
                var cmd = new EDBCommand("SELECT :x::fruit, :y::fruit[]", conn);
...
CREATE TYPE deriveparameterscomposite1 AS (x int, some_text text)");
                ...
                using (var cmd = new EDBCommand("SELECT @p1::deriveparameterscomposite1, @p2::deriveparameterscomposite1[]", conn))
                {
                    EDBCommandBuilder.DeriveParameters(cmd);
                    Assert.That(cmd.Parameters, Has.Count.EqualTo(2));
        ...
        public async Task GetUpdateCommandInfersParametersWithNpgsqDbType()
        {
            using (var conn = await OpenConnectionAsync())
            {
                ...
                Assert.That(daDataAdapter.UpdateCommand.Parameters[9].EDBDbType, Is.EqualTo(EDBDbType.Array | EDBDbType.Bigint));
                Assert.That(daDataAdapter.UpdateCommand.Parameters[10].EDBDbType, Is.EqualTo(EDBDbType.Array | EDBDbType.Varchar));
                ...
                Assert.That(daDataAdapter.UpdateCommand.Parameters[22].EDBDbType, Is.EqualTo(EDBDbType.Double));
                Assert.That(daDataAdapter.UpdateCommand.Parameters[24].EDBDbType, Is.EqualTo(EDBDbType.Bit));
                Assert.That(daDataAdapter.UpdateCommand.Parameters[26].EDBDbType, Is.EqualTo(EDBDbType.Varchar));
                Assert.That(daDataAdapter.UpdateCommand.Parameters[28].EDBDbType, Is.EqualTo(EDBDbType.Array | EDBDbType.Bigint));
                Assert.That(daDataAdapter.UpdateCommand.Parameters[30].EDBDbType, Is.EqualTo(EDBDbType.Array | EDBDbType.Varchar));
        ...
        [Test, IssueLink("https://github.com/EDB/EDB/issues/2560")]
        public void GetUpdateCommandWithColumnAliases()
        {
            ...
            using var cmd = new EDBCommand("SELECT Cod as CodAlias, Descr as DescrAlias, Data as DataAlias FROM data", conn);
            using var daDataAdapter = new EDBDataAdapter(cmd);
            using var cbCommandBuilder = new EDBCommandBuilder(daDataAdapter);
        ...
        [Test, IssueLink("https://github.com/EDB/EDB/issues/2846")]
...
");
                using var daDataAdapter = new EDBDataAdapter("SELECT cod, vettore FROM test ORDER By cod", conn);
                using var cbCommandBuilder = new EDBCommandBuilder(daDataAdapter);
                var dtTable = new DataTable();
