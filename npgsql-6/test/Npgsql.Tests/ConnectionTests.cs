...
namespace EnterpriseDB.EDBClient.Tests
{
    public class ConnectionTests : MultiplexingTestBase
    {
        ...
        public async Task ProvidePasswordCallback_is_used()
        {
            ...
            using (var conn = new EDBConnection(builder.ConnectionString) { ProvidePasswordCallback = ProvidePasswordCallback })
            {
                conn.Open();
        ...
        public void ProvidePasswordCallback_is_not_used()
        {
            ...
            using (var conn = new EDBConnection(connString) { ProvidePasswordCallback = ProvidePasswordCallback })
            {
                conn.Open();
        ...
        public async Task<string[]> ConnectionString_Host(string host)
        {
            ...
            var connections = new EDBConnection?[numberOfHosts];
            var returnValues = new string[numberOfHosts];
            try
            {
                for (var i = 0; i < connections.Length; i++)
                {
                    var c = new EDBConnection(connectionString);
                    await c.OpenAsync();
                    returnValues[i] = c.DataSource;
                    connections[i] = c;
                }

                // When multiplexing EDBConnection.DataSource is not set so we succeed
        ...
        public async Task Break_connector_in_pool(bool keepAlive)
        {
            ...
            var csb = new EDBConnectionStringBuilder(ConnectionString) { MaxPoolSize = 1 };
            if (keepAlive)
                csb.KeepAlive = 1;
            using var conn = new EDBConnection(csb.ToString());
            conn.Open();
...
LANGUAGE 'plpgsql'");
        ...
        public void Pool_by_password()
        {
            ...
            using (var goodConn = new EDBConnection(connectionString))
                goodConn.Open();

            var badConnectionString = new EDBConnectionStringBuilder(connectionString)
            {
                Password = "badpasswd"
            }.ConnectionString;
            using (var conn = new EDBConnection(badConnectionString))
...
CREATE TABLE record ()");
        ...
        {
            var builder = new EDBConnectionStringBuilder(ConnectionString) { IntegratedSecurity = false };
        ...
        {
            var builder = new EDBConnectionStringBuilder(ConnectionString) { Timeout = 1 };
            ...
            Assert.That(async () => await OpenConnectionAsync(connectionString),
                Throws.Exception.TypeOf<EDBException>()
                    .With.InnerException.TypeOf<TimeoutException>());
        }
