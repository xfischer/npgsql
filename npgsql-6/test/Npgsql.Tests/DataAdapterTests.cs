...
namespace EnterpriseDB.EDBClient.Tests
{
    public class DataAdapterTests : TestBase
    {
        ...
        [Test]
        public async Task DataAdapter_EDBCommand_in_constructor()
        {
            using var conn = await OpenConnectionAsync();
            using var command = new EDBCommand("SELECT 1", conn);
        ...
        public async Task Insert_with_DataSet()
        {
            ...
            var ds = new DataSet();
            var da = new EDBDataAdapter($"SELECT * FROM {table}", conn);

            da.InsertCommand = new EDBCommand($"INSERT INTO {table} (field_int2, field_timestamp, field_numeric) VALUES (:a, :b, :c)", conn);

            da.InsertCommand.Parameters.Add(new EDBParameter("a", DbType.Int16));
            da.InsertCommand.Parameters.Add(new EDBParameter("b", DbType.DateTime2));
            ...
            var dr2 = new EDBCommand($"SELECT field_int2, field_numeric, field_timestamp FROM {table}", conn).ExecuteReader();
            dr2.Read();
        ...
        public async Task Fill_with_empty_resultset()
        {
            ...
            var ds = new DataSet();
            var da = new EDBDataAdapter($"SELECT field_serial, field_int2, field_timestamp, field_numeric FROM {table} WHERE field_serial = -1", conn);
        ...
        public async Task Fill_add_with_key()
        {
            ...
            var ds = new DataSet();
            var da = new EDBDataAdapter($"select field_serial, field_int2, field_timestamp, field_numeric from {table}", conn);
        ...
        public async Task Update_letting_null_field_falue()
        {
            ...
            var command = new EDBCommand($"INSERT INTO {table} (field_int2) VALUES (2)", conn);
            ...
            da.UpdateCommand = new EDBCommand($"UPDATE {table} SET field_int2 = :a, field_timestamp = :b, field_numeric = :c WHERE field_serial = :d", conn);
            ...
            using var dr2 = new EDBCommand($"SELECT field_int2 FROM {table}", conn).ExecuteReader();
            dr2.Read();
        ...
        public async Task Fill_with_duplicate_column_name()
        {
            ...
            var ds = new DataSet();
            var da = new EDBDataAdapter($"SELECT field_serial, field_serial FROM {table}", conn);
            da.Fill(ds);
        }
        ...
        public async Task DoUpdateWithDataSet()
        {
            ...
            using var dr2 = new EDBCommand($"select * from {table}", conn).ExecuteReader();
            dr2.Read();
            Assert.AreEqual(4, dr2["field_int2"]);
        }
...
INSERT INTO {table} (interval) VALUES ('1 hour'::INTERVAL);");
        ...
        public async Task Auto_populate_adapter_commands()
        {
            ...
            var da = new EDBDataAdapter($"SELECT field_pk,field_int4 FROM {table}", conn);
            var builder = new EDBCommandBuilder(da);
            var ds = new DataSet();
            da.Fill(ds);
        ...
        public async Task Get_UpdateCommand()
        {
            ...
            using var da = new EDBDataAdapter($"SELECT field_pk, field_int4 FROM {table}", conn);
        ...
        public async Task Load_DataTable()
        {
            ...
            using var command = new EDBCommand($"SELECT char5, varchar5 FROM {table}", conn);
        ...
        public Task<IAsyncDisposable> SetupTempTable(EDBConnection conn, out string table)
