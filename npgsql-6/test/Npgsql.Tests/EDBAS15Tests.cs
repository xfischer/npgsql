...
#nullable restore
            ...
            [Test]
        public void DB_2021_RowVarMultipleItemINTOListTest()
        {
#nullable disable
            TestUtil.MinimumPgVersion(conn, "15.0.0");
#nullable restore

            Execute("DROP FUNCTION db2021_fn2");
            Execute("DROP TYPE ct1");
            Execute("DROP TYPE ct3");
            Execute("CREATE TYPE ct1 AS (x int, y int);");
            Execute("CREATE TYPE ct3 AS (x text, y text);");
            Execute("CREATE OR REPLACE FUNCTION db2021_fn2(OUT o1 ct1, OUT o2 ct3) RETURN void AS "
                    + " BEGIN "
                    + "   o1.x := 10;"
                    + "   o1.y := 20;"
                    + "   o2.x := 'ten';"
            ...
            try
            {
                var callProc = new EDBCommand("db2021_fn2", conn);
...
#nullable restore
            ...
            try
            {
                using (var cmd = new EDBCommand(query, conn))
                {
                    ...
                    rs.Read();
                    Assert.AreEqual("80 days 13:30:00", rs.GetInterval(0).ToString());
