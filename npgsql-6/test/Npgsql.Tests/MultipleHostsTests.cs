...
namespace EnterpriseDB.EDBClient.Tests
{
    ...
    public class MultipleHostsTests : TestBase
    {
            ...
            string targetSessionAttributes, MockState[] servers, int expectedServer)
        {
            ...
            connectionString = new EDBConnectionStringBuilder(connectionString)
            {
        ...
        public async Task Valid_host_not_found(string targetSessionAttributes, MockState[] servers)
        {
            ...
            var exception = Assert.ThrowsAsync<EDBException>(async () => await OpenConnectionAsync(connectionString))!;
        ...
        public async Task Enlist_depends_on_session_attributes()
        {
            ...
            var primaryCsb = new EDBConnectionStringBuilder(defaultConnectionString)
            {
                TargetSessionAttributes = "primary",
            };

            var standbyCsb = new EDBConnectionStringBuilder(defaultConnectionString)
            {
                TargetSessionAttributes = "standby",
            };

            var preferPrimaryCsb = new EDBConnectionStringBuilder(defaultConnectionString)
            {
                TargetSessionAttributes = "prefer-primary",
            };

            var preferStandbyCsb = new EDBConnectionStringBuilder(defaultConnectionString)
            {
            ...
            EDBConnector primaryConnector;
            EDBConnector standbyConnector;
        ...
        public async Task Cluster_offline_state_on_connection_failure()
        {
            ...
            var csb = new EDBConnectionStringBuilder(server.ConnectionString);
            await using var conn = new EDBConnection(csb.ConnectionString);
        ...
        public async Task Cluster_offline_state_on_query_execution_pg_critical_failure()
        {
            ...
            var csb = new EDBConnectionStringBuilder(postmaster.ConnectionString);
        ...
        public void IntegrationTest([Values] bool loadBalancing, [Values] bool alwaysCheckHostState)
        {
            ...
            Task Client(EDBConnectionStringBuilder csb, string targetSessionAttributes)
            {
                csb = csb.Clone();
