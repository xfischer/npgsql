...
INSERT INTO {table} (name) VALUES ('Text with '' single quote');");
        ...
        public async Task GetDataTypeName_enum()
        {
            ...
            conn.ReloadTypes();
            using var cmd = new EDBCommand("SELECT 'one'::my_enum", conn);
            using var reader = await cmd.ExecuteReaderAsync(Behavior);
            reader.Read();
        ...
        public async Task GetDataTypeName_domain()
        {
            ...
            conn.ReloadTypes();
            using var cmd = new EDBCommand("SELECT 'one'::my_domain", conn);
            using var reader = await cmd.ExecuteReaderAsync(Behavior);
            reader.Read();
...
#pragma warning restore 618
        ...
        static Func<EDBDataReader, int, Task<Stream>> BuildStreamGetter(bool isAsync)
            => isAsync
                ? (Func<EDBDataReader, int, Task<Stream>>)((r, index) => r.GetStreamAsync(index))
        ...
        public async Task SafeReadException()
        {
            ...
            using var cmd = new EDBCommand(@"SELECT 1, 'hello'", conn);
