...
namespace EnterpriseDB.EDBClient.Tests.SPL
{
    internal class EDBStaticCursorTest : TestBase
    {
        ...
        public void CursorAttributeIsopenTest()
        {
            ...
            var beforOpen = bool.Parse(cstmt.Parameters[0].Value.ToString());
            var afterOpen = bool.Parse(cstmt.Parameters[1].Value.ToString());
            var afterClose = bool.Parse(cstmt.Parameters[2].Value.ToString());
