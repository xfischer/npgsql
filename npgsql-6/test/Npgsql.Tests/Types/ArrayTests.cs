...
namespace EnterpriseDB.EDBClient.Tests.Types
{
    ...
    public class ArrayTests : MultiplexingTestBase
    {
        ...
        public async Task Array_resolution()
        {
            ...
            {
                cmd.Parameters.AddWithValue("p", EDBDbType.Array | EDBDbType.Integer, DBNull.Value);
            ...
            using (var cmd = new EDBCommand("SELECT @p", conn))
            {
                cmd.Parameters.Add(new EDBParameter { ParameterName = "p", Value = new int[0] });
                using var reader = await cmd.ExecuteReaderAsync();
            ...
            using (var cmd = new EDBCommand("SELECT @p", conn))
            {
                cmd.Parameters.Add(new EDBParameter { ParameterName="p", DataTypeName = "integer[]", Value = DBNull.Value });
                using (var reader = await cmd.ExecuteReaderAsync())
                {
                    reader.Read();
                    Assert.That(reader.GetDataTypeName(0), Is.EqualTo("integer[]"));
            ...
            conn.ReloadTypes();
            using (var cmd = new EDBCommand("SELECT '{1, 3}'::INTEGER[]", conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                reader.Read();
                Assert.That(reader.GetDataTypeName(0), Is.EqualTo("integer[]"));
        ...
        public async Task Ints()
        {
            ...
            var p1 = new EDBParameter("p1", EDBDbType.Array | EDBDbType.Integer);
            var p2 = new EDBParameter { ParameterName = "p2", Value = expected };
            var p3 = new EDBParameter<int[]>("p3", expected);
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new EDBCommand("SELECT '{1, NULL, 2}'::integer[]", conn);
            using var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
        ...
        {
            using var pool = CreateTempPool(new EDBConnectionStringBuilder(ConnectionString){ ArrayNullabilityMode = mode}, out var connectionString);
            await using var conn = await OpenConnectionAsync(connectionString);
            await using var cmd = new EDBCommand("SELECT onedim, twodim FROM (VALUES" +
        ...
        public async Task Empty_array()
        {
            ...
            cmd.Parameters.Add(new EDBParameter("p", EDBDbType.Array | EDBDbType.Integer) { Value = new int[0] });
            var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
        ...
        public async Task Empty_multidimensional_array()
        {
            ...
            cmd.Parameters.AddWithValue("p", EDBDbType.Array | EDBDbType.Integer, expected);
        ...
        {
            using var conn = await OpenConnectionAsync();
            using var cmd = new EDBCommand("SELECT ARRAY[[1], [2]]", conn);
        ...
        public async Task Read_null_as_non_nullable_array_throws()
        {
            ...
            var expected = new int?[] { 1, 5, null, 9 };
            cmd.Parameters.AddWithValue("p1", EDBDbType.Array | EDBDbType.Integer, expected);
        ...
        public async Task Strings_with_null()
        {
            ...
            using var cmd = new EDBCommand("SELECT @p", conn);
            var p = new EDBParameter("p", EDBDbType.Array | EDBDbType.Text) {Value = expected};
            cmd.Parameters.Add(p);
            using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SequentialAccess);
            reader.Read();
        ...
        public async Task Zero_dimensional()
        {
            ...
            var p = new EDBParameter("p", EDBDbType.Array | EDBDbType.Integer) { Value = expected };
            cmd.Parameters.Add(p);
            var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
...
#pragma warning disable 618 // EDBDate is obsolete, remove in 7.0
        [Test, Ignore(""), Description("Reads a one-dimensional array dates, both as DateTime and as the provider-specific EDBDate")]
        public async Task Read_provider_specific_type()
        {
            ...
            var expectedPsv = new[] { new EDBDate(2014, 1, 4), new EDBDate(2014, 1, 8) };
            using var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
...
#pragma warning restore 618
        ...
        public async Task Read_non_zero_lower_bounded()
        {
            ...
            using (var cmd = new EDBCommand("SELECT '[2:3][2:3]={ {8,9}, {1,2} }'::INT[][]", conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                reader.Read();
        ...
        public async Task Array_of_byte_arrays()
        {
            ...
            var p1 = new EDBParameter("p1", EDBDbType.Array | EDBDbType.Bytea);
            var p2 = new EDBParameter { ParameterName = "p2", Value = expected };
            cmd.Parameters.Add(p1);
            cmd.Parameters.Add(p2);
            p1.Value = expected;
        ...
        public async Task IList_generic()
        {
            ...
            var p1 = new EDBParameter { ParameterName = "p1", Value = expected };
            var p2 = new EDBParameter { ParameterName = "p2", Value = expected };
            cmd.Parameters.Add(p1);
            cmd.Parameters.Add(p2);
            using var reader = await cmd.ExecuteReaderAsync();
        ...
        public void Generic_List_get_EDBDbType()
        {
        ...
        public async Task Array_of_domain()
        {
            ...
            conn.ReloadTypes();
            using var cmd = new EDBCommand("SELECT @p1::posint[], @p2::posint[][]", conn);
            var oneDim = new[] { 1, 3, 5, 9 };
            var twoDim = new[,] { { 1, 3 }, { 5, 9 } };
            cmd.Parameters.AddWithValue("p1", EDBDbType.Integer | EDBDbType.Array, oneDim);
            cmd.Parameters.AddWithValue("p2", EDBDbType.Integer | EDBDbType.Array, twoDim);
        ...
        public async Task Domain_of_array()
        {
            ...
            conn.ReloadTypes();
            using var cmd = new EDBCommand("SELECT @p1::int_array_1d, @p2::int_array_2d", conn);
            var oneDim = new[] { 1, 3, 5, 9 };
            var twoDim = new[,] { { 1, 3 }, { 5, 9 } };
            cmd.Parameters.AddWithValue("p1", EDBDbType.Integer | EDBDbType.Array, oneDim);
            cmd.Parameters.AddWithValue("p2", EDBDbType.Integer | EDBDbType.Array, twoDim);
