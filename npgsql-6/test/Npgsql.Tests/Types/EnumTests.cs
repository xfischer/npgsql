    ...
    public class EnumTests : TestBase
    {
        ...
        public async Task Enum_resolution_with_global_mapping()
        {
            ...
            try
            {
                ...
                conn.ReloadTypes();
                using (var cmd = new EDBCommand($"SELECT 'happy'::{type}", conn))
                using (var reader = await cmd.ExecuteReaderAsync())
                {
                    reader.Read();
                    Assert.That(reader.GetDataTypeName(0), Is.EqualTo($"public.{type}"));
        ...
        public async Task Late_mapping()
        {
            ...
            var cmd = new EDBCommand($"SELECT @p1::{type}, @p2::{type}", conn);
...
CREATE TYPE {type2} AS ENUM ('label1', 'label2', 'label3')");
        ...
        public async Task Global_mapping()
        {
            ...
            using (var conn = await OpenConnectionAsync())
            {
                ...
                conn.ReloadTypes();
                const Mood expected = Mood.Ok;
                using (var cmd = new EDBCommand($"SELECT @p::{type}", conn))
                {
        ...
        {
            using var conn = await OpenConnectionAsync();
            EDBConnection.GlobalTypeMapper.MapEnum<Mood>("unknown_enum");
            try
            {
            ...
            {
                EDBConnection.GlobalTypeMapper.UnmapEnum<Mood>("unknown_enum");
            }
        }
        ...
        public async Task Array()
        {
            ...
            using var cmd = new EDBCommand($"SELECT @p1::{type}[], @p2::{type}[]", conn);
            var p1 = new EDBParameter
            {
                ParameterName = "p1",
                DataTypeName = $"{type}[]",
        ...
        public async Task Read_unmapped_enum_as_string()
        {
            ...
            await conn.ExecuteNonQueryAsync($"CREATE TYPE {type} AS ENUM ('Sad', 'Ok', 'Happy')");
            conn.ReloadTypes();
            using var cmd = new EDBCommand($"SELECT 'Sad'::{type}, ARRAY['Ok', 'Happy']::{type}[]", conn);
            using var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
...
CREATE TABLE {table} (id SERIAL, value1 {type}, value2 {type});");
            ...
            using var cmd = new EDBCommand($"INSERT INTO {table} (id, value1, value2) VALUES (default, @p1, @p2);", conn);
            cmd.Parameters.AddWithValue("p2", EDBDbType.Unknown, expected);
            var p2 = new EDBParameter("p1", EDBDbType.Unknown) {Value = expected};
            cmd.Parameters.Add(p2);
            cmd.ExecuteNonQuery();
        }
        ...
        public async Task Write_enum_as_EDBDbType_Unknown()
        {
            using var conn = await OpenConnectionAsync();
            await using var _ = await GetTempTypeName(conn, out var type);
            await using var __ = await GetTempTableName(conn, out var table);
...
CREATE TABLE {table} (value1 {type})");
            ...
            using var cmd = new EDBCommand($"INSERT INTO {table} (value1) VALUES (@p1);", conn);
            cmd.Parameters.AddWithValue("p1", EDBDbType.Unknown, expected);
            cmd.ExecuteNonQuery();
        }
        ...
        public async Task Name_translation_default_snake_case()
        {
            ...
            EDBConnection.GlobalTypeMapper.MapEnum<NameTranslationEnum>(enumName2);
            try
            {
                using var conn = await OpenConnectionAsync();
...
CREATE TYPE {schema2}.my_enum AS ENUM ('alpha');");
                ...
                // Global mapping
                EDBConnection.GlobalTypeMapper.MapEnum<Enum1>($"{schema1}.my_enum");
                EDBConnection.GlobalTypeMapper.MapEnum<Enum2>($"{schema2}.my_enum");
                using (var conn = await OpenConnectionAsync())
                {
                    using (var cmd = new EDBCommand("SELECT @p1, @p2", conn))
                    {
            ...
            {
                EDBConnection.GlobalTypeMapper.UnmapEnum<Enum1>($"{schema1}.my_enum");
                EDBConnection.GlobalTypeMapper.UnmapEnum<Enum2>($"{schema2}.my_enum");
            }
        }
        ...
        public async Task GetPostgresType()
        {
            ...
            using var cmd = new EDBCommand($"SELECT 'ok'::{type}", conn);
            using var reader = await cmd.ExecuteReaderAsync();
            reader.Read();
