...
sealed class TypeHandlerSourceGenerator : ISourceGenerator
{
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        ...
        var (simpleTypeHandlerInterfaceSymbol, typeHandlerInterfaceSymbol) = (
            compilation.GetTypeByMetadataName("EnterpriseDB.EDBClient.Internal.TypeHandling.IEDBSimpleTypeHandler`1"),
            compilation.GetTypeByMetadataName("EnterpriseDB.EDBClient.Internal.TypeHandling.IEDBTypeHandler`1"));

        if (simpleTypeHandlerInterfaceSymbol is null || typeHandlerInterfaceSymbol is null)
            throw new Exception("Could not find IEDBSimpleTypeHandler or IEDBTypeHandler");
            ...
            bool isSimple)
        {
            var usings = new HashSet<string>(
                new[]
                {
                    ...
                    "EnterpriseDB.EDBClient.Internal"
