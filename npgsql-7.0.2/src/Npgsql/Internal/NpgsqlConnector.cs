...
/// </summary>
public sealed partial class EDBConnector : IDisposable
{
                ...
                case 'S':
                    ...
                    else if (sslMode is SslMode.Prefer or SslMode.Require)
                    {
                        if (isFirstAttempt && sslMode is SslMode.Require && !Settings.TrustServerCertificate)
                            throw new ArgumentException(EDBStrings.CannotUseSslModeRequireWithoutTrustServerCertificate);
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
            ...
            bool isReadingPrependedMessage = false)
        {
            ...
            if (connector.PendingPrependedResponses > 0 && !isReadingPrependedMessage)
            {
                ...
                {
                    LogMessages.TryEDBTrace(connector.ConnectionLogger, $"Error: {e}");
            ...
            try
            {
                ...
                while (true)
                {
                    LogMessages.TryEDBTrace(connector.ConnectionLogger, $"--> ReadMessage");
                    ...
                    LogMessages.TryEDBTrace(connector.ConnectionLogger, $"--> ValidateBackendMessageCode {messageCode}");
                    PGUtil.ValidateBackendMessageCode(messageCode);
                    ...
                    LogMessages.TryEDBTrace(connector.ConnectionLogger, $"ParseServerMessage {messageCode}");
            ...
            {
                LogMessages.TryEDBTrace(connector.ConnectionLogger, $"EDBException: {e}");
    ...
    internal IBackendMessage? ParseServerMessage(EDBReadBuffer buf, BackendMessageCode code, int len, bool isPrependedMessage)
    {
        ...
        case BackendMessageCode.DataRow:
            ...
            {
#pragma warning disable CS8602 // Dereference of a possibly null reference.
                if (_currentCommand != null && _currentCommand.CommandType == CommandType.StoredProcedure
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
        ...
        case BackendMessageCode.PortalSuspended:
            throw new EDBException("Unimplemented message: " + code);
        case BackendMessageCode.ErrorResponse:
            return null;
    ...
    void ProcessNewTransactionStatus(TransactionStatus newStatus)
    {
        ...
        case TransactionStatus.Pending:
            throw new Exception($"Internal EDB bug: invalid TransactionStatus {nameof(TransactionStatus.Pending)} received, should be frontend-only");
        default:
            throw new InvalidOperationException(
                $"Internal EDB bug: unexpected value {newStatus} of enum {nameof(TransactionStatus)}. Please file a bug.");
        }
    }
        ...
        bool attemptPgCancellation = true)
    {
        UserCancellationToken = cancellationToken;
        AttemptPostgresCancellation = attemptPgCancellation;
        LogMessages.TryEDBTrace(ConnectionLogger, "StartNestedCancellableOperation");
        return _cancellationTokenRegistration =
            cancellationToken.Register(static c => ((EDBConnector)c!).PerformUserCancellation(), this);
    }
