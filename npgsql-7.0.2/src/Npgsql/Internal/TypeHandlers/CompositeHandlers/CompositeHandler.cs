...
[module: UnconditionalSuppressMessage(
    ...
    "IL2046", Scope = "type", Target = "EnterpriseDB.EDBClient.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2080", Scope = "type", Target = "EnterpriseDB.EDBClient.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2026", Scope = "type", Target = "EnterpriseDB.EDBClient.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2090", Scope = "type", Target = "EnterpriseDB.EDBClient.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2087", Scope = "type", Target = "EnterpriseDB.EDBClient.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2055", Scope = "type", Target = "EnterpriseDB.EDBClient.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]
[module: UnconditionalSuppressMessage(
    "Composite type mapping currently isn't trimming-safe, and warnings are generated at the MapComposite level.",
    "IL2077", Scope = "type", Target = "EnterpriseDB.EDBClient.Internal.TypeHandlers.CompositeHandlers.CompositeHandler")]

#endregion

namespace EnterpriseDB.EDBClient.Internal.TypeHandlers.CompositeHandlers;

sealed partial class CompositeHandler<T> : EDBTypeHandler<T>, ICompositeHandler
{
    ...
    public CompositeHandler(PostgresCompositeType postgresType, TypeMapper typeMapper, IEDBNameTranslator nameTranslator)
        : base(postgresType)
    {
    ...
    public override ValueTask<T> Read(EDBReadBuffer buffer, int length, bool async, FieldDescription? fieldDescription = null)
    {
        ...
        async ValueTask<T> ReadUsingMemberHandlers(EDBReadBuffer buffer, bool async)
        {
    ...
    public override async Task Write(T value, EDBWriteBuffer buffer, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
    {
        Initialize();
    ...
    static CompositeConstructorHandler<T>? CreateConstructorHandler(PostgresCompositeType pgType, TypeMapper typeMapper, IEDBNameTranslator nameTranslator)
    {
        ...
        foreach (var clrConstructor in clrType.GetConstructors())
        {
            ...
            for (var pgFieldIndex = 0; pgFieldIndex < pgFields.Count; ++pgFieldIndex)
            {
                ...
                if (!typeMapper.TryResolveByOID(pgField.Type.OID, out var handler))
                    throw new EDBException($"PostgreSQL composite type {pgType.DisplayName} has field {pgField.Type.DisplayName} with an unknown type (OID = {pgField.Type.OID}).");
    ...
    static CompositeMemberHandler<T>[] CreateMemberHandlers(PostgresCompositeType pgType, TypeMapper typeMapper, IEDBNameTranslator nameTranslator)
    {
        var pgFields = pgType.Fields;
