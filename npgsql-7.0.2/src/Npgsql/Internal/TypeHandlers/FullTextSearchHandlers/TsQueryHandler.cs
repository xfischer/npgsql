    ...
    IEDBTypeHandler<EDBTsQueryOr>, IEDBTypeHandler<EDBTsQueryFollowedBy>
{
    ...
    public override async Task Write(EDBTsQuery query, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
    {
        ...
        static async Task WriteCore(EDBTsQuery node, EDBWriteBuffer buf, bool async, CancellationToken cancellationToken = default)
        {
            if (buf.WriteSpaceLeft < 4)
                await buf.Flush(async, cancellationToken);
            ...
            {
                await WriteCore(((EDBTsQueryNot)node).Child, buf, async, cancellationToken);
                return;
            }
            ...
            await WriteCore(((EDBTsQueryBinOp)node).Right, buf, async, cancellationToken);
            await WriteCore(((EDBTsQueryBinOp)node).Left, buf, async, cancellationToken);
        }
    }
