...
sealed class UnmappedEnumHandler : TextHandler
{
    ...
    internal UnmappedEnumHandler(PostgresEnumType pgType, IEDBNameTranslator nameTranslator, Encoding encoding)
        : base(pgType, encoding)
        => _nameTranslator = nameTranslator;

    #region Read

    protected internal override async ValueTask<TAny> ReadCustom<TAny>(EDBReadBuffer buf, int len, bool async, FieldDescription? fieldDescription)
    {
    ...
    protected internal override int ValidateAndGetLengthCustom<TAny>(TAny value, ref EDBLengthCache? lengthCache, EDBParameter? parameter)
        => ValidateAndGetLength(value!, ref lengthCache, parameter);
    ...
    protected override Task WriteWithLengthCustom<TAny>(TAny value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken)
    ...
    public override Task WriteObjectWithLength(object? value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
    {
        ...
        async Task WriteWithLengthLong(object value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken)
        {
    ...
    [UnconditionalSuppressMessage("Unmapped enums currently aren't trimming-safe.", "IL2072")]
    internal Task Write(object value, EDBWriteBuffer buf, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async, CancellationToken cancellationToken = default)
    {
        var type = value.GetType();
        if (type == typeof(string))
    ...
    static TypeRecord CreateTypeRecord(Type type, IEDBNameTranslator nameTranslator)
    {
