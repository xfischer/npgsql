...
static class EDBActivitySource
{
    ...
    static EDBActivitySource()
    {
        var assembly = typeof(EDBActivitySource).Assembly;
        var version = assembly.GetCustomAttribute<AssemblyFileVersionAttribute>()?.Version ?? "0.0.0";
        Source = new("EDB", version);
    }

    internal static bool IsEnabled => Source.HasListeners();

    internal static Activity? CommandStart(EDBConnector connector, string commandText, CommandType commandType)
    {
