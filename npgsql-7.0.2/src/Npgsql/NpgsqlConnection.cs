...
public sealed class EDBConnection : DbConnection, ICloneable, IComponent
{
    ...
    internal Task Open(bool async, CancellationToken cancellationToken)
    {
        ...
        async Task PerformMultiplexingStartupCheck(bool async, CancellationToken cancellationToken)
        {
            try
            {
                ...
                LogMessages.OpenedMultiplexingConnection(_connectionLogger, Settings.Host!, Settings.Port, Settings.Database!, _userFacingConnectionString);
                ((MultiplexingDataSource)EDBDataSource).StartupCheckPerformed = true;
        ...
        get => _fullState switch
        {
            ConnectionState.Open => Connector == null
                ...
                : Connector.State switch
                {
                    ...
                    ConnectorState.Closed      => throw new InvalidOperationException("Internal EDB bug: connection is in state Open but connector is in state Closed"),
    ...
    public new EDBBatch CreateBatch() => new(this);
#else
    /// <summary>
    ...
    async ValueTask<EDBTransaction> BeginTransaction(IsolationLevel level, bool async, CancellationToken cancellationToken)
    {
        ...
        var connector = ConnectorBindingScope == ConnectorBindingScope.Transaction
            ? Connector
            : await StartBindingScope(ConnectorBindingScope.Transaction, EDBTimeout.Infinite, async, cancellationToken);
    ...
    async Task<EDBBinaryImporter> BeginBinaryImport(string copyFromCommand, bool async, CancellationToken cancellationToken = default)
    {
    ...
    async Task<EDBBinaryExporter> BeginBinaryExport(string copyToCommand, bool async, CancellationToken cancellationToken = default)
    {
    ...
    async Task<EDBRawCopyStream> BeginRawBinaryCopy(string copyCommand, bool async, CancellationToken cancellationToken = default)
    {
        if (copyCommand == null)
    ...
    public override DataTable GetSchema(string? collectionName, string?[]? restrictions)
        => EDBSchema.GetSchema(this, collectionName, restrictions, async: false).GetAwaiter().GetResult();
    ...
    {
        using (NoSynchronizationContextScope.Enter())
            return EDBSchema.GetSchema(this, collectionName, restrictions, async: true, cancellationToken);
    }
    ...
    object ICloneable.Clone()
    {
        ...
        var conn = _dataSource is null
            ? new EDBConnection(_connectionString)
