...
public sealed class EDBMultiHostDataSource : EDBDataSource
{
        ...
        for (var i = 0; i < pools.Length; i++)
        {
            ...
            try
            {
                if (pool.TryGetIdleConnector(out connector))
                {
                    ...
                    {
                        databaseState = await connector.QueryDatabaseState(new EDBTimeout(timeoutPerHost), async, cancellationToken);
                ...
                {
                    connector = await pool.OpenNewConnector(conn, new EDBTimeout(timeoutPerHost), async, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < pools.Length; i++)
        {
            ...
            try
            {
                connector = await pool.Get(conn, new EDBTimeout(timeoutPerHost), async, cancellationToken);
                if (databaseState == DatabaseState.Unknown)
                {
