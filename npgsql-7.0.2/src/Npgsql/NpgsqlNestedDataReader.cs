...
/// Can be retrieved using <see cref="EDBDataReader.GetData(int)"/> or
/// <see cref="EDBNestedDataReader.GetData(int)"/>.
/// </summary>
public sealed class EDBNestedDataReader : DbDataReader
{
    readonly EDBDataReader _outermostReader;
    ulong _uniqueOutermostReaderRowId;
    readonly EDBNestedDataReader? _outerNestedReader;
    EDBNestedDataReader? _cachedFreeNestedDataReader;
    ...
    readonly struct ColumnInfo
    {
        ...
        public readonly EDBTypeHandler TypeHandler;

        public ColumnInfo(uint typeOid, int bufferPos, EDBTypeHandler typeHandler)
        {
            TypeOid = typeOid;
            BufferPos = bufferPos;
    ...
    EDBReadBuffer Buffer => _outermostReader.Buffer;
    TypeMapper TypeMapper => _outermostReader.Connector.TypeMapper;

    internal EDBNestedDataReader(EDBDataReader outermostReader, EDBNestedDataReader? outerNestedReader,
    ...
    public new EDBNestedDataReader GetData(int ordinal)
    {
        ...
        {
            reader = new EDBNestedDataReader(_outermostReader, this, _uniqueOutermostReaderRowId, _depth + 1, compositeType);
        }
        if (isArray)
            reader.InitArray();
        else
    ...
    (EDBTypeHandler Handler, int Length) CheckRowAndColumnAndSeek(int ordinal)
    {
        var column = CheckRowAndColumn(ordinal);
