...
/// </summary>
static class EDBSchema
{
    public static Task<DataTable> GetSchema(EDBConnection conn, string? collectionName, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
    ...
    static EDBCommand BuildCommand(EDBConnection conn, StringBuilder query, string?[]? restrictions, params string[]? names)
        => BuildCommand(conn, query, restrictions, true, names);

    static EDBCommand BuildCommand(EDBConnection conn, StringBuilder query, string?[]? restrictions, bool addWhere, params string[]? names)
    {
        ...
        if (restrictions != null && names != null)
        {
            for (var i = 0; i < restrictions.Length && i < names.Length; ++i)
            {
                if (restrictions[i] is string restriction && restriction.Length != 0)
                {
                    ...
                    command.Parameters.Add(new EDBParameter(paramName, restriction));
                }
            }
        }
    ...
    static async Task<DataTable> GetDatabases(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
    ...
    static async Task<DataTable> GetSchemata(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
SELECT * FROM (
    ...
    static async Task<DataTable> GetTables(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
WHERE
    ...
    table_schema NOT IN ('pg_catalog', 'information_schema', 'sys')"); // EnterpriseDB Team : add sys schema
    ...
    static async Task<DataTable> GetColumns(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
JOIN pg_type AS typ ON typnamespace = ns.oid AND typname = udt_name");
    ...
    static async Task<DataTable> GetViews(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
WHERE table_schema NOT IN ('pg_catalog', 'information_schema', 'sys')"); // EnterpriseDB Team : add sys schema
    ...
    static async Task<DataTable> GetUsers(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
    ...
    static async Task<DataTable> GetIndexes(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
WHERE
    ...
    static async Task<DataTable> GetIndexColumns(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
WHERE
    ...
    static async Task<DataTable> GetConstraints(EDBConnection conn, string?[]? restrictions, string? constraintType, bool async, CancellationToken cancellationToken = default)
    {
...
FROM
    ...
    static async Task<DataTable> GetConstraintColumns(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
FROM pg_constraint c
    ...
    static DataTable GetDataSourceInformation(EDBConnection conn)
    {
        ...
        if (EDBCommand.EnableSqlRewriting)
        {
    ...
    static DataTable GetDataTypes(EDBConnection conn)
    {
                     ...
                     .Concat(connector.DatabaseInfo.CompositeTypes))
        {
            ...
            if (mapping.EDBDbType.HasValue)
                row["ProviderDbType"] = (int)mapping.EDBDbType.Value;
        }
        ...
        foreach (var arrayType in connector.DatabaseInfo.ArrayTypes)
        {
            ...
            if (elementMapping.EDBDbType.HasValue)
                row["ProviderDbType"] = (int)(elementMapping.EDBDbType.Value | EDBDbType.Array);
        }
        ...
        foreach (var rangeType in connector.DatabaseInfo.RangeTypes)
        {
            ...
            if (subtypeMapping.ClrTypes.Length > 0)
                row["DataType"] = typeof(EDBRange<>).MakeGenericType(subtypeMapping.ClrTypes[0]).FullName;
            if (subtypeMapping.EDBDbType.HasValue)
                row["ProviderDbType"] = (int)(subtypeMapping.EDBDbType.Value | EDBDbType.Range);
        }
        ...
        foreach (var domainType in connector.DatabaseInfo.DomainTypes)
        {
            ...
            if (baseMapping.EDBDbType.HasValue)
                row["ProviderDbType"] = (int)baseMapping.EDBDbType.Value;
        }
