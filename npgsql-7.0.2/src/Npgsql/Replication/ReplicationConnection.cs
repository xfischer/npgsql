...
public abstract class ReplicationConnection : IAsyncDisposable
{
    ...
    /// <see cref="EDBConnectionStringBuilder.Pooling"/>, <see cref="EDBConnectionStringBuilder.Enlist"/>,
    /// <see cref="EDBConnectionStringBuilder.Multiplexing" /> and <see cref="EDBConnectionStringBuilder.KeepAlive"/>
    ...
    public string ConnectionString {
        ...
        set
        {
            ...
            var cs = new EDBConnectionStringBuilder(value)
            {
                Pooling = false,
                Enlist = false,
                Multiplexing = false,
    ...
    /// </summary>
    public EDBLogSequenceNumber LastReceivedLsn
    {
        get => (EDBLogSequenceNumber)unchecked((ulong)Interlocked.Read(ref _lastReceivedLsn));
    ...
    /// </summary>
    public EDBLogSequenceNumber LastFlushedLsn
    {
        get => (EDBLogSequenceNumber)unchecked((ulong)Interlocked.Read(ref _lastFlushedLsn));
    ...
    /// </summary>
    public EDBLogSequenceNumber LastAppliedLsn
    {
        get => (EDBLogSequenceNumber)unchecked((ulong)Interlocked.Read(ref _lastAppliedLsn));
    ...
    public Task<ReplicationSystemIdentification> IdentifySystem(CancellationToken cancellationToken = default)
    {
        ...
        async Task<ReplicationSystemIdentification> IdentifySystemInternal(CancellationToken cancellationToken)
        {
            ...
            return new ReplicationSystemIdentification(
                (string)row[0], (uint)row[1], EDBLogSequenceNumber.Parse((string)row[2]), (string)row[3]);
        }
    }
    ...
    internal async Task<ReplicationSlotOptions> CreateReplicationSlot(string command, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return new ReplicationSlotOptions(slotName, EDBLogSequenceNumber.Parse(consistentPoint), snapshotName);
        }
        ...
        switch (slotType)
        {
            case "physical":
                ...
                return new PhysicalReplicationSlot(
                    ...
                    restartLsn == null ? null : EDBLogSequenceNumber.Parse(restartLsn),
                    restartTli);
            case null:
                return null;
            default:
                throw new NotSupportedException(
                    $"The replication slot type '{slotType}' is currently not supported by EnterpriseDB.EDBClient. Please file an issue.");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        EDBReadBuffer.ColumnStream? columnStream = null;
        ...
        try
        {
            ...
            columnStream = new EDBReadBuffer.ColumnStream(connector, startCancellableOperations: false);
            ...
            while (true)
            {
                ...
                case 'w': // XLogData
                {
                    ...
                    _cachedXLogDataMessage.Populate(new EDBLogSequenceNumber(startLsn), new EDBLogSequenceNumber(endLsn),
                ...
                case 'k': // Primary keepalive message
                {
                    ...
                    {
                        var endLsn = new EDBLogSequenceNumber(end);
                ...
                default:
                    throw Connector.Break(new EDBException($"Unknown replication message code '{code}'"));
                }
            }
        }
        finally
    ...
    public Task SendStatusUpdate(CancellationToken cancellationToken = default)
    {
        ...
        async Task SendStatusUpdateInternal(CancellationToken cancellationToken)
        {
            ...
            // However, we generally aren't safe for this in EDB, leaving as-is for now.
            if (Connector.State != ConnectorState.Replication)
        ...
        try
        {
            ...
            if (ReplicationLogger.IsEnabled(LogLevel.Trace))
            {
                LogMessages.SentReplicationFeedbackMessage(
                    ReplicationLogger,
                    new EDBLogSequenceNumber(unchecked((ulong)lastReceivedLsn)),
                    new EDBLogSequenceNumber(unchecked((ulong)lastFlushedLsn)),
                    new EDBLogSequenceNumber(unchecked((ulong)lastAppliedLsn)),
                    timestamp,
                    Connector.Id);
            }
        }
    ...
    async Task<object[]> ReadSingleRow(string command, CancellationToken cancellationToken = default)
    {
        ...
        for (var i = 0; i < results.Length; i++)
        {
            ...
            case "bytea":
                ...
                {
                    throw Connector.Break(
                        new EDBException($"Could not parse data as bytea in field {field.Name}", e));
                }

                continue;
            default:

                throw Connector.Break(new EDBException(
