...
sealed class DbColumnSchemaGenerator
{
    ...
    internal DbColumnSchemaGenerator(EDBConnection connection, RowDescriptionMessage rowDescription, bool fetchAdditionalInfo)
    {
...
ORDER BY attnum";
    ...
    internal async Task<ReadOnlyCollection<EDBDbColumn>> GetColumnSchema(bool async, CancellationToken cancellationToken = default)
    {
        ...
        if (populatedColumns != numFields)
            throw new EDBException("Could not load all columns for the resultset");
    ...
    /// </summary>
    void ColumnPostConfig(EDBDbColumn column, int typeModifier)
    {
        var typeMapper = _connection.Connector!.TypeMapper;

        column.EDBDbType = typeMapper.GetTypeInfoByOid(column.TypeOID).npgsqlDbType;
