...
public class BugTests : TestBase
{
    ...
    {
        using var conn = OpenConnection();
        using var cmd = new EDBCommand("SELECT ('one'::TEXT, 2)", conn);
    ...
    public async Task Bug1497()
    {
        ...
        await using var cmd = new EDBCommand($"SELECT * FROM {tableName}", conn);
    ...
    public async Task Bug1987()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            await using var cmd = new EDBCommand("SELECT @p", connection);
    ...
    public async Task Bug2046()
    {
        ...
        using var conn = OpenConnection();
        using var cmd = new EDBCommand("SELECT @p = 64.27245::real, 64.27245::real, @p", conn);
    ...
    public void Bug1761()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            try
            {
                ...
                using (var cmd = new EDBCommand("SELECT 1", connection))
                {
                    cmd.CommandText = "select 1;";
                    cmd.ExecuteNonQuery();
                }
...
WHERE table_name LIKE @p0 escape '\' AND (is_updatable = 'NO') = @p1";
    ...
    public void Bug1285()
    {
        ...
        cmd.Parameters.Add(new EDBParameter("@1", Guid.NewGuid()));
        cmd.ExecuteNonQuery();
    }
...
(
  ...
  CONSTRAINT ""pk_OrganisatieQmo_Organisatie_QueryModelObjects_Imp"" PRIMARY KEY (""Id"")
    ...
    public async Task Chunked_string_write_buffer_encoding_space()
    {
        ...
        await binaryImporter.WriteAsync(almostBufferFillingString, EDBDbType.Text);
        ...
        await binaryImporter.WriteAsync(longStringStartingWithAforementionedUnicodeCharacter, EDBDbType.Text);
    ...
    public async Task Chunked_char_array_write_buffer_encoding_space()
    {
        ...
        // get encoded to multiple bytes
        var longStringStartingWithAforementionedUnicodeCharacter = unicodeCharacterThatEncodesToThreeBytesInUtf8 + new string('a', 10000);
        await binaryImporter.WriteAsync(longStringStartingWithAforementionedUnicodeCharacter.ToCharArray(), EDBDbType.Text);
...
$$;");
    ...
    public void Bug3117()
    {
        ...
        {
            conn.Open();
            var okCommand = new EDBCommand(OkCommand, conn);
            okCommand.Prepare();
            using (okCommand.ExecuteReader()) { }

            var errorCommand = new EDBCommand(ErrorCommand, conn);
    ...
    public async Task SingleThreadedSynchronizationContext_deadlock()
    {
        ...
        using (var _ = syncContext.Enter())
        {
            ...
            using var cmd = new EDBCommand("SELECT generate_series(1, 500000); SELECT @p", connection);
