...
public class CommandTests : MultiplexingTestBase
{
    ...
    public async Task Timeout_async_hard()
    {
        ...
        var builder = new EDBConnectionStringBuilder(ConnectionString) { CommandTimeout = 1 };
    ...
    public async Task Cursor_statement()
    {
        ...
        var i = 0;
        var command = new EDBCommand($"DECLARE TE CURSOR FOR SELECT * FROM {table}", conn);
    ...
    public async Task Positional_parameter()
    {
        ...
        cmd.Parameters.Add(new EDBParameter { EDBDbType = EDBDbType.Integer, Value = 8 });
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT $1; SELECT $1", conn);
        cmd.Parameters.Add(new EDBParameter { EDBDbType = EDBDbType.Integer, Value = 8 });
    ...
    public async Task Named_parameters_are_not_supported_when_EnableSqlParsing_is_disabled()
    {
        ...
        cmd.Parameters.Add(new EDBParameter("p", 8));
    ...
    public async Task Unreferenced_positional_parameter_works()
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT 1", conn);
        cmd.Parameters.Add(new EDBParameter { Value = 8 });
        Assert.That(await cmd.ExecuteScalarAsync(), Is.EqualTo(1));
    }
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT $1, @p", conn);
        cmd.Parameters.Add(new EDBParameter { Value = 8 });
        cmd.Parameters.Add(new EDBParameter { ParameterName = "p", Value = 9 });
    ...
    public async Task Positional_output_parameters_are_not_supported()
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT $1", conn);
        cmd.Parameters.Add(new EDBParameter { Value = 8, Direction = ParameterDirection.InputOutput });
        Assert.That(() => cmd.ExecuteNonQueryAsync(), Throws.Exception.TypeOf<NotSupportedException>());
    }
    ...
    public async Task ExecuteScalar()
    {
        using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "name TEXT");
        using var command = new EDBCommand($"SELECT name FROM {table}", conn);
    ...
    public async Task Command_Dispose_does_not_close_reader()
    {
        ...
        Assert.That(() => cmd.ExecuteScalarAsync(), Throws.Exception.TypeOf<EDBOperationInProgressException>());
    }
...
$$ LANGUAGE plpgsql;";
        ...
        command = new EDBCommand(sproc, conn);
    ...
    public async Task TableDirect()
    {
        ...
        using var cmd = new EDBCommand(table, conn) { CommandType = CommandType.TableDirect };
    ...
    [Test]
    public async Task Send_EDBDbType_Unknown([Values(PrepareOrNot.NotPrepared, PrepareOrNot.Prepared)] PrepareOrNot prepare)
    {
        if (prepare == PrepareOrNot.Prepared && IsMultiplexing)
            return;
    ...
    public void Batched_small_then_big_statements_do_not_deadlock_in_sync_io()
    {
        ...
        using var cmd = new EDBCommand("SELECT generate_series(1, 500000); SELECT @p", conn);
        cmd.Parameters.AddWithValue("p", EDBDbType.Text, data);
        cmd.ExecuteNonQuery();
    }
    ...
    public async Task Prepare_with_positional_placeholders_after_named()
    {
        ...
        command.Parameters.Add(new() { EDBDbType = EDBDbType.Integer });
    ...
    public async Task Concurrent_read_write_failure_deadlock()
    {
        ...
        Assert.ThrowsAsync<EDBException>(async () => await queryTask);
    }
    ...
    public async Task Not_cancel_prepended_query([Values] bool failPrependedQuery)
    {
        ...
        await using var postmasterMock = PgPostmasterMock.Start(ConnectionString);
        var csb = new EDBConnectionStringBuilder(postmasterMock.ConnectionString)
        {
    ...
    public async Task Oversize_buffer_lost_messages()
    {
        ...
        await using var dataSource = EDBDataSource.Create(mock.ConnectionString);
