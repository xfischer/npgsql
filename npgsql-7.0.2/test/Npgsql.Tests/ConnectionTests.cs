...
public class ConnectionTests : MultiplexingTestBase
{
    ...
    {
        var csb = new EDBConnectionStringBuilder(ConnectionString) { Port = 44444, Pooling = pooled };
    ...
    public async Task Fail_connect_then_succeed([Values] bool pooling)
    {
        ...
        try
        {
            ...
            await using var conn2 = new EDBConnection(connString);
    ...
    public async Task Client_encoding_connection_param()
    {
        ...
        var connString = new EDBConnectionStringBuilder(ConnectionString) { ClientEncoding = "SQL_ASCII" };
        using (var conn = await OpenConnectionAsync(connString))
            Assert.That(await conn.ExecuteScalarAsync("SHOW client_encoding"), Is.EqualTo("SQL_ASCII"));
    }
    ...
    public async Task Unix_domain_socket()
    {
        ...
        var port = new EDBConnectionStringBuilder(ConnectionString).Port;
    ...
    {
        await using var c = new EDBConnection(ConnectionString);
    ...
    class TestDbInfo : EDBDatabaseInfo
    {
        ...
        public new static Version ParseServerVersion(string versionString)
            => EDBDatabaseInfo.ParseServerVersion(versionString);
    }
    ...
    {
        var conn = new EDBConnection(null);
        Assert.That(conn.ConnectionString, Is.SameAs(string.Empty));
        Assert.That(() => conn.Open(), Throws.Exception.TypeOf<InvalidOperationException>());
    }
    ...
    public async Task ChangeDatabase()
    {
        ...
        using var cmd = new EDBCommand("select current_database()", conn);
    ...
    {
        using var conn1 = new EDBConnection(ConnectionString);
    ...
    {
        using var conn = await OpenConnectionAsync(new EDBConnectionStringBuilder(ConnectionString) { SearchPath = "foo" });
    ...
    {
        using var _ = CreateTempPool(new EDBConnectionStringBuilder(ConnectionString)
        {
    ...
    {
        //[#1011001] Bug in EDBConnectionStringBuilder affects on cache and connection pool

        var csb1 = new EDBConnectionStringBuilder(@"Server=server;Port=5444;User Id=user;Password=passwor;Database=database;");
    ...
    {
        await using var conn = await OpenConnectionAsync(new EDBConnectionStringBuilder(ConnectionString)
        {
...
LANGUAGE 'plpgsql'");
    ...
    public async Task Concurrent_use_throws()
    {
        ...
        using (await cmd.ExecuteReaderAsync())
            Assert.That(async () => await conn.ExecuteScalarAsync("SELECT 2"),
                Throws.Exception.TypeOf<EDBOperationInProgressException>()
                    .With.Property(nameof(EDBOperationInProgressException.CommandInProgress)).SameAs(cmd));
    ...
    public void PersistSecurityInfo_is_true([Values(true, false)] bool pooling)
    {
        ...
        using var conn = new EDBConnection(connString);
        var passwd = new EDBConnectionStringBuilder(conn.ConnectionString).Password;
        Assert.That(passwd, Is.Not.Null);
        conn.Open();
        Assert.That(new EDBConnectionStringBuilder(conn.ConnectionString).Password, Is.EqualTo(passwd));
    }
    ...
    public void No_password_without_PersistSecurityInfo([Values(true, false)] bool pooling)
    {
        ...
        conn.Open();
        Assert.That(new EDBConnectionStringBuilder(conn.ConnectionString).Password, Is.Null);
    }
    ...
    public void Clone_with_PersistSecurityInfo()
    {
        ...
        using var connWithPersist = new EDBConnection(connStringWithPersist);
    ...
    public async Task Clone_with_data_source()
    {
        ...
        await using var clonedConnection = (EDBConnection)((ICloneable)connection).Clone();

        Assert.That(clonedConnection.EDBDataSource, Is.SameAs(SharedDataSource));
    ...
    public async Task Rollback_on_close()
    {
        ...
        var dummyConn = new EDBConnection(ConnectionString);
        EDBConnection.ClearPool(dummyConn);
    ...
    public async Task NoTypeLoading()
    {
        ...
        Assert.That(await conn.ExecuteScalarAsync("SELECT '[1,10)'::int4range"), Is.EqualTo(new EDBRange<int>(1, true, 10, false)));

        if (conn.PostgreSqlVersion >= new Version(14, 0))
        {
            var multirangeArray = (EDBRange<int>[])(await conn.ExecuteScalarAsync("SELECT '{[3,7), (8,]}'::int4multirange"))!;
            Assert.That(multirangeArray.Length, Is.EqualTo(2));
            Assert.That(multirangeArray[0], Is.EqualTo(new EDBRange<int>(3, true, false, 7, false, false)));
            Assert.That(multirangeArray[1], Is.EqualTo(new EDBRange<int>(9, true, false, 0, false, true)));
        }
        else
        {
            using var cmd = new EDBCommand("SELECT $1", conn)
            {
                Parameters = { new() { Value = DBNull.Value, EDBDbType = EDBDbType.IntegerMultirange } }
            };

            Assert.That(async () => await cmd.ExecuteScalarAsync(),
                Throws.Exception.TypeOf<ArgumentException>()
                    .With.Message.EqualTo(string.Format(EDBStrings.NoMultirangeTypeFound, "integer")));
        }
    }
...
CREATE TABLE record ()");
    ...
    public async Task Non_UTF8_Encoding()
    {
        ...
        try
        {
            ...
            var goodBuilder = new EDBConnectionStringBuilder(ConnectionString)
            {
                Database = "sqlascii",
            ...
            var badBuilder = new EDBConnectionStringBuilder(ConnectionString)
            {
                Database = "sqlascii",
            };
    ...
    public async Task Oversize_buffer()
    {
        ...
        using (var conn = await OpenConnectionAsync(connectionString))
        {
            ...
            using (var cmd = new EDBCommand($"SELECT '{bigString1}'", conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                reader.Read();
            ...
            using (var cmd = new EDBCommand($"SELECT '{bigString2}'", conn))
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                reader.Read();
    ...
    public async Task NoResetOnClose(bool noResetOnClose)
    {
        ...
        using var _ = CreateTempPool(builder, out var connectionString);
        var original = new EDBConnectionStringBuilder(connectionString).ApplicationName;
    ...
    public async Task PhysicalConnectionInitializer_sync_with_break()
    {
        ...
        Assert.That(() => dataSource.OpenConnection(), Throws.Exception.InstanceOf<EDBException>());
    ...
    public async Task PhysicalConnectionInitializer_async_with_break()
    {
        ...
        Assert.That(async () => await dataSource.OpenConnectionAsync(), Throws.Exception.InstanceOf<EDBException>());
        Assert.That(dataSource.Statistics, Is.EqualTo((0, 0, 0)));
    }
    ...
    {
        // With multiplexing a physical connection might open on EDBConnection.OpenAsync (if there was no completed bootstrap beforehand)
        // or on EDBCommand.ExecuteReaderAsync.
    ...
    {
        EDBConnection? initializerConnection = null;
    ...
    public async Task Breaking_connection_while_loading_database_info()
    {
        ...
        await using var firstConn = new EDBConnection(connString);
        EDBDatabaseInfo.RegisterFactory(new BreakingDatabaseInfoFactory());
        try
        {
    ...
    class BreakingDatabaseInfoFactory : IEDBDatabaseInfoFactory
    {
    ...
    public async Task Log_Open_Close_pooled()
    {
        ...
        var openingConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == EDBEventId.OpeningConnection);
        ...
        var openedConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == EDBEventId.OpenedConnection);
        ...
        var closingConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == EDBEventId.ClosingConnection);
        ...
        var closedConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == EDBEventId.ClosedConnection);
        ...
        var ids = new[]
        {
            EDBEventId.OpeningPhysicalConnection,
            EDBEventId.OpenedPhysicalConnection,
            EDBEventId.ClosingPhysicalConnection,
            EDBEventId.ClosedPhysicalConnection
        };
    ...
    public async Task Log_Open_Close_physical()
    {
        ...
        var openingConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == EDBEventId.OpeningPhysicalConnection);
        AssertLoggingConnectionString(conn, openingConnectionEvent.State);
        AssertLoggingStateContains(openingConnectionEvent, "Host", host);
        AssertLoggingStateContains(openingConnectionEvent, "Port", port);
        AssertLoggingStateContains(openingConnectionEvent, "Database", database);

        var openedConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == EDBEventId.OpenedPhysicalConnection);
        AssertLoggingConnectionString(conn, openedConnectionEvent.State);
        AssertLoggingStateContains(openedConnectionEvent, "ConnectorId", processId);
        AssertLoggingStateContains(openingConnectionEvent, "Host", host);
        AssertLoggingStateContains(openingConnectionEvent, "Port", port);
        ...
        var closingConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == EDBEventId.ClosingPhysicalConnection);
        AssertLoggingConnectionString(conn, closingConnectionEvent.State);
        AssertLoggingStateContains(closingConnectionEvent, "ConnectorId", processId);
        AssertLoggingStateContains(closingConnectionEvent, "Host", host);
        AssertLoggingStateContains(closingConnectionEvent, "Port", port);
        ...
        var closededConnectionEvent = listLoggerProvider.Log.Single(l => l.Id == EDBEventId.ClosedPhysicalConnection);
    ...
    void AssertLoggingConnectionString(EDBConnection connection, object? logState)
    {
