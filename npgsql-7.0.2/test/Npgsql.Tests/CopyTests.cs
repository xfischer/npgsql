...
INSERT INTO {table} (bits, bitarray) VALUES (B'101', ARRAY[B'101', B'111'])");
    ...
    public async Task Binary_copy_throws_for_nullable()
    {
        ...
        int? value = 1;
        await writer.StartRowAsync();
        Assert.ThrowsAsync<InvalidCastException>(async () => await writer.WriteAsync(value, EDBDbType.Integer));
    }
...
INSERT INTO {table} (field_text, field_int4) VALUES ('HELLO', 1)");
    ...
    public async Task RawBinaryCopy_write_nre([Values] bool async)
    {
        ...
        await using var dataSource = EDBDataSource.Create(postmasterMock.ConnectionString);
        await using var conn = await dataSource.OpenConnectionAsync();
