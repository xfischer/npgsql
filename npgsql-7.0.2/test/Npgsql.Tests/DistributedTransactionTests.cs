...
public class DistributedTransactionTests : TestBase
{
    ...
    public void Two_connections_with_failure()
    {
        ...
        using var dataSource = EDBDataSource.Create(ConnectionStringEnlistOn);
...
Exception {2}",
    ...
    void AssertNumberOfRows(EDBConnection connection, string table, int expected)
...
-------------"));
    ...
    public void OneTimeSetUp()
    {
        ...
        using (var cmd = new EDBCommand("SELECT gid FROM pg_prepared_xacts WHERE database=@database", connection))
        {
    ...
    internal static string CreateTempTable(EDBConnection conn, string columns)
    {
