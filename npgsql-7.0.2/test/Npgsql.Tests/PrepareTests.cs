...
public class PrepareTests: TestBase
{
    ...
    public void Positional_parameters()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            using var command = new EDBCommand("SELECT $1, $2", conn);
            command.Parameters.Add(new EDBParameter { DbType = DbType.Int32 });
            command.Parameters.Add(new EDBParameter { Value = 8 });
            command.Prepare();
            command.Parameters[0].Value = 3;
            command.Parameters[1].Value = 5;
    ...
    public void Reuse_prepared_statement()
    {
        ...
        using (var cmd1 = new EDBCommand("SELECT @p", conn1))
        {
        ...
        using (var cmd2 = new EDBCommand("SELECT @p", conn1))
        {
    ...
    {
        using var conn = OpenConnectionAndUnprepare();
        using (var batch = new EDBBatch(conn) { BatchCommands = { new("SELECT 1"), new("SELECT 2") } })
        {
            batch.Prepare();
    ...
    {
        using var conn = OpenConnectionAndUnprepare();
        using var cmd = new EDBCommand("SELECT 1; SELECT 1", conn);
        cmd.Prepare();
        AssertNumPreparedStatements(conn, 1);
        cmd.ExecuteNonQuery();
        cmd.Unprepare();
    ...
    public void One_command_same_sql_auto_prepare()
    {
        ...
        for (var i = 0; i < 2 + 1; i++)
            sql.Append("SELECT 1;");
        using (var cmd = new EDBCommand(sql.ToString(), conn))
            cmd.ExecuteNonQuery();
        AssertNumPreparedStatements(conn, 1);
    }
    ...
    {
        using var conn = OpenConnectionAndUnprepare();
        using var cmd1 = new EDBCommand("SELECT 1; SELECT 2", conn);
        using var cmd2 = new EDBCommand("SELECT 2; SELECT 3", conn);
        cmd1.Prepare();
        cmd2.Prepare();
    ...
    public void Overloaded_sql()
    {
        ...
        {
            cmd.Parameters.Add("p", EDBDbType.Integer);
            cmd.Prepare();
            Assert.That(cmd.IsPrepared, Is.True);
        }
        using (var cmd = new EDBCommand("SELECT @p", conn))
        {
            cmd.Parameters.AddWithValue("p", EDBDbType.Text, "foo");
            cmd.Prepare();
    ...
    {
        var connSettings = new EDBConnectionStringBuilder(ConnectionString)
        {
    ...
    public void Prepared_command_failure()
    {
        ...
        using (var command = new EDBCommand("INSERT INTO test_table (id) VALUES (1)", conn))
    ...
    public void Same_sql_different_params()
    {
        ...
        using (var cmd = new EDBCommand("SELECT @p", conn))
        {
            ...
            cmd.Parameters[0].EDBDbType = EDBDbType.Text;
            Assert.That(cmd.IsPrepared, Is.False);
            cmd.Prepare(true);
            using (var crapCmd = new EDBCommand("SELECT name,statement,parameter_types::TEXT[] FROM pg_prepared_statements", conn))
    ...
    */
    ...
    {
        var builder = new EDBConnectionStringBuilder(ConnectionString) { Multiplexing = true };
    ...
    public async Task Explicitly_prepared_statement_invalidation()
    {
        ...
        await using var command = new EDBCommand($"SELECT * FROM {table}", connection);
        ...
        // However, EDB should invalidate the prepared statement in this case, so the next execution should work
    ...
    EDBConnection OpenConnectionAndUnprepare(string? connectionString = null)
    {
    ...
    EDBConnection OpenConnectionAndUnprepare(EDBConnectionStringBuilder csb)
        => OpenConnectionAndUnprepare(csb.ToString());

    void AssertNumPreparedStatements(EDBConnection conn, int expected)
    ...
    void AssertNumPreparedStatements(EDBConnection conn, int expected, string message)
    ...
    List<string> GetPreparedStatements(EDBConnection conn)
    {
        var statements = new List<string>();
        using var cmd = new EDBCommand("SELECT name FROM pg_prepared_statements WHERE statement NOT LIKE '%FROM pg_prepared_statement%'", conn);
        using var reader = cmd.ExecuteReader();
        while (reader.Read())
