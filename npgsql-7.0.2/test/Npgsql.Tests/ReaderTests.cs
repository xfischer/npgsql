...
public class ReaderTests : MultiplexingTestBase
{
    ...
    public async Task Seek_columns()
    {
        ...
        using var cmd = new EDBCommand("SELECT 1,2,3", conn);
    ...
    public async Task No_resultset()
    {
        ...
        using (var cmd = new EDBCommand($"INSERT INTO {table} VALUES (8)", conn))
        using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
        ...
        using (var cmd = new EDBCommand($"SELECT 1; INSERT INTO {table} VALUES (8)", conn))
        using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand("SELECT 1 AS foo WHERE FALSE", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        Assert.That(reader.Read(), Is.False);
    ...
    public async Task RecordsAffected()
    {
        ...
        cmd = new EDBCommand($"SELECT * FROM {table}", conn);
        reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Close();
        Assert.That(reader.RecordsAffected, Is.EqualTo(-1));

        cmd = new EDBCommand($"UPDATE {table} SET int=int+1 WHERE int > 10", conn);
        reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Close();
        Assert.That(reader.RecordsAffected, Is.EqualTo(4));

        cmd = new EDBCommand($"UPDATE {table} SET int=8 WHERE int=666", conn);
        reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Close();
        Assert.That(reader.RecordsAffected, Is.EqualTo(0));

        cmd = new EDBCommand($"DELETE FROM {table} WHERE int > 10", conn);
        reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Close();
        Assert.That(reader.RecordsAffected, Is.EqualTo(4));
        ...
        {
            cmd = new EDBCommand($"MERGE INTO {table} S USING (SELECT 2 as int) T ON T.int = S.int WHEN MATCHED THEN UPDATE SET int = S.int", conn);
            reader = await cmd.ExecuteReaderAsync(Behavior);
            reader.Close();
            Assert.That(reader.RecordsAffected, Is.EqualTo(1));
        }
...
UPDATE {table} SET name='b' WHERE name='doesnt_exist';";
        ...
        using (var cmd = new EDBCommand($"SELECT name FROM {table}; DELETE FROM {table}", conn))
        {
            using var reader = await cmd.ExecuteReaderAsync(Behavior);
...
#pragma warning restore CS0618
    ...
    public async Task Get_string_with_parameter()
    {
        ...
        var command = new EDBCommand($"SELECT name FROM {table} WHERE name = :value;", conn);
...
INSERT INTO {table} (name) VALUES ('Text with '' single quote');");
    ...
    public async Task GetFieldType_SchemaOnly()
    {
        ...
        await using var cmd = new EDBCommand(@"SELECT 1::INT4 AS some_column", conn);
    ...
    public async Task GetPostgresType()
    {
        ...
        using (var cmd = new EDBCommand(@"SELECT 1::INTEGER AS some_column", conn))
        using (var reader = await cmd.ExecuteReaderAsync(Behavior))
        {
            reader.Read();
    ...
    public async Task GetDataTypeName(string typeName, string? normalizedName = null)
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand($"SELECT NULL::{typeName} AS some_column", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    public async Task GetDataTypeName_enum()
    {
        ...
        conn.ReloadTypes();
        await using var cmd = new EDBCommand($"SELECT 'one'::{typeName}", conn);
    ...
    public async Task ExecuteReader_getting_empty_resultset_with_output_parameter()
    {
        using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "name TEXT");
        var command = new EDBCommand($"SELECT * FROM {table} WHERE name = NULL;", conn);
        var param = new EDBParameter("some_param", EDBDbType.Varchar);
    ...
    public async Task Get_value_from_empty_resultset()
    {
        using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "name TEXT");
        using var command = new EDBCommand($"SELECT * FROM {table} WHERE name = :value;", conn);
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var command = new EDBCommand(@"SELECT 1; SELECT 2", conn);
...
LANGUAGE 'plpgsql';
        ...
        using var cmd = new EDBCommand($"SELECT {function}()", conn);
...
LANGUAGE 'plpgsql';
        ...
        using var cmd = new EDBCommand($"SELECT 1; SELECT {function}()", conn);
        if (prepare == PrepareOrNot.Prepared)
            cmd.Prepare();
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task EDBException_references_BatchCommand_with_single_command()
    {
        await using var conn = await OpenConnectionAsync();
        var function = await GetTempFunctionName(conn);
...
LANGUAGE 'plpgsql'");
        ...
        // We use EDBConnection.CreateCommand to test that the command isn't recycled when referenced in an exception
    ...
    [Test, IssueLink("https://github.com/npgsql/npgsql/issues/967")]
    public async Task EDBException_references_BatchCommand_with_multiple_commands()
    {
        await using var conn = await OpenConnectionAsync();
        var function = await GetTempFunctionName(conn);
...
LANGUAGE 'plpgsql'");
    ...
    public async Task SchemaOnly_next_result_beyond_end()
    {
        ...
        using var cmd = new EDBCommand($"SELECT * FROM {table}", conn);
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand(@"select aggfnoid from pg_aggregate", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var command = new EDBCommand("select 123 as FIELD1", conn);
        using var reader = await command.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var command = new EDBCommand("select 123 as ｦｧｨｩｪｫｬ", conn);
        using var reader = await command.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    public async Task Reader_is_still_open()
    {
        ...
        Assert.That(async () => await conn.ExecuteScalarAsync("SELECT 1"), Throws.Exception.TypeOf<EDBOperationInProgressException>());

        using var cmd2 = new EDBCommand("SELECT 2", conn);
        Assert.That(() => cmd2.ExecuteReader(Behavior), Throws.Exception.TypeOf<EDBOperationInProgressException>());
        if (!IsMultiplexing)
    ...
    public async Task HasRows([Values(PrepareOrNot.NotPrepared, PrepareOrNot.Prepared)] PrepareOrNot prepare)
    {
        ...
        var command = new EDBCommand($"SELECT 1; SELECT * FROM {table} WHERE name='does_not_exist'", conn);
        if (prepare == PrepareOrNot.Prepared)
            command.Prepare();
        using (var reader = await command.ExecuteReaderAsync(Behavior))
        {
    ...
    public async Task HasRows_without_resultset()
    {
        using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "name TEXT");
        using var command = new EDBCommand($"DELETE FROM {table} WHERE name = 'unknown'", conn);
        using var reader = await command.ExecuteReaderAsync(Behavior);
...
LANGUAGE plpgsql VOLATILE";
        ...
        using var cmd = new EDBCommand($"SELECT {function}(1)", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task Invalid_cast()
    {
        ...
        using (var cmd = new EDBCommand("SELECT 'foo'", conn))
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand($"SELECT generate_series(1, {conn.Settings.ReadBufferSize})", conn);
        using var reader = await cmd.ExecuteReaderAsync();
    ...
    public async Task Nullable_scalar()
    {
        ...
        var p1 = new EDBParameter { ParameterName = "p1", Value = DBNull.Value, EDBDbType = EDBDbType.Smallint };
        var p2 = new EDBParameter { ParameterName = "p2", Value = (short)8 };
        Assert.That(p2.EDBDbType, Is.EqualTo(EDBDbType.Smallint));
    ...
    public async Task Bug2913_reading_previous_query_messages()
    {
        ...
        var secondQuery = Task.Run(async () =>
        {
            ...
            using var secondCmd = new EDBCommand(@"SELECT 1; SELECT 2;", secondConn);
        ...
        {
            // Executing a query, which fails with EDBException on reader disposing, as NotExistingTable doesn't exist
            using var firstCmd = new EDBCommand(@"SELECT 1; SELECT * FROM NotExistingTable;", firstConn);
        ...
        using var thirdCmd = new EDBCommand(@"SELECT 1; SELECT 2;", thirdConn);
    ...
    public async Task Bug3772()
    {
        ...
        using var cmd = new EDBCommand("SELECT some_int, some_byte FROM some_table", conn);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task Dispose_does_not_swallow_exceptions([Values(true, false)] bool async)
    {
        ...
        if (async)
            Assert.Throws<EDBException>(() => reader.Dispose());
        else
    ...
    public async Task GetStream_second_time_throws([Values(true, false)] bool isAsync)
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand($"SELECT {EncodeByteaHex(expected)}::bytea", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task Open_stream_when_changing_columns([Values(true, false)] bool isAsync)
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand(@"SELECT @p, @p", conn);
        var data = new byte[] { 1, 2, 3 };
        cmd.Parameters.Add(new EDBParameter("p", data));
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    public async Task GetBytes_with_null([Values(true, false)] bool isAsync)
    {
        ...
        using var cmd = new EDBCommand($"SELECT bytes FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    static Func<EDBDataReader, int, Task<Stream>> BuildStreamGetter(bool isAsync)
        => isAsync
    ...
    public async Task GetStream_after_consuming_column_throws([Values] bool async)
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand(@"SELECT '\xDEADBEEF'::bytea", conn);
    ...
    {
        Func<EDBDataReader, int, Task<TextReader>> textReaderGetter;
        if (isAsync)
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand(@"SELECT 'some_text', 'some_text'", conn);
        using var reader = await cmd.ExecuteReaderAsync(Behavior);
        reader.Read();
    ...
    public async Task Reader_is_reused()
    {
        ...
        using (var cmd = new EDBCommand("SELECT 9", conn))
    ...
    public async Task GetTextReader_after_consuming_column_throws([Values] bool async)
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT 'foo'", conn);
        await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SequentialAccess);
        await reader.ReadAsync();
    ...
    public async Task SafeReadException()
    {
        ...
        await using var cmd = new EDBCommand(@"SELECT 1, 'hello'", connection);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
        await reader.ReadAsync();
    ...
    [Test, Description("Tests that when a type handler generates an exception that isn't a EDBSafeReadException, the connection is properly broken")]
    ...
    public async Task GetFieldValueAsync_sequential_cancel([Values(true, false)] bool passCancelledToken)
    {
        ...
        using var cmd = new EDBCommand("SELECT some_bytea FROM some_table", conn);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
    ...
    public async Task IsDBNullAsync_sequential_cancel([Values(true, false)] bool passCancelledToken)
    {
        ...
        using var cmd = new EDBCommand("SELECT some_bytea, some_int FROM some_table", conn);
        await using var reader = await cmd.ExecuteReaderAsync(Behavior);
...
class ExplodingTypeHandlerResolverFactory : TypeHandlerResolverFactory
{
    ...
    public override TypeHandlerResolver Create(EDBConnector connector) => new ExplodingTypeHandlerResolver(_safe);
    ...
    class ExplodingTypeHandlerResolver : TypeHandlerResolver
    {
        ...
        public override EDBTypeHandler? ResolveByClrType(Type type) => null;
