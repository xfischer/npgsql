...
using EnterpriseDB.EDBClient.Replication.TestDecoding;
using EDBTypes;
    ...
    where TConnection : ReplicationConnection, new()
{
    ...
    ");
                ...
                {
                    await using var insertConn = await OpenConnectionAsync(new EDBConnectionStringBuilder(ConnectionString)
                    {
                ...
                {
                    await using var insertConn = OpenConnection(new EDBConnectionStringBuilder(ConnectionString)
                    {
                ...
                static async IAsyncEnumerable<(EDBLogSequenceNumber Lsn, string? MessageData)> ParseMessages(
                ...
                async Task<EDBLogSequenceNumber> GetCommitLsn(string valueString)
                {
                    if (typeof(TConnection) == typeof(PhysicalReplicationConnection))
    ...
    async IAsyncEnumerable<ReplicationMessage> StartReplication(TConnection connection, string slotName,
        EDBLogSequenceNumber xLogPos, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        if (typeof(TConnection) == typeof(PhysicalReplicationConnection))
        {
