...
public class SecurityTests : TestBase
{
    ...
    public void Reject_self_signed_certificate([Values(SslMode.VerifyCA, SslMode.VerifyFull)] SslMode sslMode)
    {
        ...
        var ex = Assert.Throws<EDBException>(conn.Open)!;
    ...
    public void SslMode_Require_throws_without_TSC()
    {
        ...
        Assert.That(ex.Message, Is.EqualTo(EDBStrings.CannotUseSslModeRequireWithoutTrustServerCertificate));
    }
    ...
    public async Task Bug4305_Secure([Values] bool async)
    {
        ...
        var ex = async
            ? Assert.ThrowsAsync<EDBException>(() => cmd.ExecuteNonQueryAsync())!
            : Assert.Throws<EDBException>(() => cmd.ExecuteNonQuery())!;
