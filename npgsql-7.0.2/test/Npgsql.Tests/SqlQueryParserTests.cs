...
class SqlQueryParserTests
{
    ...
    {
        var parameters = new EDBParameter[] { new(":p1", "foo"), new(":p2", "foo") };
    ...
    {
        var p = new EDBParameter(":a.parameter", "foo");
    ...
    {
        var results = ParseCommand(sql, new EDBParameter(":param", "foo"));
...
2", TestName = "LineComment")]
    ...
    {
        var parameters = new EDBParameter[]
        {
    ...
    List<EDBBatchCommand> ParseCommand(string sql, params EDBParameter[] parameters)
        => ParseCommand(sql, parameters, standardConformingStrings: true);

    List<EDBBatchCommand> ParseCommand(string sql, EDBParameter[] parameters, bool standardConformingStrings)
    {
