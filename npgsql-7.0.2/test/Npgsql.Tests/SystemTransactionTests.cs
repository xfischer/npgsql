...
public class SystemTransactionTests : TestBase
{
    ...
    {
        using var conn = new EDBConnection(ConnectionStringEnlistOff);
        conn.Open();
    ...
    {
        var conn = new EDBConnection(ConnectionStringEnlistOn);
        using (var scope = new TransactionScope())
        {
            conn.Open();
            Assert.That(conn.ExecuteNonQuery(@"INSERT INTO data (name) VALUES ('test')"), Is.EqualTo(1), "Unexpected insert rowcount");
    ...
    {
        var connString = new EDBConnectionStringBuilder(ConnectionStringEnlistOn)
        {
    ...
    {
        using (var scope = new TransactionScope())
        using (var conn = new EDBConnection(ConnectionStringEnlistOn))
        {
            conn.Open();
    ...
    public void Single_unpooled_closed_connection()
    {
        ...
        using var dataSource = EDBDataSource.Create(csb);
    ...
    {
        var csb = new EDBConnectionStringBuilder(ConnectionStringEnlistOn)
        {
            Pooling = pooling,
        };
    ...
    {
        ConnectionStringEnlistOn = new EDBConnectionStringBuilder(ConnectionString) { Enlist = true }.ToString();
        ConnectionStringEnlistOff = new EDBConnectionStringBuilder(ConnectionString) { Enlist = false }.ToString();
    }

    EDBConnection _controlConn = default!;
