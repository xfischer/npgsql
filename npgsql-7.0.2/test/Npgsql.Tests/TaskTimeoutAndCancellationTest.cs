...
public class TaskTimeoutAndCancellationTest : TestBase
{
    ...
    public async Task SuccessfulResultTaskAsync() =>
        Assert.AreEqual(TestResultValue, await TaskTimeoutAndCancellation.ExecuteAsync(ct => GetResultTaskAsync(10, ct), EDBTimeout.Infinite, CancellationToken.None));

    [Test]
    public async Task SuccessfulVoidTaskAsync() =>
        await TaskTimeoutAndCancellation.ExecuteAsync(ct => GetVoidTaskAsync(10, ct), EDBTimeout.Infinite, CancellationToken.None);

    [Test]
    public void InfinitelyLongTaskTimeout() =>
        Assert.ThrowsAsync<TimeoutException>(async () =>
            await TaskTimeoutAndCancellation.ExecuteAsync(ct => GetVoidTaskAsync(Timeout.Infinite, ct), new EDBTimeout(TimeSpan.FromMilliseconds(10)), CancellationToken.None));
    ...
    public void InfinitelyLongTaskCancellation()
    {
        ...
        Assert.ThrowsAsync<TaskCanceledException>(async () =>
            await TaskTimeoutAndCancellation.ExecuteAsync(ct => GetVoidTaskAsync(Timeout.Infinite, ct), EDBTimeout.Infinite, cts.Token));
    }
    ...
    static async Task CreateTaskAndPreemptWithCancellationAsync(int delayMs, bool cancel, bool timeout, SemaphoreSlim notifyDelayCompleted)
    {
        ...
        try
        {
            await TaskTimeoutAndCancellation.ExecuteAsync(
                ...
                timeout ? new EDBTimeout(TimeSpan.FromMilliseconds(timeoutMs)) : EDBTimeout.Infinite,
