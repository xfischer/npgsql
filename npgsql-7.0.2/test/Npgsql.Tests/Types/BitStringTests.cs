...
public class BitStringTests : MultiplexingTestBase
{
    ...
    public async Task BitArray(string sqlLiteral)
    {
        ...
        await AssertType(bitArray, sqlLiteral, "bit varying", EDBDbType.Varbit);

        if (len > 0)
            await AssertType(bitArray, sqlLiteral, $"bit({len})", EDBDbType.Bit, isDefaultForWriting: false);
    }
    ...
    public Task BitVector32()
        => AssertType(
            new BitVector32(4), "00000000000000000000000000000100", "bit varying", EDBDbType.Varbit, isDefaultForReading: false);
    ...
    [Test]
    public Task Bool()
        => AssertType(true, "1", "bit(1)", EDBDbType.Bit, isDefault: false);
    ...
    public Task Write_as_string()
        => AssertTypeWrite("010101", "010101", "bit varying", EDBDbType.Varbit, isDefault: false);
