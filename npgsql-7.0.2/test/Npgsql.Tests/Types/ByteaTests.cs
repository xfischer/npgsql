...
public class ByteaTests : MultiplexingTestBase
{
    ...
    public Task Bytea(byte[] byteArray, string sqlLiteral)
        => AssertType(byteArray, sqlLiteral, "bytea", EDBDbType.Bytea, DbType.Binary);
    ...
    public Task Write_as_MemoryStream_truncated()
    {
        ...
        return AssertTypeWrite(
            msFactory, "\\x020304", "bytea", EDBDbType.Bytea, DbType.Binary, isDefault: false);
    }
    ...
    public async Task Write_as_MemoryStream_long()
    {
        ...
        await AssertTypeWrite(
            () => new MemoryStream(bytes), expectedSql, "bytea", EDBDbType.Bytea, DbType.Binary, isDefault: false);
    }
    ...
    public async Task Write_as_FileStream_long()
    {
        ...
        try
        {
            ...
            await AssertTypeWrite(
                () => FileStreamFactory(filePath, fsList), expectedSql, "bytea", EDBDbType.Bytea, DbType.Binary, isDefault: false);
        }
        finally
        {
            foreach (var fs in fsList)
    ...
    [Test, Description("Tests that bytea array values are truncated when the EDBParameter's Size is set")]
    ...
    [Test, Description("Tests that bytea stream values are truncated when the EDBParameter's Size is set")]
    ...
    public async Task Truncate_stream()
    {
        ...
        // EDBParameter.Size larger than the value size should throw
        p.Size = data2.Length + 10;
        p.Value = new MemoryStream(data2);
        var ex = Assert.ThrowsAsync<EDBException>(async () => await cmd.ExecuteScalarAsync())!;
    ...
    public async Task Write_as_NonSeekable_stream()
    {
        ...
        p.Size = 0;
        Assert.ThrowsAsync<EDBException>(async () => await cmd.ExecuteScalarAsync());
