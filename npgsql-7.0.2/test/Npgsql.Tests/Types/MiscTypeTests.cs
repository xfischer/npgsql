...
class MiscTypeTests : MultiplexingTestBase
{
    ...
    {
        await AssertType(true, "true", "boolean", EDBDbType.Boolean, DbType.Boolean);
        await AssertType(false, "false", "boolean", EDBDbType.Boolean, DbType.Boolean);
    }
    ...
    public async Task Read_Record_as_object_array()
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand($"SELECT {recordLiteral}, ARRAY[{recordLiteral}, {recordLiteral}]", conn);
        await using var reader = await cmd.ExecuteReaderAsync();
        reader.Read();
