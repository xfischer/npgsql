...
class NetworkTypeTests : MultiplexingTestBase
{
    ...
    public Task Inet_v4_as_IPAddress()
        => AssertType(IPAddress.Parse("192.168.1.1"), "192.168.1.1/32", "inet", EDBDbType.Inet);
    ...
    public Task Inet_v4_as_tuple()
        => AssertType((IPAddress.Parse("192.168.1.1"), 24), "192.168.1.1/24", "inet", EDBDbType.Inet, isDefaultForReading: false);
    ...
    public Task IPAddress_Any()
        => AssertTypeWrite(IPAddress.Any, "0.0.0.0/32", "inet", EDBDbType.Inet);
...
#pragma warning disable 618  // For EDBInet
    ...
    [Test]
    public Task Inet_v6_as_EDBInet()
        => AssertType(
            new EDBInet(IPAddress.Parse("2001:1db8:85a3:1142:1000:8a2e:1370:7334"), 24),
            "2001:1db8:85a3:1142:1000:8a2e:1370:7334/24",
            "inet",
            EDBDbType.Inet,
            isDefaultForReading: false);
#pragma warning restore 618  // For EDBInet

    [Test]
    public Task Macaddr()
        => AssertType(PhysicalAddress.Parse("08-00-2B-01-02-03"), "08:00:2b:01:02:03", "macaddr", EDBDbType.MacAddr);
    ...
    public async Task Macaddr8()
    {
        ...
        await AssertType(PhysicalAddress.Parse("08-00-2B-01-02-03-04-05"), "08:00:2b:01:02:03:04:05", "macaddr8", EDBDbType.MacAddr8,
            isDefaultForWriting: false);
    }
    ...
    public async Task Macaddr8_write_with_6_bytes()
    {
        ...
        await AssertTypeWrite(PhysicalAddress.Parse("08-00-2B-01-02-03"), "08:00:2b:ff:fe:01:02:03", "macaddr8", EDBDbType.MacAddr8,
            isDefault: false);
    }
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT unnest(ARRAY['08-00-2B-01-02-03'::MACADDR, '08-00-2B-01-02-04'::MACADDR])", conn);
