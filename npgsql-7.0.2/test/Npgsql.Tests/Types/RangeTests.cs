...
class RangeTests : MultiplexingTestBase
{
    ...
    public async Task Range_resolution()
    {
        ...
        using (var cmd = new EDBCommand("SELECT @p", conn))
        {
            cmd.Parameters.Add(new EDBParameter { ParameterName="p", DataTypeName = "int4range", Value = DBNull.Value });
            using (var reader = await cmd.ExecuteReaderAsync())
            {
                reader.Read();
                Assert.That(reader.GetDataTypeName(0), Is.EqualTo("int4range"));
        ...
        using (var reader = await cmd.ExecuteReaderAsync())
        {
            reader.Read();
            Assert.That(reader.GetDataTypeName(0), Is.EqualTo("int4range"));
            Assert.That(reader.GetFieldValue<EDBRange<int>>(0), Is.EqualTo(new EDBRange<int>(3, true, 5, false)));
        }
    }
