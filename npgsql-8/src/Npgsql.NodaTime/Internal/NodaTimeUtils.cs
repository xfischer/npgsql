...
static class NodaTimeUtils
{
    ...
    static NodaTimeUtils() => LegacyTimestampBehavior = AppContext.TryGetSwitch("EnterpriseDB.EDBClient.EnableLegacyTimestampBehavior", out var enabled) && enabled;
    ...
    internal static Instant DecodeInstant(long value, bool dateTimeInfinityConversions)
        => value switch
        {
            long.MaxValue => dateTimeInfinityConversions
                ? Instant.MaxValue
                : throw new InvalidCastException(EDBNodaTimeStrings.CannotReadInfinityValue),
