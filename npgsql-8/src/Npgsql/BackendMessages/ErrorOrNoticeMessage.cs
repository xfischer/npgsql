...
sealed class ErrorOrNoticeMessage
{
    ...
    internal static ErrorOrNoticeMessage Load(EDBReadBuffer buf, bool includeDetail, ILogger exceptionLogger)
    {
        ...
        while (true)
        {
            ...
            case ErrorFieldTypeCode.Detail:
                ...
                if (!includeDetail && !string.IsNullOrEmpty(detail))
                    detail = $"Detail redacted as it may contain sensitive data. Specify '{EDBConnectionStringBuilder.IncludeExceptionDetailDisplayName}' in the connection string to include this information.";
                break;
