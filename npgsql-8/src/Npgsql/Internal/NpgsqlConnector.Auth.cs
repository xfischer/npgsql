...
partial class EDBConnector
{
    ...
    async Task AuthenticateSASL(List<string> mechanisms, string username, bool async, CancellationToken cancellationToken)
    {
        ...
        if (!clientSupportsSha256 && !clientSupportsSha256Plus)
        {
            ...
            if (serverSupportsSha256Plus && Settings.ChannelBinding == ChannelBinding.Disable)
                throw new EDBException($"Couldn't connect because {nameof(ChannelBinding)} is set to {nameof(ChannelBinding.Disable)} " +
        ...
        await WriteSASLInitialResponse(mechanism, EDBWriteBuffer.UTF8Encoding.GetBytes($"{cbindFlag},,n=*,r={clientNonce}"), async, cancellationToken).ConfigureAwait(false);
        await Flush(async, cancellationToken).ConfigureAwait(false);
    ...
    async Task AuthenticateMD5(string username, byte[] salt, bool async, CancellationToken cancellationToken = default)
    {
        ...
        {
            // First phase
            var passwordBytes = EDBWriteBuffer.UTF8Encoding.GetBytes(passwd);
            var usernameBytes = EDBWriteBuffer.UTF8Encoding.GetBytes(username);
            ...
            var prehashbytes = EDBWriteBuffer.UTF8Encoding.GetBytes(prehash);
