...
partial class EDBConnector
{
    ...
    {
        EDBWriteBuffer.AssertASCIIOnly(asciiName);
        LogMessages.TryEDBTrace(ConnectionLogger, "FE=> Describe (portal)");
        ...
        static void Write(EDBWriteBuffer writeBuffer, int len, StatementOrPortal statementOrPortal, byte[] name)
        {
    ...
    // EnterpriseDB Team
    // EDBMERGE: cancellationtoken added, update caller sites
    internal Task WriteDescribeOut(StatementOrPortal statementOrPortal, byte[] asciiName, bool async, CancellationToken cancellationToken = default)
    {

        EDBWriteBuffer.AssertASCIIOnly(asciiName);
        LogMessages.TryEDBTrace(ConnectionLogger, $"FE=> DescribeOut (portal)");
    ...
    internal Task WriteExecuteOut(int maxRows, bool async)
    {
        ...
        {
            await Flush(async).ConfigureAwait(false);
    ...
    */
    ...
    internal async Task WriteParseOut(string sql, byte[] asciiName, EDBParameterCollection parameters, bool async, CancellationToken cancellationToken = default)
    {
        ...
        EDBWriteBuffer.AssertASCIIOnly(asciiName);
        // EDBMERGE: TODO XFI dump param names
        //LogMessages.TryEDBTrace(ConnectionLogger, $"FE=> ParseOut (stmt=, query=\"{sql}\", parameters=({string.Join(",", _parameters.Select(p => $"{p.ParameterName}={p.DataTypeName},<{p.Value}>"))})");
        LogMessages.TryEDBTrace(ConnectionLogger, $"FE=> ParseOut (stmt=, query=\"{sql}\"");
        ...
        var messageLength =
            ...
            localParameters.Count * sizeof(int) +   // Parameter OIDs
        ...
        WriteBuffer.WriteInt16((short)localParameters.Count);
        ...
        for (int i = 0; i < localParameters.Count; i++)
        {
            ...
            {
                oid = databaseInfo.GetOid(localParameters[i].PgTypeId);
            }
            if (oid != 0)
            {
                WriteBuffer.WriteUInt32(oid.Value);
            }
            else
            {
                WriteBuffer.WriteInt32((int)EDBParameter.ParamToOid((string)localParameters[i].EDBDbType.ToString()));
            }
    ...
    internal async Task WriteBindOut(
       List<EDBParameter> positionalParameters,
    ...
    {
        EDBWriteBuffer.AssertASCIIOnly(asciiName);
        EDBWriteBuffer.AssertASCIIOnly(portal);
        LogMessages.TryEDBTrace(ConnectionLogger, $"FE=> BindOut(stmt=,portal={portal})");
        ...
        for (var paramIndex = 0; paramIndex < parameters.Count; paramIndex++)
        {
            ...
            // Need to null value for out params, to avoid type resolution based on value
        ...
        if (parameters.Count > 0)
        {
            ...
            try
            {
                for (var paramIndex = 0; paramIndex < parameters.Count; paramIndex++)
                {
                    ...
                    //writeBuffer.WriteInt32(param.WriteSize?.GetValueOrDefault() ?? 0);
                    await param.Write(async, writer, cancellationToken).ConfigureAwait(false);
                }
            }
            catch (Exception ex)
            {
                LogMessages.TryEDBTrace(ConnectionLogger, $"WriteBindOut error : {ex.Message}");
                Break(ex);
                throw;
            }
        }
    ...
    internal Task WriteSync(bool async, CancellationToken cancellationToken = default)
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Write(EDBWriteBuffer writeBuffer)
        {
    ...
    internal Task WriteExecute(int maxRows, bool async, CancellationToken cancellationToken = default)
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Write(EDBWriteBuffer writeBuffer, int maxRows)
        {
    ...
    internal async Task WriteParse(string sql, byte[] asciiName, List<EDBParameter> inputParameters, bool async, CancellationToken cancellationToken = default)
    {
        EDBWriteBuffer.AssertASCIIOnly(asciiName);
        LogMessages.TryEDBTrace(ConnectionLogger, $"FE=> Parse(stmt, query=\"{sql}\")");
        ...
        CancellationToken cancellationToken = default)
    {
        EDBWriteBuffer.AssertASCIIOnly(asciiName);
        EDBWriteBuffer.AssertASCIIOnly(portal);
        LogMessages.TryEDBTrace(ConnectionLogger, $"FE=> Bind(stmt, portal={portal})");
        ...
        if (parameters.Count > 0)
        {
            ...
            {
                LogMessages.TryEDBTrace(ConnectionLogger, $"WriteBind error : {ex.Message}");
                Break(ex);
                throw;
            }
        }
    ...
    internal Task WriteClose(StatementOrPortal type, byte[] asciiName, bool async, CancellationToken cancellationToken = default)
    {
        ...
        LogMessages.TryEDBTrace(ConnectionLogger, "FE=> Close");
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void Write(EDBWriteBuffer writeBuffer, int len, StatementOrPortal type, byte[] name)
        {
    ...
    internal void WriteStartup(Dictionary<string, string> parameters)
    {
        ...
        foreach (var kvp in parameters)
            len += EDBWriteBuffer.UTF8Encoding.GetByteCount(kvp.Key) + 1 +
    ...
    internal async Task WriteSASLInitialResponse(string mechanism, byte[] initialResponse, bool async, CancellationToken cancellationToken = default)
    {
        var len = sizeof(byte) +  // Message code
                  sizeof(int) +  // Length
                  EDBWriteBuffer.UTF8Encoding.GetByteCount(mechanism) + sizeof(byte) +  // Mechanism plus null terminator
