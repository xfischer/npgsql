...
/// </summary>
public sealed partial class EDBConnector
{
...
#pragma warning restore CS0618
    ...
    /// </summary>
    internal EDBReadBuffer ReadBuffer { get; private set; } = default!;
    ...
    /// </summary>
    internal EDBWriteBuffer WriteBuffer { get; private set; } = default!;
    ...
    internal EDBDataReader? CurrentReader;
    ...
    /// <see cref="EDBMultiHostDataSource"/>.
    /// </summary>
    internal EDBDataSource DataSource { get; }
    ...
    /// <see cref="EDBConnectionStringBuilder.ConnectionLifetime"/>.
    /// </summary>
    ...
    internal EDBDataReader DataReader { get; set; }

    internal EDBDataReader? UnboundDataReader { get; set; }
...
#pragma warning restore CS0618
    ...
    EDBConnector(EDBDataSource dataSource)
    {
        ...
        if (Settings.Multiplexing)
        {
            ...
            var commandsInFlightChannel = Channel.CreateUnbounded<EDBCommand>(
    ...
    internal async Task Open(EDBTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (DataSource.ConnectionInitializerAsync is not null)
            {
                ...
                var tempConnection = new EDBConnection(DataSource, this);
                ...
                {
                    // Note that we can't just close/dispose the EDBConnection, since that puts the connector back in the pool.
                    // But we transition it to disposed immediately, in case the user decides to capture the EDBConnection and use it
                    // later.
            ...
            bool isFirstAttempt = true)
        {
            ...
            if (msg.Code != BackendMessageCode.ReadyForQuery)
                throw new EDBException($"Received backend message {msg.Code} while expecting ReadyForQuery. Please file a bug.");
    ...
    ValueTask<string> GetUsernameAsync(bool async, CancellationToken cancellationToken)
    {
        ...
        async ValueTask<string> GetUsernameAsyncInternal()
        {
            ...
            throw new EDBException("No username could be found, please specify one explicitly");
        }
    }
    ...
    async Task RawOpen(SslMode sslMode, EDBTimeout timeout, bool async, CancellationToken cancellationToken, bool isFirstAttempt = true)
    {
        try
        {
            ...
            {
                TextEncoding = EDBWriteBuffer.UTF8Encoding;
                RelaxedTextEncoding = EDBWriteBuffer.RelaxedUTF8Encoding;
            }
            else
            {
            ...
            ReadBuffer = new EDBReadBuffer(this, _stream, _socket, Settings.ReadBufferSize, TextEncoding, RelaxedTextEncoding);
            WriteBuffer = new EDBWriteBuffer(this, _stream, _socket, Settings.WriteBufferSize, TextEncoding);
    ...
    internal async Task NegotiateEncryption(SslMode sslMode, EDBTimeout timeout, bool async, bool isFirstAttempt)
    {
    ...
    void Connect(EDBTimeout timeout)
    {
        ...
        for (var i = 0; i < endpoints.Length; i++)
        {
            ...
            try
            {
                ...
                {
                    LogMessages.TryEDBTrace(ConnectionLogger, $"SocketErrorCode: {e.SocketErrorCode}, Error:{e}");
    ...
    readonly ChannelReader<EDBCommand>? CommandsInFlightReader;
    internal readonly ChannelWriter<EDBCommand>? CommandsInFlightWriter;
        ...
        try
        {
            while (await CommandsInFlightReader.WaitToReadAsync().ConfigureAwait(false))
            {
                ...
                while (CommandsInFlightReader.TryRead(out command))
                {
                    ...
                    // true, so that the user code calling EDBDataReader.Dispose will not continue executing
        ...
        catch (Exception e)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    pendingCommand.ExecutionCompletion.SetException(new EDBException("A previous command on this connection caused an error requiring all pending commands on this connection to be aborted", _breakReason!));
                }
            }
        ...
        bool checkDataAvailable = false) // EnterpriseDB Team
    {
        ...
        {
            return ReadMessageLong(async, dataRowLoadingMode, readingNotifications: false, checkDataAvailable)!;
        }

        var messageCode = (BackendMessageCode)ReadBuffer.ReadByte();

#if NETFRAMEWORK
        LogMessages.TryEDBTrace(ConnectionLogger, $"<=BE {messageCode} readingNotifications:{readingNotifications}, checkDataAvailable:{checkDataAvailable}");
#else
        LogMessages.TryEDBTrace(ConnectionLogger, $"<=BE {messageCode}");
#endif
        switch (messageCode)
        {
        ...
        bool isReadingPrependedMessage = false)
    {
        ...
        try
        {
            while (true)
            {
                LogMessages.TryEDBTrace(ConnectionLogger, $"--> ReadMessage");

                await ReadBuffer.Ensure(5, async, readingNotifications, checkDataAvailable).ConfigureAwait(false);
                var messageCode = (BackendMessageCode)ReadBuffer.ReadByte();

                LogMessages.TryEDBTrace(ConnectionLogger, $"--> ValidateBackendMessageCode {messageCode}");
                ValidateBackendMessageCode(messageCode);
                ...
                LogMessages.TryEDBTrace(ConnectionLogger, $"ParseServerMessage {messageCode}");
                ...
                case BackendMessageCode.ReadyForQuery:
                    if (error != null)
                    {
                        EDBEventSource.Log.CommandFailed();
        ...
        catch (PostgresException e)
        {
            if (e.SqlState == PostgresErrorCodes.QueryCanceled && PostgresCancellationPerformed)
            {
                ...
                throw UserCancellationRequested
                    ...
                    : new EDBException("Exception while reading from stream",
        ...
        {
            LogMessages.TryEDBTrace(ConnectionLogger, $"EDBException: {e}");
    ...
    internal IBackendMessage? ParseResultSetMessage(EDBReadBuffer buf, BackendMessageCode code, int len, bool handleCallbacks = false)
        => code switch
        {
    ...
    internal IBackendMessage? ParseServerMessage(EDBReadBuffer buf, BackendMessageCode code, int len, bool isPrependedMessage, bool handleCallbacks = true)
    {
        switch (code)
        {
        /* EnterpriseDB Team (diagnostics) */
        case BackendMessageCode.RowDescription:
            var rowDescription = _rowDescriptionMessage.Load(buf, SerializerOptions, isOutDescription: false);
            try
            {
                // Some tests are purposely using exception raising type handlers, make sure to swallow this exception
                LogMessages.TryEDBTrace(ConnectionLogger, $"ParseServerMessage/RowDescription : {string.Join(",", rowDescription)}");
            }
        ...
        case BackendMessageCode.OutDescription:
            var rowOutDescriptionMessage = new RowDescriptionMessage(connectorOwned: false); // EDBMERGE: ctor changed, check
            rowOutDescriptionMessage.Load(buf, SerializerOptions, isOutDescription: true);
            LogMessages.TryEDBTrace(ConnectionLogger, $"ParseServerMessage/OutDescription : {string.Join(",", rowOutDescriptionMessage)}");
        ...
        case BackendMessageCode.NotificationResponse:
            if (handleCallbacks)
            {
                Connection?.OnNotification(new EDBNotificationEventArgs(buf));
            }
            return null;
        ...
        default:
            ThrowHelper.ThrowInvalidOperationException($"Internal EDB bug: unexpected value {code} of enum {nameof(BackendMessageCode)}. Please file a bug.");
            return null;
        }
    }
    ...
    internal async ValueTask<IBackendMessage> SkipUntil(BackendMessageCode stopAt1, BackendMessageCode stopAt2, bool async)
    {
        ...
        while (true)
        {
            var msg = await ReadMessage(async, DataRowLoadingMode.Skip).ConfigureAwait(false);
    ...
    internal bool InTransaction
    {
        get
        {
            ...
            case TransactionStatus.InFailedTransactionBlock:
                return true;
            default:
                ThrowHelper.ThrowInvalidOperationException($"Internal EDB bug: unexpected value {{0}} of enum {nameof(TransactionStatus)}. Please file a bug.", TransactionStatus);
                return false;
            }
        }
    }
    ...
    void ProcessNewTransactionStatus(TransactionStatus newStatus)
    {
        ...
        case TransactionStatus.InFailedTransactionBlock:
            ...
            return;
        case TransactionStatus.Pending:
            ThrowHelper.ThrowInvalidOperationException($"Internal EDB bug: invalid TransactionStatus {nameof(TransactionStatus.Pending)} received, should be frontend-only");
            return;
        default:
            ThrowHelper.ThrowInvalidOperationException($"Internal EDB bug: unexpected value {{0}} of enum {nameof(TransactionStatus)}. Please file a bug.", newStatus);
            return;
        }
    }
    ...
    internal bool PerformPostgresCancellation()
    {
        ...
        lock (CancelLock)
        {
            ...
            try
            {
                var cancelConnector = new EDBConnector(this);
                cancelConnector.DoCancelRequest(BackendProcessId, _backendSecretKey);
            }
            catch (Exception e)
            {
                LogMessages.TryEDBTrace(ConnectionLogger, $"Exception: {e}");
    ...
    /// <see cref="EDBConnectionStringBuilder.CancellationTimeout"/> has elapsed. If <see langword="false" />,
    ...
    internal readonly struct NestedCancellableScope : IDisposable
    {
        ...
        public NestedCancellableScope(EDBConnector connector, CancellationTokenRegistration registration, CancellationToken previousCancellationToken, bool previousAttemptPostgresCancellation)
        {
    ...
    internal async Task CloseOngoingOperations(bool async)
    {
        ...
        else if (copyOperation != null)
        {
            ...
            if (copyOperation is EDBBinaryImporter ||
                copyOperation is EDBCopyTextWriter ||
                copyOperation is EDBRawCopyStream rawCopyStream && rawCopyStream.CanWrite)
            {
                try
                {
                    if (async)
    ...
    internal Exception Break(Exception reason)
    {
        ...
        try
        {
            ...
            lock (CancelLock)
            {
                ...
                if (reason is EDBException { IsTransient: true } ne &&
    ...
    void Cleanup()
    {
        ...
        if (CurrentReader != null)
        {
            ...
            {
                LogMessages.TryEDBTrace(ConnectionLogger, "Reader close error");
                // ignored
            }
            CurrentReader = null;
        }
        ...
        if (CurrentCopyOperation != null)
        {
            ...
            {
                // ignored
                LogMessages.TryEDBTrace(ConnectionLogger, "CurrentCopyOperation Dispose error");
            }
    ...
    internal async Task Reset(bool async)
    {
        ...
        using (StartUserAction(attemptPgCancellation: false))
        {
            ...
            case TransactionStatus.InFailedTransactionBlock:
                ...
                break;
            default:
                ThrowHelper.ThrowInvalidOperationException($"Internal EDB bug: unexpected value {TransactionStatus} of enum {nameof(TransactionStatus)}. Please file a bug.");
                return;
            }
    ...
    /// <param name="command">
    /// The <see cref="EDBCommand" /> that is starting execution - if an <see cref="EDBOperationInProgressException" /> is
    ...
    /// occurs, followed by a client-side socket cancellation once <see cref="EDBConnectionStringBuilder.CancellationTimeout"/> has
        ...
        bool attemptPgCancellation = true)
    {
        ...
        UserAction DoStartUserAction(ConnectorState newState, EDBCommand? command,
            CancellationToken cancellationToken, bool attemptPgCancellation)
        {
            ...
            case ConnectorState.Copy:
                ...
                if (currentCommand is null)
                    ThrowHelper.ThrowEDBOperationInProgressException(State);
                else
                    ThrowHelper.ThrowEDBOperationInProgressException(currentCommand);
                break;
            default:
        ...
        UserAction DoStartUserActionWithKeepAlive(ConnectorState newState, EDBCommand? command,
            CancellationToken cancellationToken, bool attemptPgCancellation)
        {
            lock (SyncObj)
            {
                ...
                {
                    if (IsBroken)
                        ThrowHelper.ThrowEDBException("The connection was previously broken because of the following exception", _breakReason);
                    else
                        ThrowHelper.ThrowEDBException("The connection is closed");
                }
                ...
                catch (Exception ex) when (ex is not EDBOperationInProgressException)
                {
    ...
    internal void EndUserAction()
    {
        ...
        LogMessages.TryEDBTrace(ConnectionLogger, "EndUserAction");
    ...
    {
        readonly EDBConnector _connector;
        internal UserAction(EDBConnector connector) => _connector = connector;
...
#pragma warning restore CA1801 // Review unused parameters
    ...
    internal async Task<bool> Wait(bool async, int timeout, CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                {
                    throw Break(
                        new EDBException($"Received unexpected message of type {msg.Code} while waiting"));
                }
                return true;
            }
            catch (EDBException e) when (e.InnerException is TimeoutException)
            {
            ...
            while (true)
            {
                ...
                catch (Exception e) when (e is OperationCanceledException || e is EDBException npgEx && npgEx.InnerException is TimeoutException)
                {
    ...
    /// <summary>
    /// Creates and returns a <see cref="EDBCommand"/> object associated with the <see cref="EDBConnector"/>.
    /// </summary>
