...
/// </summary>
sealed class EDBWriteBuffer : IDisposable
{
    ...
    internal PgWriter GetWriter(EDBDatabaseInfo typeCatalog, FlushMode flushMode = FlushMode.None)
        ...
        Encoding textEncoding)
    {
        ...
        _pgWriter = new PgWriter(new EDBBufferWriter(this));
    }
    ...
    public Task WriteBytesRaw(ReadOnlyMemory<byte> bytes, bool async, CancellationToken cancellationToken = default)
    {
        ...
        static async Task WriteBytesLong(EDBWriteBuffer buffer, bool async, ReadOnlyMemory<byte> bytes, CancellationToken cancellationToken)
        {
