        ...
        _multiplexWriteLoop = Task.Run(MultiplexingWriteLoop, CancellationToken.None)
            .ContinueWith(t =>
            {
                if (t.IsFaulted)
                {
                    ...
                    _connectionLogger.LogError(t.Exception, "Exception in multiplexing write loop, this is an EDB .NET Connector bug, please file an issue.");
                }
            });
    }
    ...
    async Task MultiplexingWriteLoop()
    {
        ...
        bool WriteCommand(EDBConnector connector, EDBCommand command, ref MultiplexingStats stats)
        {
            ...
            default:
                ...
                ThrowHelper.ThrowEDBException("When writing command to connector, task is in invalid state " + task.Status);
                return false;
            }
        }
        ...
        void Flush(EDBConnector connector, ref MultiplexingStats stats)
        {
            ...
            default:
                ...
                ThrowHelper.ThrowEDBException("When flushing, task is in invalid state " + task.Status);
                return;
            }
        }
