...
public sealed class EDBBinaryExporter : ICancelable
{
    ...
    /// <returns>The value of the column</returns>
    public T Read<T>(EDBDbType type) => Read<T>(async: false, type, CancellationToken.None).GetAwaiter().GetResult();
    ...
    async ValueTask<T> Read<T>(bool async, EDBDbType? type, CancellationToken cancellationToken)
    {
        ThrowIfNotOnRow();
        ...
        PgConverterInfo GetInfo(EDBDbType? type, out bool asObject)
        {
