...
/// a PostgreSQL table. Initiated by <see cref="EDBConnection.BeginBinaryImport(string)"/>
/// </summary>
/// <remarks>
/// See https://www.postgresql.org/docs/current/static/sql-copy.html.
/// </remarks>
public sealed class EDBBinaryImporter : ICancelable
{
    ...
    internal EDBBinaryImporter(EDBConnector connector)
    {
    ...
    void WriteHeader()
    {
        _buf.WriteBytes(EDBRawCopyStream.BinarySignature, 0, EDBRawCopyStream.BinarySignature.Length);
    ...
    /// corruption will occur. If in doubt, use <see cref="Write{T}(T, EDBDbType)"/> to manually
    /// specify the type.
    /// </typeparam>
    ...
    Task Write<T>(bool async, T value, CancellationToken cancellationToken = default)
    {
        ...
        if (p is not EDBParameter<T> typedParam)
            typedParam = new EDBParameter<T>();
    ...
    public void Write<T>(T value, EDBDbType npgsqlDbType) =>
    ...
    /// </param>
    /// <typeparam name="T">The .NET type of the column to be written.</typeparam>
    public Task WriteAsync<T>(T value, EDBDbType npgsqlDbType, CancellationToken cancellationToken = default)
        => Write(async: true, value, npgsqlDbType, cancellationToken);

    Task Write<T>(bool async, T value, EDBDbType npgsqlDbType, CancellationToken cancellationToken = default)
    {
        CheckColumnIndex();
        if (cancellationToken.IsCancellationRequested)
            return Task.FromCanceled(cancellationToken);
        ...
        ref var p = ref _params[_column];
        if (p is not EDBParameter<T> typedParam)
            typedParam = new EDBParameter<T> { EDBDbType = npgsqlDbType };
    ...
    Task Write<T>(bool async, T value, string dataTypeName, CancellationToken cancellationToken = default)
    {
        ...
        ref var p = ref _params[_column];
        if (p is not EDBParameter<T> typedParam)
            typedParam = new EDBParameter<T> { DataTypeName = dataTypeName };
    ...
    async Task Write<T>(bool async, T value, EDBParameter<T> param, PgTypeInfo? previousTypeInfo, PgConverter? previousConverter, PgTypeId previousTypeId, CancellationToken cancellationToken = default)
    {
        CheckReady();
        if (_column == -1)
    ...
    async Task Cancel(bool async, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            throw _connector.Break(
                new EDBException("Expected ErrorResponse when cancelling COPY but got: " + msg.Code));
        }
