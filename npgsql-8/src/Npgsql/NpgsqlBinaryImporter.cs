...
public sealed class EDBBinaryImporter : ICancelable
{
    ...
    Task Write<T>(bool async, T value, EDBDbType? npgsqlDbType, string? dataTypeName, CancellationToken cancellationToken = default)
    {
        ...
        async Task Core(bool async, T value, EDBDbType? npgsqlDbType, string? dataTypeName, CancellationToken cancellationToken = default)
        {
            CheckReady();
            cancellationToken.ThrowIfCancellationRequested();
            CheckColumnIndex();
            ...
            var newParam = false;
            if (_params[_column] is not EDBParameter<T> param)
            {
                newParam = true;
                param = new EDBParameter<T>();
