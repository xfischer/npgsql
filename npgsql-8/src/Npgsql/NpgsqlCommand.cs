...
";
                ...
                foreach (var batchCommand in InternalBatchCommands)
                {
                    ...
                    if (batchCommand.PositionalParameters.Count != paramTypeOIDs.Count)
                    {
                        ...
                        throw new EDBException(
                            "There was a mismatch in the number of derived parameters between the EDB .NET Connector SQL parser and the PostgreSQL parser. Please report this as bug to the EDB .NET Connector developers (https://github.com/npgsql/npgsql/issues).");
                    }
                    ...
                    for (var i = 0; i < paramTypeOIDs.Count; i++)
                    {
                        try
                        {
                            ...
                            var npgsqlDbType = postgresType.GetRepresentationalType().DataTypeName.ToEDBDbType();
    ...
    internal void ProcessRawQuery(SqlQueryParser? parser, bool standardConformingStrings, EDBBatchCommand? batchCommand, bool redwoodDialect) // EnterpriseDB (additional param)
    {
        ...
        case CommandType.Text:
            ...
            case PlaceholderType.Named:
                if (!EnableSqlRewriting)
                    ThrowHelper.ThrowNotSupportedException($"Named parameters are not supported when EnterpriseDB.EDBClient.{nameof(EnableSqlRewriting)} is disabled");
        ...
        case CommandType.StoredProcedure:
            ...
            if (!redwoodDialect) // EnterpriseDB
            {
                ...
                var inputParameters = EDBBatchCommand.EmptyParameters;
                if (parameters is not null)
                {
                    inputParameters = new List<EDBParameter>(parameters.Count);
                    for (var i = 0; i < parameters.Count; i++)
                    {
                        ...
                        {
                            if (seenNamedParam)
                                ThrowHelper.ThrowArgumentException(EDBStrings.PositionalParameterAfterNamed);
                        }
                        else
                        {
                            seenNamedParam = true;
            ...
            {
                // transforms PROC to PROC(:name1, :name2, ..., :namen)
                // transforms to PROC($1, $2, ..., $n)
                // prefixes with CALL
                ...
                for (var i = 0; i < _parameters.Count; i++)
                {
                    ...
                    parseCommand = ReplaceParameterName(parseCommand, parameterName, "$" + (i + 1));
                }
        ...
        default:
            ThrowHelper.ThrowArgumentOutOfRangeException(nameof(CommandType), $"Internal EDB bug: unexpected value {{0}} of enum {nameof(CommandType)}. Please file a bug.", commandType);
            break;
        }
        ...
        static string ReplaceParameterName(string commandText, string parameterName, string newName)
        {
            ...
            do
            {
                pos = commandText.IndexOfAny(paramIdentifier, start);
                if (pos >= 0)
                {
                    var nextSeparatorIndex = commandText.IndexOfAny(paramTerminator, pos);
                    var paramToken = commandText.Substring(pos + 1, nextSeparatorIndex - pos - 1).Trim();
                    if (paramToken == parameterName)
                    {
                        found = true;
                        commandText = commandText.Substring(0, pos) + newName + commandText.Substring(nextSeparatorIndex);
                    }
                    start = pos + 1;
                }
            } while (!found && pos > -1);
    ...
    internal Task Write(EDBConnector connector, bool async, bool flush, CancellationToken cancellationToken = default)
    {
        ...
        async Task WriteExecute(EDBConnector connector, bool async, bool flush, CancellationToken cancellationToken)
        {
            ...
            for (var i = 0; i < InternalBatchCommands.Count; i++)
            {
                ...
                if (pStatement == null || batchCommand.IsPreparing)
                {
                    ...
                    if (CommandType == CommandType.StoredProcedure)
                    {
                        await connector.WriteBindOut(
                           batchCommand.CurrentParametersReadOnly, _parameters!, string.Empty, batchCommand.StatementName, AllResultTypesAreUnknown,
                ...
                {
                    await connector.WriteDescribe(StatementOrPortal.Portal, Array.Empty<byte>(), async, cancellationToken).ConfigureAwait(false);
                    await connector.WriteDescribeOut(StatementOrPortal.Portal, Array.Empty<byte>(), async, cancellationToken).ConfigureAwait(false);
                ...
                if (CommandType == CommandType.StoredProcedure)
                {
                    await connector.WriteExecuteOut(0, async).ConfigureAwait(false);
                }
    ...
    async Task SendPrepare(EDBConnector connector, bool async, CancellationToken cancellationToken = default)
    {
        ...
        for (var i = 0; i < InternalBatchCommands.Count; i++)
        {
            ...
            if (CommandType == CommandType.StoredProcedure) //EnterpriseDB Team
            {
                await connector.WriteParseOut(batchCommand.FinalCommandText!, pStatement.Name!, _parameters, async, cancellationToken).ConfigureAwait(false);
            }
            else
            {
    ...
    internal virtual async ValueTask<EDBDataReader> ExecuteReader(bool async, CommandBehavior behavior, CancellationToken cancellationToken)
    {
        var conn = CheckAndGetConnection();
        _behavior = behavior;
        ...
        if (_connector is not null)
        {
            ...
            if (behavior.HasFlag(CommandBehavior.CloseConnection))
                ThrowHelper.ThrowArgumentException($"{nameof(CommandBehavior.CloseConnection)} is not supported with {nameof(EDBConnector)}", nameof(behavior));
        ...
        try
        {
            ...
            else
            {
                ...
                connector = await new ValueTask<EDBConnector>(ExecutionCompletion, ExecutionCompletion.Version).ConfigureAwait(false);
