...
/// <summary>
/// An <see cref="EDBDataSource" /> which manages connections for multiple hosts, is aware of their states (primary, secondary,
...
/// </remarks>
public sealed class EDBMultiHostDataSource : EDBDataSource
{
    internal override bool OwnsConnectors => false;

    readonly EDBDataSource[] _pools;

    internal EDBDataSource[] Pools => _pools;
        ...
        : base(settings, dataSourceConfig)
    {
        ...
        for (var i = 0; i < hosts.Length; i++)
        {
            ...
            if (EDBConnectionStringBuilder.TrySplitHostPort(host, out var newHost, out var newPort))
            {
    ...
    public EDBConnection CreateConnection(TargetSessionAttributes targetSessionAttributes)
        => EDBConnection.FromDataSource(_wrappers[(int)targetSessionAttributes]);

    /// <summary>
    /// Returns a new, opened connection from this data source.
    /// </summary>
    /// <param name="targetSessionAttributes">Specifies the server type (e.g. primary, standby).</param>
    public EDBConnection OpenConnection(TargetSessionAttributes targetSessionAttributes)
    {
    ...
    /// </param>
    public async ValueTask<EDBConnection> OpenConnectionAsync(
    ...
    /// <summary>
    /// Returns an <see cref="EDBDataSource" /> that wraps this multi-host one with the given server type.
    /// </summary>
    /// <param name="targetSessionAttributes">Specifies the server type (e.g. primary, standby).</param>
    public EDBDataSource WithTargetSession(TargetSessionAttributes targetSessionAttributes)
    ...
    async ValueTask<EDBConnector?> TryGetIdleOrNew(
        EDBConnection conn,
        ...
        for (var i = 0; i < pools.Length; i++)
        {
            ...
            try
            {
                if (pool.TryGetIdleConnector(out connector))
                {
                    ...
                    {
                        databaseState = await connector.QueryDatabaseState(new EDBTimeout(timeoutPerHost), async, cancellationToken).ConfigureAwait(false);
                ...
                {
                    connector = await pool.OpenNewConnector(conn, new EDBTimeout(timeoutPerHost), async, cancellationToken).ConfigureAwait(false);
    ...
    async ValueTask<EDBConnector?> TryGet(
        EDBConnection conn,
        TimeSpan timeoutPerHost,
        bool async,
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < pools.Length; i++)
        {
            ...
            try
            {
                connector = await pool.Get(conn, new EDBTimeout(timeoutPerHost), async, cancellationToken).ConfigureAwait(false);
                if (databaseState == DatabaseState.Unknown)
                {
    ...
    static EDBException NoSuitableHostsException(IList<Exception> exceptions)
    {
        return exceptions.Count == 0
            ? new EDBException("No suitable host was found.")
            : exceptions[0] is PostgresException firstException && AllEqual(firstException, exceptions)
                ? firstException
                : new EDBException("Unable to connect to a suitable host. Check inner exception for more details.",
    ...
    internal override void Return(EDBConnector connector)
        => throw new EDBException("EDB bug: a connector was returned to " + nameof(EDBMultiHostDataSource));

    internal override bool TryGetIdleConnector([NotNullWhen(true)] out EDBConnector? connector)
        => throw new EDBException("EDB bug: trying to get an idle connector from " + nameof(EDBMultiHostDataSource));
    ...
    /// Can be useful to make EDB retry a PostgreSQL instance which was previously detected to be offline.
    /// </summary>
        ...
        [NotNullWhen(true)] out EDBConnector? connector)
    {
        ...
        bool TryGetValidConnector(List<EDBConnector> list, TargetSessionAttributes preferredType,
            Func<DatabaseState, TargetSessionAttributes, bool> validationFunc, [NotNullWhen(true)] out EDBConnector? connector)
        {
