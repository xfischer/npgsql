...
public sealed class EDBNestedDataReader : DbDataReader
{
    ...
    public new EDBNestedDataReader GetData(int ordinal)
    {
        ...
        {
            reader = new EDBNestedDataReader(_outermostReader, this, _depth + 1, compositeType);
        }
        if (isArray)
            reader.InitArray();
        else
    ...
    /// <inheritdoc />
    [UnconditionalSuppressMessage("ILLink", "IL2093", Justification = "No members are dynamically accessed by EnterpriseDB.EDBCLient via EDBNestedDataReader.GetFieldType.")]
