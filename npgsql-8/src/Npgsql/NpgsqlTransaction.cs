...
public sealed class EDBTransaction : DbTransaction
{
    ...
    void CheckReady()
    {
        CheckDisposed();
        if (IsCompleted)
            ThrowHelper.ThrowInvalidOperationException("This EDBTransaction has completed; it is no longer usable.");
    }

    void CheckDisposed()
    {
        if (IsDisposed)
            ThrowHelper.ThrowObjectDisposedException(nameof(EDBTransaction), _disposeReason);
    }
