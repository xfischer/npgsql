...
public abstract class TypeHandlerBenchmarks<T>
{
    ...
    protected TypeHandlerBenchmarks(PgConverter handler)
    {
        ...
        _readBuffer = new EDBReadBuffer(null, stream, null, EDBReadBuffer.MinimumSize, EDBWriteBuffer.UTF8Encoding, EDBWriteBuffer.RelaxedUTF8Encoding);
        _writeBuffer =  new EDBWriteBuffer(null, stream, null, EDBWriteBuffer.MinimumSize, EDBWriteBuffer.UTF8Encoding);
        _reader = new PgReader(_readBuffer);
        _writer = new PgWriter(new EDBBufferWriter(_writeBuffer));
