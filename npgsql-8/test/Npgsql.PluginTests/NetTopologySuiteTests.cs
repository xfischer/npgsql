...
public class NetTopologySuiteTests : TestBase
{
    ...
    public async Task Array()
    {
        ...
        await AssertType(
            ...
            '{' + GetSqlLiteral(point) + '}',
            "geometry[]",
            EDBDbType.Geometry | EDBDbType.Array,
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT st_makepoint(1,1)", conn);
    ...
    public async Task Roundtrip_geometry_geography()
    {
        ...
        await using (var cmd = new EDBCommand("INSERT INTO data (geom, geog) VALUES (@p, @p)", conn))
        {
            cmd.Parameters.AddWithValue("@p", point);
            cmd.ExecuteNonQuery();
        }

        await using (var cmd = new EDBCommand("SELECT geom, geog FROM data", conn))
        await using (var reader = cmd.ExecuteReader())
        {
            reader.Read();
    ...
    public async Task Concurrency_test()
    {
        ...
        await Task.WhenAll(Enumerable.Range(0, 30).Select(i => Task.Run(async () =>
        {
            for (var i = 0; i < 1000; i++)
            {
                ...
                await using (var cmd = new EDBCommand($"SELECT * FROM {table}", connection))
    ...
    protected ValueTask<EDBConnection> OpenConnectionAsync(string? connectionString = null, Ordinates handleOrdinates = Ordinates.None)
    {
    ...
    protected override EDBDataSource DataSource => _xyDataSource ?? throw new InvalidOperationException();
    EDBDataSource? _xyDataSource;

    ConcurrentDictionary<Ordinates, EDBDataSource> NtsDataSources = new();
}
