...
public class CommandTests : MultiplexingTestBase
{
    ...
    public async Task Cancel_cross_command()
    {
        ...
        await using var cmd2 = new EDBCommand("SELECT 1", conn);
    ...
    public async Task SingleRow([Values(PrepareOrNot.NotPrepared, PrepareOrNot.Prepared)] PrepareOrNot prepare)
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT 1, 2 UNION SELECT 3, 4", conn);
        if (prepare == PrepareOrNot.Prepared)
            cmd.Prepare();
    ...
    public async Task ExecuteScalar()
    {
        await using var conn = await OpenConnectionAsync();
        var table = await CreateTempTable(conn, "name TEXT");
        await using var command = new EDBCommand($"SELECT name FROM {table}", conn);
...
$$ LANGUAGE plpgsql;";
    ...
    public async Task Range_overflow_message_length_throws()
    {
        ...
        var range = new EDBRange<BigComposite>(composite, composite);
