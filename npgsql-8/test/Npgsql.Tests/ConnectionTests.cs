...
public class ConnectionTests : MultiplexingTestBase
{
    ...
    public void Bug1011001()
    {
        ...
        var csb1 = new EDBConnectionStringBuilder(@"Server=server;Port=5432;User Id=user;Password=passwor;Database=database;");
...
LANGUAGE 'plpgsql'");
    ...
    public async Task Clone_with_data_source()
    {
        ...
        Assert.That(clonedConnection.EDBDataSource, Is.SameAs(DataSource));
        ...
        else
        {
            ...
            Assert.That(async () => await cmd.ExecuteScalarAsync(),
                Throws.Exception.TypeOf<NotSupportedException>()
                    .With.Message.EqualTo("The EDBDbType 'IntegerMultirange' isn't present in your database. You may need to install an extension or upgrade to a newer version."));
        }
    }
...
//#endif
    ...
    {
        var originalApplicationName = new EDBConnectionStringBuilder(ConnectionString).ApplicationName ?? "";
    ...
    [Test]
    [Description("Test whether the internal EDBConnection.Open method stays on the same thread with async=false")]
