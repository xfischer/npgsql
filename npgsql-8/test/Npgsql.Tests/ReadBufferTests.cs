...
#if NETFRAMEWORK
        Writer
            .Write(EDBWriteBuffer.UTF8Encoding.GetBytes("foo"))
            .WriteByte(0)
            .Write(EDBWriteBuffer.UTF8Encoding.GetBytes("bar"))
            .WriteByte(0);
#else
        Writer
            .Write(EDBWriteBuffer.UTF8Encoding.GetBytes(new string("foo")))
            .WriteByte(0)
            .Write(EDBWriteBuffer.UTF8Encoding.GetBytes(new string("bar")))
            .WriteByte(0);
#endif
...
#if NETFRAMEWORK
        Writer.Write(EDBWriteBuffer.UTF8Encoding.GetBytes("Chunked "));
#else
        Writer.Write(EDBWriteBuffer.UTF8Encoding.GetBytes(new string("Chunked ")));
#endif
...
#if NETFRAMEWORK
        Writer
         .Write(EDBWriteBuffer.UTF8Encoding.GetBytes("string"))
         .WriteByte(0)
         .Write(EDBWriteBuffer.UTF8Encoding.GetBytes("bar"))
         .WriteByte(0);
#else
        Writer
            .Write(EDBWriteBuffer.UTF8Encoding.GetBytes(new string("string")))
            .WriteByte(0)
            .Write(EDBWriteBuffer.UTF8Encoding.GetBytes(new string("bar")))
            .WriteByte(0);
#endif
...
#pragma warning disable CS8625
    ...
    public void SetUp()
    {
        ...
        ReadBuffer = new EDBReadBuffer(null, stream, null, EDBReadBuffer.DefaultSize, EDBWriteBuffer.UTF8Encoding, EDBWriteBuffer.RelaxedUTF8Encoding);
        Writer = stream.Writer;
    }
#pragma warning restore CS8625

    // ReSharper disable once InconsistentNaming
    EDBReadBuffer ReadBuffer = default!;
    // ReSharper disable once InconsistentNaming
