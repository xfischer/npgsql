...
public class ReaderNewSchemaTests : SyncOrAsyncTestBase
{
    ...
    public async Task Allow_DBNull()
    {
        ...
        using var cmd = new EDBCommand($"SELECT nullable,non_nullable,8 FROM {table}", conn);
    ...
    public async Task BaseCatalogName()
    {
        ...
        using var cmd = new EDBCommand($"SELECT foo,8 FROM {table}", conn);
    ...
    public async Task BaseColumnName()
    {
        ...
        using var cmd = new EDBCommand($"SELECT foo, foo AS foobar, 8 AS bar, 8, '8'::VARCHAR(10) FROM {table}", conn);
    ...
    public async Task BaseColumnName_with_column_aliases()
    {
        ...
        var cmd = new EDBCommand("SELECT Cod as CodAlias, Descr as DescrAlias, Date, NULL AS Generated FROM data", conn);
    ...
    public async Task ColumnName()
    {
        ...
        using (var conn = await OpenConnectionAsync())
        {
            ...
            using var command = new EDBCommand($"SELECT col AS col_alias FROM {table}", conn);
    ...
    public async Task ColumnOrdinal()
    {
        ...
        using var cmd = new EDBCommand($"SELECT second,first FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task ColumnSize()
    {
        ...
        using var cmd = new EDBCommand($"SELECT bounded,unbounded,'a'::VARCHAR(10),'b'::VARCHAR FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task IsAutoIncrement()
    {
        ...
        await using var cmd = new EDBCommand($"SELECT serial, int, 8 FROM {table}", conn);
        await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task IsAutoIncrement_identity()
    {
        ...
        await using var cmd = new EDBCommand($"SELECT identity1, identity2 FROM {table}", conn);
        await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task IsIdentity()
    {
        ...
        await using var cmd = new EDBCommand($"SELECT identity1, identity2, serial, int, 8 FROM {table}", conn);
        await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task IsKey()
    {
        ...
        using var cmd = new EDBCommand($"SELECT id,non_id,uniq,8 FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task IsKey_composite()
    {
        ...
        using var cmd = new EDBCommand($"SELECT id1,id2 FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
        Assert.That(columns[0].IsKey, Is.True);
    ...
    public async Task IsLong()
    {
        ...
        using var cmd = new EDBCommand($"SELECT long, non_long, 8 FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
        var columns = await GetColumnSchema(reader);
...
CREATE TABLE {table} (bar INTEGER)");
        ...
        using var cmd = new EDBCommand($"SELECT foo,bar FROM {view},{table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
...
CREATE UNIQUE INDEX idx_{table} ON {table} (non_id_second, non_id_third)");
        ...
        using var cmd = new EDBCommand($"SELECT id,non_id,uniq,8,non_id_second,non_id_third FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task NumericPrecision()
    {
        ...
        using var cmd = new EDBCommand($"SELECT a,b,c,8.3::NUMERIC(8) FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task NumericScale()
    {
        ...
        using var cmd = new EDBCommand($"SELECT a,b,c,8.3::NUMERIC(8,5) FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task DataType()
    {
        ...
        using var cmd = new EDBCommand($"SELECT foo,8::INTEGER FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task DataType_unknown_type()
    {
        ...
        using var cmd = new EDBCommand($"SELECT foo::INTEGER FROM {table}", conn);
    ...
    public async Task DataType_with_composite()
    {
        ...
        await using var cmd = new EDBCommand($"SELECT comp,'(4)'::{type} FROM {tableName}", connection);
    ...
    public async Task DataTypeName(string typeName, int? size = null, int? precision = null, int? scale = null)
    {
        ...
        using var cmd = new EDBCommand($"SELECT foo,NULL::{typeName} FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task DefaultValue()
    {
        ...
        using var cmd = new EDBCommand($"SELECT with_default,without_default,8 FROM {table}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
...
CREATE TABLE {table2} (foo INTEGER)");
        ...
        using var cmd = new EDBCommand($"SELECT {table1}.foo,{table2}.foo FROM {table1},{table2}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
        Assert.That(columns[0].ColumnName, Is.EqualTo("foo"));
        Assert.That(columns[0].BaseTableName, Does.StartWith("temp_table"));
    ...
    public async Task Domain_type()
    {
        ...
        using var cmd = new EDBCommand($"SELECT domain FROM {tableName}", conn);
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
        var columns = await GetColumnSchema(reader);
    ...
    public async Task EDBDbType()
    {
        ...
        using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly);
        var columns = await GetColumnSchema(reader);
        Assert.That(columns[0].EDBDbType, Is.EqualTo(EDBTypes.EDBDbType.Integer));
        Assert.That(columns[1].EDBDbType, Is.EqualTo(EDBTypes.EDBDbType.Integer));
    }

    [Test]
    [NonParallelizable]
    public async Task EDBDbType_extension()
    {
        using var conn = await OpenConnectionAsync();
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand("COMMIT", conn);
    ...
    public async Task IsAliased()
    {
        ...
        using var cmd = new EDBCommand($"SELECT foo, foo AS bar, NULL AS foobar FROM {table}", conn);
        await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
    ...
    public async Task With_parameter_without_value()
    {
        ...
        using var cmd = new EDBCommand($"SELECT foo FROM {table} WHERE foo > @p", conn)
        {
            Parameters = { new() { ParameterName = "p", EDBDbType = EDBTypes.EDBDbType.Integer } }
        };
        await using var reader = await cmd.ExecuteReaderAsync(CommandBehavior.SchemaOnly | CommandBehavior.KeyInfo);
