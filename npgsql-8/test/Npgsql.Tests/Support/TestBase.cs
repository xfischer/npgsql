...
public abstract class TestBase
{
    ...
    public Task<InvalidCastException> AssertTypeUnsupportedRead<T>(string sqlLiteral, string pgTypeName, EDBDataSource? dataSource = null)
        => AssertTypeUnsupportedRead<T, InvalidCastException>(sqlLiteral, pgTypeName, dataSource);

    public async Task<TException> AssertTypeUnsupportedRead<T, TException>(string sqlLiteral, string pgTypeName, EDBDataSource? dataSource = null)
    ...
    public async Task<TException> AssertTypeUnsupportedWrite<T, TException>(T value, string? pgTypeName = null, EDBDataSource? dataSource = null)
        where TException : Exception
    {
        dataSource ??= DataSource;
