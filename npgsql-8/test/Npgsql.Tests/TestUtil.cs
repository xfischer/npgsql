...
public static class TestUtil
{
    ...
    "port=5445;Server=localhost;Username=enterprisedb;Password=edb;Database=test;Timeout=0;Command Timeout=0;SSL Mode=Disable;Multiplexing=False";
    ...
    public static void MinimumPgVersion(EDBDataSource dataSource, string minVersion, string? ignoreText = null)
    {
    ...
    public static void MaximumPgVersionExclusive(EDBConnection conn, string maxVersion, string? ignoreText = null)
    {
    ...
    public static void IgnoreOnRedshift(EDBConnection conn, string? ignoreText = null)
    {
        if (new EDBConnectionStringBuilder(conn.ConnectionString).ServerCompatibilityMode == ServerCompatibilityMode.Redshift)
        {
    ...
    public static bool IsPgPrerelease(EDBConnection conn)
    ...
    public static Task EnsureExtensionAsync(EDBConnection conn, string extension, string? minVersion = null)
        => EnsureExtension(conn, extension, minVersion, async: true);

    static async Task EnsureExtension(EDBConnection conn, string extension, string? minVersion, bool async)
    {
    ...
    public static void IgnoreIfFeatureNotSupported(EDBConnection conn, string testQuery)
    ...
    /// <param name="testQuery">The query to test for the feature.
    /// This query needs to fail with SqlState 0A000 (feature_not_supported) if the feature isn't present.</param>
    public static Task IgnoreIfFeatureNotSupportedAsync(EDBConnection conn, string testQuery)
        => IgnoreIfFeatureNotSupported(conn, testQuery, async: true);

    static async Task IgnoreIfFeatureNotSupported(EDBConnection conn, string testQuery, bool async)
    {
        try
        {
            if (async)
    ...
    /// </summary>
    internal static async Task<string> CreateTempTable(EDBConnection conn, string columns)
    {
...
CREATE TABLE {tableName} ({columns});");
    ...
    /// </summary>
    internal static async Task<string> GetTempTableName(EDBConnection conn)
    {
        var tableName = "temp_table" + Interlocked.Increment(ref _tempTableCounter);
        await conn.ExecuteNonQueryAsync(@$"
START TRANSACTION;
...
COMMIT");
    ...
    /// </summary>
    internal static async Task<string> CreateTempTable(EDBDataSource dataSource, string columns)
    {
        var tableName = "temp_table" + Interlocked.Increment(ref _tempTableCounter);
...
CREATE TABLE {tableName} ({columns});");
    ...
    /// </summary>
    internal static async Task<string> CreateTempSchema(EDBConnection conn)
    {
    ...
    /// </summary>
    internal static async Task<string> GetTempViewName(EDBConnection conn)
    {
    ...
    /// </summary>
    internal static async Task<string> GetTempMaterializedViewName(EDBConnection conn)
    {
    ...
    /// </summary>
    internal static async Task<string> GetTempFunctionName(EDBConnection conn)
    {
    ...
    /// </returns>
    internal static async Task<string> GetTempProcedureName(EDBDataSource dataSource)
    {
    ...
    /// An <see cref="IDisposable"/> to drop the function at the end of the test.
    /// </returns>
    internal static async Task<string> GetTempProcedureName(EDBConnection connection)
    {
        var procedureName = "temp_procedure" + Interlocked.Increment(ref _tempProcedureCounter);
    ...
    /// </summary>
    internal static async Task<string> GetTempTypeName(EDBConnection conn)
    {
    ...
    {
#if DEBUG
        EDBCommand.EnableSqlRewriting = false;
        return new DeferredExecutionDisposable(() => EDBCommand.EnableSqlRewriting = true);
#else
...
public static class EDBConnectionExtensions
{
    ...
    {
        using var command = tx == null ? new EDBCommand(sql, conn) : new EDBCommand(sql, conn, tx);
    ...
    public static async Task<int> ExecuteNonQueryAsync(
        this EDBConnection conn, string sql, EDBTransaction? tx = null, CancellationToken cancellationToken = default)
    {
        await using var command = tx == null ? new EDBCommand(sql, conn) : new EDBCommand(sql, conn, tx);
...
public static class EDBDataSourceExtensions
{
    public static int ExecuteNonQuery(this EDBDataSource dataSource, string sql)
    {
        using var command = dataSource.CreateCommand(sql);
        return command.ExecuteNonQuery();
    }

    public static object? ExecuteScalar(this EDBDataSource dataSource, string sql)
    {
        using var command = dataSource.CreateCommand(sql);
        return command.ExecuteScalar();
    }
    ...
    public static async Task<int> ExecuteNonQueryAsync(
        this EDBDataSource dataSource, string sql, CancellationToken cancellationToken = default)
    {
