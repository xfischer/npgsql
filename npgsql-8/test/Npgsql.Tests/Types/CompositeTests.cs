...
[NonParallelizable] // fails in multiplexing // parrallel
public class CompositeTests : MultiplexingTestBase
{
    ...
    class CustomTranslator : IEDBNameTranslator
    {
...
#pragma warning disable CS0618 // GlobalTypeMapper is obsolete
    ...
    public async Task Global_mapping()
    {
        ...
        await adminConnection.ExecuteNonQueryAsync($"CREATE TYPE {type} AS (x int, some_text text)");
        EDBConnection.GlobalTypeMapper.MapComposite<SomeComposite>(type);
...
#endif
        if (TestUtil.IsEPASRedwood(connection) ?? false)
        {
            await AssertType(
               connection,
               new SomeCompositeWithConverterResolverType { DateTimes = new[] { new DateTime(unixEpoch.Ticks, DateTimeKind.Unspecified), new DateTime(unixEpoch.Ticks, DateTimeKind.Unspecified).AddDays(1) } },
               """("{""01-JAN-70 00:00:00"",""02-JAN-70 00:00:00""}")""",
               compositeType,
               npgsqlDbType: null,
...
CREATE TYPE {compositeType} AS (date_times timestamp[])");
        ...
        Assert.ThrowsAsync<ArgumentException>(() => AssertType(
            connection,
            new SomeCompositeWithConverterResolverType { DateTimes = new[] { unixEpoch } }, // UTC DateTime
...
CREATE TYPE {type2} AS (comp {type1}, comps {type1}[]);");
    ...
    public async Task CompositeOverRange()
    {
        ...
        await AssertType(
            connection,
            new EDBRange<SomeComposite>(composite1, composite2),
            "[\"(8,foo)\",\"(42,bar)\"]",
            rangeType,
            npgsqlDbType: null,
            isDefaultForWriting: false);
