...
public class RecordTests : MultiplexingTestBase
{
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT ('one'::TEXT, 2)", conn);
        await using var reader = await cmd.ExecuteReaderAsync();
        reader.Read();
    ...
    [Test]
    public async Task Records_not_supported_by_default_on_EDBSlimSourceBuilder()
    {
    ...
    [Test]
    public async Task EDBSlimSourceBuilder_EnableRecords()
    {
        var dataSourceBuilder = new EDBSlimDataSourceBuilder(ConnectionString);
