...
/// <summary>
///     Manages resolving and creating <see cref="EDBDataSourceBuilder" /> instances.
/// </summary>
/// <remarks>
///     <para>
...
public class NpgsqlDataSourceManager : IDisposable, IAsyncDisposable
{
    ...
    private readonly ConcurrentDictionary<string, EDBDataSource> _dataSources = new();
    ...
    public virtual DbDataSource? GetDataSource(NpgsqlOptionsExtension? npgsqlOptionsExtension, IServiceProvider? applicationServiceProvider)
        => npgsqlOptionsExtension switch
        {
            ...
            { ConnectionString: null } when applicationServiceProvider?.GetService<EDBDataSource>() is DbDataSource dataSource
                => dataSource,
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    protected virtual EDBDataSource CreateDataSource(NpgsqlOptionsExtension npgsqlOptionsExtension)
    {
        var dataSourceBuilder = new EDBDataSourceBuilder(npgsqlOptionsExtension.ConnectionString);
