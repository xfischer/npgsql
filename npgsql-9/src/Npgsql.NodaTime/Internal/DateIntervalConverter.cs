...
public class DateIntervalConverter : PgStreamingConverter<DateInterval>
{
    ...
    readonly PgConverter<EDBRange<LocalDate>> _rangeConverter;

    public DateIntervalConverter(PgConverter<EDBRange<LocalDate>> rangeConverter, bool dateTimeInfinityConversions)
    {
    ...
    public override Size GetSize(SizeContext context, DateInterval value, ref object? writeState)
        => _rangeConverter.GetSize(context, new EDBRange<LocalDate>(value.Start, value.End), ref writeState);

    public override void Write(PgWriter writer, DateInterval value)
        => _rangeConverter.Write(writer, new EDBRange<LocalDate>(value.Start, value.End));

    public override ValueTask WriteAsync(PgWriter writer, DateInterval value, CancellationToken cancellationToken = default)
        => _rangeConverter.WriteAsync(writer, new EDBRange<LocalDate>(value.Start, value.End), cancellationToken);
}
