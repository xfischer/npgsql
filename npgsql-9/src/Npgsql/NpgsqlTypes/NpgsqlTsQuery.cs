...
/// </summary>
public abstract class EDBTsQuery : IEquatable<EDBTsQuery>
{
    /// <summary>
    /// Node kind
    /// </summary>
    ...
    /// <summary>
    /// Constructs an <see cref="EDBTsQuery"/>.
    /// </summary>
    /// <param name="kind"></param>
    protected EDBTsQuery(NodeKind kind) => Kind = kind;
    ...
    /// <returns></returns>
    [Obsolete("Client-side parsing of EDBTsQuery is unreliable and cannot fully duplicate the PostgreSQL logic. Use PG functions instead (e.g. to_tsquery)")]
    public static EDBTsQuery Parse(string value)
    {
        ...
        if (ch == ')')
        {
            while (opStack.Count > 0 && opStack.Peek() != '(')
            {
                ...
                {
                    '&' => new EDBTsQueryAnd(left, right),
                    '|' => new EDBTsQueryOr(left, right),
                    '<' => new EDBTsQueryFollowedBy(left, tsOp.FollowedByDistance, right),
        ...
        {
            valStack.Push(new EDBTsQueryLexeme(sb.ToString()));
            goto PushedVal;
        }
        pos++;
        if (ch == ':')
        ...
        {
        case '*':
            ((EDBTsQueryLexeme)valStack.Peek()).IsPrefixSearch = true;
            break;
        case 'a' or 'A':
            ((EDBTsQueryLexeme)valStack.Peek()).Weights |= EDBTsQueryLexeme.Weight.A;
            break;
        case 'b' or 'B':
            ((EDBTsQueryLexeme)valStack.Peek()).Weights |= EDBTsQueryLexeme.Weight.B;
            break;
        case 'c' or 'C':
            ((EDBTsQueryLexeme)valStack.Peek()).Weights |= EDBTsQueryLexeme.Weight.C;
            break;
        case 'd' or 'D':
            ((EDBTsQueryLexeme)valStack.Peek()).Weights |= EDBTsQueryLexeme.Weight.D;
            break;
        default:
            goto PushedVal;
        }
        ...
        while (opStack.Count > 0)
        {
            ...
            {
                '&' => (EDBTsQuery)new EDBTsQueryAnd(left, right),
                '|' => new EDBTsQueryOr(left, right),
                '<' => new EDBTsQueryFollowedBy(left, tsOp.FollowedByDistance, right),
                _   => throw new FormatException("Syntax error in tsquery")
            };
    ...
    public override bool Equals(object? obj)
        => obj is EDBTsQuery query && query.Equals(this);

    /// <summary>
    /// Returns a value indicating whether this instance and a specified <see cref="EDBTsQuery"/> object represent the same value.
    /// </summary>
    /// <param name="other">An object to compare to this instance.</param>
    /// <returns><see langword="true"/> if g is equal to this instance; otherwise, <see langword="false"/>.</returns>
    public abstract bool Equals(EDBTsQuery? other);

    /// <summary>
    /// Indicates whether the values of two specified <see cref="EDBTsQuery"/> objects are equal.
    /// </summary>
    /// <param name="left">The first object to compare.</param>
    /// <param name="right">The second object to compare.</param>
    /// <returns><see langword="true"/> if <paramref name="left"/> and <paramref name="right"/> are equal; otherwise, <see langword="false"/>.</returns>
    public static bool operator ==(EDBTsQuery? left, EDBTsQuery? right)
        => left is null ? right is null : left.Equals(right);


    /// <summary>
    /// Indicates whether the values of two specified <see cref="EDBTsQuery"/> objects are not equal.
    /// </summary>
    /// <param name="left">The first object to compare.</param>
    /// <param name="right">The second object to compare.</param>
...
/// TsQuery Lexeme node.
/// </summary>
public sealed class EDBTsQueryLexeme : EDBTsQuery
{
    string _text;
    ...
    public EDBTsQueryLexeme(string text) : this(text, Weight.None, false) { }
    ...
    public EDBTsQueryLexeme(string text, Weight weights) : this(text, weights, false) { }
    ...
    public EDBTsQueryLexeme(string text, Weight weights, bool isPrefixSearch)
    ...
    /// <inheritdoc/>
    public override bool Equals(EDBTsQuery? other)
        => other is EDBTsQueryLexeme lexeme &&
           lexeme.Text == Text &&
...
/// TsQuery Not node.
/// </summary>
public sealed class EDBTsQueryNot : EDBTsQuery
{
    /// <summary>
    /// Child node
    /// </summary>
    public EDBTsQuery Child { get; set; }
    ...
    /// <inheritdoc/>
    public override bool Equals(EDBTsQuery? other)
        => other is EDBTsQueryNot not && not.Child == Child;
...
/// </summary>
public abstract class EDBTsQueryBinOp : EDBTsQuery
{
    /// <summary>
    /// Left child
    /// </summary>
    public EDBTsQuery Left { get; set; }

    /// <summary>
    /// Right child
    /// </summary>
    public EDBTsQuery Right { get; set; }

    /// <summary>
    /// Constructs a <see cref="EDBTsQueryBinOp"/>.
    /// </summary>
    protected EDBTsQueryBinOp(NodeKind kind, EDBTsQuery left, EDBTsQuery right)
        : base(kind)
    {
        Left = left;
        Right = right;
...
/// TsQuery And node.
/// </summary>
public sealed class EDBTsQueryAnd : EDBTsQueryBinOp
{
    /// <summary>
    ...
    public EDBTsQueryAnd(EDBTsQuery left, EDBTsQuery right)
    ...
    /// <inheritdoc/>
    public override bool Equals(EDBTsQuery? other)
        => other is EDBTsQueryAnd and && and.Left == Left && and.Right == Right;
...
/// TsQuery Or Node.
/// </summary>
public sealed class EDBTsQueryOr : EDBTsQueryBinOp
{
    /// <summary>
    ...
    /// <param name="left"></param>
    /// <param name="right"></param>
    public EDBTsQueryOr(EDBTsQuery left, EDBTsQuery right)
    ...
    /// <inheritdoc/>
    public override bool Equals(EDBTsQuery? other)
        => other is EDBTsQueryOr or && or.Left == Left && or.Right == Right;
...
public sealed class EDBTsQueryFollowedBy : EDBTsQueryBinOp
{
    ...
    /// <inheritdoc/>
    public override bool Equals(EDBTsQuery? other)
        => other is EDBTsQueryFollowedBy followedBy &&
...
public sealed class EDBTsQueryEmpty : EDBTsQuery
{
    ...
    /// </summary>
    public EDBTsQueryEmpty() : base(NodeKind.Empty) {}

    internal override void WriteCore(StringBuilder sb, bool first = false) { }

    /// <inheritdoc/>
    public override bool Equals(EDBTsQuery? other)
        => other is EDBTsQueryEmpty;
