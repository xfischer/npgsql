...
sealed class PreparedStatementManager
{
    ...
    private HashSet<long> _assignedTimestamps = new(); // EnterpriseDB Team
    ...
    static readonly List<EDBParameter> EmptyParameters = new(); // EnterpriseDB 
    ...
    internal PreparedStatement? TryGetAutoPrepared(EDBBatchCommand batchCommand)
    {
        ...
        PreparedStatement? TryGetAutoPreparedSlow(EDBBatchCommand batchCommand, PreparedStatement? pStatement)
        {
            var sql = batchCommand.FinalCommandText!;
            ...
            case PreparedState.BeingPrepared:
                ...
                if (!pStatement.DoParametersMatch(batchCommand.CurrentParametersReadOnly))
                    return null;
                // Prevent this statement from being replaced within this batch
                SetLastUsed(pStatement, long.MaxValue); // EnterpriseDB Team (SetLastUsed)
                return pStatement;
            ...
            if (++pStatement.Usages < UsagesBeforePrepare)
            {
                ...
                SetLastUsed(pStatement, Stopwatch.GetTimestamp()); // EnterpriseDB Team (SetLastUsed)
                return null;
            }
            ...
            // EDBMERGE: removed Linq for native AOT
            ...
            LogMessages.TryEDBTrace(_commandLogger, $"oldPreparedStatement: AutoPrepared (index: {selectedIndex})");
    ...
    {
        _assignedTimestamps.Clear(); // EnterpriseDB Team (SetLastUsed)
        BySql.Clear();
        NumPrepared = 0;
    ...
    // EnterpriseDB: prevent DateTime.UtcNow collision bug in .net Framework (EC-2772)
    // due to > 1 ms precision error
    internal void SetLastUsed(PreparedStatement pStatement, long timeStamp)
    {
        if (pStatement.LastUsed == timeStamp)
            return;

#if NETFRAMEWORK
        if (timeStamp < long.MaxValue)
        {
            _assignedTimestamps.Remove(pStatement.LastUsed);
            while (_assignedTimestamps.Contains(timeStamp))
            {
                timeStamp++;
            }
            _assignedTimestamps.Add(timeStamp);
        }
#endif

        pStatement.LastUsed = timeStamp;
    }
}
