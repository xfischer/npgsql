...
sealed class UserTypeMapper : PgTypeInfoResolverFactory
{
    ...
    public IEDBNameTranslator DefaultNameTranslator { get; set; } = EDBSnakeCaseNameTranslator.Instance;
    ...
    public UserTypeMapper MapEnum<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicFields)] TEnum>(string? pgName = null, IEDBNameTranslator? nameTranslator = null)
    ...
    public UserTypeMapper MapEnum([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicFields | DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)]Type clrType, string? pgName = null, IEDBNameTranslator? nameTranslator = null)
    {
        if (!clrType.IsEnum || !clrType.IsValueType)
            throw new ArgumentException("Type must be a concrete Enum", nameof(clrType));

        var openMethod = typeof(UserTypeMapper).GetMethod(nameof(MapEnum), new[] { typeof(string), typeof(IEDBNameTranslator) })!;
        var method = openMethod.MakeGenericMethod(clrType);
        method.Invoke(this, new object?[] { pgName, nameTranslator });
        return this;
    }

    public bool UnmapEnum([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicFields)]Type clrType,string? pgName = null, IEDBNameTranslator? nameTranslator = null)
    {
        if (!clrType.IsEnum || !clrType.IsValueType)
            throw new ArgumentException("Type must be a concrete Enum", nameof(clrType));
    ...
    public UserTypeMapper MapComposite<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicFields | DynamicallyAccessedMemberTypes.PublicProperties)] T>(
        string? pgName = null, IEDBNameTranslator? nameTranslator = null) where T : class
    {
    ...
    public UserTypeMapper MapStructComposite<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicFields | DynamicallyAccessedMemberTypes.PublicProperties)] T>(
        string? pgName = null, IEDBNameTranslator? nameTranslator = null) where T : struct
    {
        Unmap(typeof(T), out var resolvedName, pgName, nameTranslator);
        ...
        Type clrType, string? pgName = null, IEDBNameTranslator? nameTranslator = null)
    {
        ...
        var openMethod = typeof(UserTypeMapper).GetMethod(
            ...
            new[] { typeof(string), typeof(IEDBNameTranslator) })!;
    ...
    public bool UnmapComposite<T>(string? pgName = null, IEDBNameTranslator? nameTranslator = null) where T : class
        => UnmapComposite(typeof(T), pgName, nameTranslator);

    public bool UnmapStructComposite<T>(string? pgName = null, IEDBNameTranslator? nameTranslator = null) where T : struct
        => UnmapComposite(typeof(T), pgName, nameTranslator);
    ...
    bool Unmap(Type type, out string resolvedName, string? pgName = null, IEDBNameTranslator? nameTranslator = null)
    {
    ...
    static string GetPgName(Type type, IEDBNameTranslator nameTranslator)
    ...
    sealed class CompositeMapping<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicFields | DynamicallyAccessedMemberTypes.PublicProperties)] T> : UserTypeMapping where T : class
    {
        ...
        public CompositeMapping(string pgTypeName, IEDBNameTranslator nameTranslator)
    ...
    sealed class StructCompositeMapping<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.PublicFields | DynamicallyAccessedMemberTypes.PublicProperties)] T> : UserTypeMapping where T : struct
    {
        ...
        public StructCompositeMapping(string pgTypeName, IEDBNameTranslator nameTranslator)
            : base(pgTypeName, typeof(T))
            => _nameTranslator = nameTranslator;
    ...
    {
        internal IEDBNameTranslator NameTranslator { get; }

        public EnumMapping(string pgTypeName, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicFields)]Type enumClrType, IEDBNameTranslator nameTranslator)
        ...
        where TEnum : struct, Enum
    {
        ...
        public EnumMapping(string pgTypeName, IEDBNameTranslator nameTranslator)
