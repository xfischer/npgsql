...
public class NodaTimeTests : MultiplexingTestBase, IDisposable
{
    ...
    public Task Timestamp_as_LocalDateTime(LocalDateTime localDateTime, string sqlLiteral)
        => AssertType(localDateTime, sqlLiteral, "timestamp without time zone", EDBDbType.Timestamp, DbType.DateTime2,
    ...
    [Test]
    public async Task Tsrange_as_EDBRange_of_LocalDateTime()
    {
        await AssertType(
            new EDBRange<LocalDateTime>(
                new(1998, 4, 12, 13, 26, 38),
                new(1998, 4, 12, 15, 26, 38)),
            """["1998-04-12 13:26:38","1998-04-12 15:26:38"]""",
            "tsrange",
            EDBDbType.TimestampRange,
            isEDBDbTypeInferredFromClrType: false, skipArrayCheck: true); // EDBRange<T>[] is mapped to multirange by default, not array; test separately

        await AssertType(
            new [] { new EDBRange<LocalDateTime>(
                new(1998, 4, 12, 13, 26, 38),
                new(1998, 4, 12, 15, 26, 38)), },
            """{"[\"1998-04-12 13:26:38\",\"1998-04-12 15:26:38\"]"}""",
            "tsrange[]",
        ...
        await AssertType(
            ...
            """{["1998-04-12 13:26:38","1998-04-12 15:26:38"]}""",
            "tsmultirange",
            EDBDbType.TimestampMultirange, isEDBDbTypeInferredFromClrType: false, skipArrayCheck: true);
    }

    [Test]
    public async Task Tsmultirange_as_array_of_EDBRange_of_LocalDateTime()
    {
        await using var conn = await OpenConnectionAsync();
    ...
    public Task Timestamptz_as_Instant(Instant instant, string sqlLiteral)
        => AssertType(instant, sqlLiteral, "timestamp with time zone", EDBDbType.TimestampTz, DbType.DateTime,
            isEDBDbTypeInferredFromClrType: false);
    ...
    public async Task Tstzrange_as_Interval()
    {
         ...
         await AssertType(
             ...
             """{["1998-04-12 15:26:38+02","1998-04-12 17:26:38+02")}""",
             "tstzmultirange",
             EDBDbType.TimestampTzMultirange, isEDBDbTypeInferredFromClrType: false, skipArrayCheck: true);
    }
    ...
    public Task Tstzrange_with_no_end_as_Interval()
        => AssertType(
            ...
            "tstzrange",
            EDBDbType.TimestampTzRange,
            isEDBDbTypeInferredFromClrType: false, skipArrayCheck: true);
    ...
    [Test]
    public Task Tstzrange_as_EDBRange_of_Instant()
        => AssertType(
            new EDBRange<Instant>(
                new LocalDateTime(1998, 4, 12, 13, 26, 38).InUtc().ToInstant(),
                new LocalDateTime(1998, 4, 12, 15, 26, 38).InUtc().ToInstant()),
            """["1998-04-12 15:26:38+02","1998-04-12 17:26:38+02"]""",
            "tstzrange",
    ...
    [Test]
    public Task Tstzrange_as_EDBRange_of_ZonedDateTime()
        => AssertType(
            new EDBRange<ZonedDateTime>(
                new LocalDateTime(1998, 4, 12, 13, 26, 38).InUtc(),
    ...
    [Test]
    public Task Tstzrange_as_EDBRange_of_OffsetDateTime()
        => AssertType(
            new EDBRange<OffsetDateTime>(
                new LocalDateTime(1998, 4, 12, 13, 26, 38).WithOffset(Offset.Zero),
    ...
    [Test]
    public async Task Tstzmultirange_as_array_of_EDBRange_of_Instant()
    {
        await using var conn = await OpenConnectionAsync();
        MinimumPgVersion(conn, "14.0", "Multirange types were introduced in PostgreSQL 14");
    ...
    [Test]
    public async Task Tstzmultirange_as_array_of_EDBRange_of_ZonedDateTime()
    {
        await using var conn = await OpenConnectionAsync();
        MinimumPgVersion(conn, "14.0", "Multirange types were introduced in PostgreSQL 14");
    ...
    [Test]
    public async Task Tstzmultirange_as_array_of_EDBRange_of_OffsetDateTime()
    {
        await using var conn = await OpenConnectionAsync();
        MinimumPgVersion(conn, "14.0", "Multirange types were introduced in PostgreSQL 14");
    ...
    [Test]
    public async Task Tstzrange_array_as_array_of_EDBRange_of_Instant()
    {
        await using var conn = await OpenConnectionAsync();
    ...
    public async Task Daterange_as_DateInterval()
    {
        await AssertType(
            ...
            "daterange",
            EDBDbType.DateRange,
            isEDBDbTypeInferredFromClrType: false, skipArrayCheck: true); // DateInterval<T>[] is mapped to multirange by default, not array; test separately
        ...
        await AssertType(
            ...
            """{[2002-03-04,2002-03-07)}""",
            "datemultirange",
            EDBDbType.DateMultirange, isEDBDbTypeInferredFromClrType: false, skipArrayCheck: true);
    }

    [Test]
    public async Task Daterange_as_EDBRange_of_LocalDate()
    {
         await AssertType(
            new EDBRange<LocalDate>(new(2002, 3, 4), true, new(2002, 3, 6), false),
            "[2002-03-04,2002-03-06)",
            "daterange",
            EDBDbType.DateRange,
            isEDBDbTypeInferredFromClrType: false,
            isDefaultForReading: false, skipArrayCheck: true); // EDBRange<T>[] is mapped to multirange by default, not array; test separately

         await AssertType(
             new [] { new EDBRange<LocalDate>(new(2002, 3, 4), true, new(2002, 3, 6), false) },
             """{"[2002-03-04,2002-03-06)"}""",
             "daterange[]",
             EDBDbType.DateRange | EDBDbType.Array,
             isDefault: false, skipArrayCheck: true);
         ...
         await AssertType(
             ...
             """{[2002-03-04,2002-03-06)}""",
             "datemultirange",
             EDBDbType.DateMultirange, isDefault: false, skipArrayCheck: true);
    }
    ...
    [Test]
    public async Task Datemultirange_as_array_of_EDBRange_of_LocalDate()
    {
        await using var conn = await OpenConnectionAsync();
        MinimumPgVersion(conn, "14.0", "Multirange types were introduced in PostgreSQL 14");
        ...
        await AssertType(
            ...
            {
                new EDBRange<LocalDate>(new(2002, 3, 4), true, new(2002, 3, 6), false),
                new EDBRange<LocalDate>(new(2002, 3, 8), true, new(2002, 3, 11), false)
            },
            "{[2002-03-04,2002-03-06),[2002-03-08,2002-03-11)}",
            "datemultirange",
            EDBDbType.DateMultirange,
    ...
    public async Task Daterange_as_EDBRange_of_DateOnly()
    {
        ...
        await AssertType(
            new [] { new EDBRange<DateOnly>(new(2002, 3, 4), true, new(2002, 3, 6), false) },
            """{"[2002-03-04,2002-03-06)"}""",
            "daterange[]",
            EDBDbType.DateRange | EDBDbType.Array,
            isDefault: false, skipArrayCheck: true);
    ...
    [Test]
    public async Task Daterange_array_as_array_of_EDBRange_of_LocalDate()
    {
        await using var conn = await OpenConnectionAsync();
    ...
    public Task Time_as_LocalTime()
        => AssertType(new LocalTime(10, 45, 34, 500), "10:45:34.5", "time without time zone", EDBDbType.Time, DbType.Time,
            isEDBDbTypeInferredFromClrType: false);
    ...
    public async Task Interval_as_Duration_with_months_fails()
    {
        ...
        Assert.That(exception.Message, Is.EqualTo(EDBNodaTimeStrings.CannotReadIntervalWithMonthsAsDuration));
    }
    ...
    public async Task Period_write_throw_on_overflow()
    {
        ...
        Assert.That(ex.Message, Is.EqualTo(EDBNodaTimeStrings.CannotWritePeriodDueToOverflow));
