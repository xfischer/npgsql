...
public class AuthenticationTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    {
        using var _ = SetEnvironmentVariable("PGUSER", new EDBConnectionStringBuilder(ConnectionString).Username);
    ...
    {
        using var _ = SetEnvironmentVariable("PGPASSWORD", new EDBConnectionStringBuilder(ConnectionString).Password);
        await using var dataSource = CreateDataSource(csb => csb.Passfile = null);
        await using var __ = await dataSource.OpenConnectionAsync();
    }

    [Test]
    public async Task Set_Password_on_EDBDataSource()
    {
        var dataSourceBuilder = GetPasswordlessDataSourceBuilder();
        await using var dataSource = dataSourceBuilder.Build();

        // No password provided
        Assert.That(() => dataSource.OpenConnectionAsync(), Throws.Exception.TypeOf<EDBException>());

        var connectionStringBuilder = new EDBConnectionStringBuilder(TestUtil.ConnectionString);
    ...
    {
        var dataSourceBuilder = GetPasswordlessDataSourceBuilder();
        var password = new EDBConnectionStringBuilder(TestUtil.ConnectionString).Password!;
    ...
    public void Password_provider_exception()
    {
        ...
        using var dataSource = dataSourceBuilder.Build();
        Assert.ThrowsAsync<EDBException>(async () => await dataSource.OpenConnectionAsync());
    }
    ...
    public async Task Periodic_password_provider_with_first_time_exception()
    {
        ...
        Assert.That(() => dataSource.OpenConnection(), Throws.Exception.TypeOf<EDBException>()
            .With.InnerException.With.Message.EqualTo("FOO"));
    }
    ...
    {
        var dataSourceBuilder = new EDBDataSourceBuilder(TestUtil.ConnectionString);
        ...
        Assert.That(() => dataSourceBuilder.Build(), Throws.Exception.TypeOf<NotSupportedException>()
            .With.Message.EqualTo(EDBStrings.CannotSetBothPasswordProviderAndPassword));
    }
    ...
    public void Multiple_password_providers_is_not_supported()
    {
        ...
        Assert.That(() => dataSourceBuilder.Build(), Throws.Exception.TypeOf<NotSupportedException>()
            .With.Message.EqualTo(EDBStrings.CannotSetMultiplePasswordProviderKinds));
    }
    ...
    {
        await using var dataSource = EDBDataSource.Create(new EDBConnectionStringBuilder(ConnectionString)
        {
            Username = null,
            Password = null
        });
...
#pragma warning restore CS0618 // ProvidePasswordCallback is Obsolete
    ...
    EDBDataSourceBuilder GetPasswordlessDataSourceBuilder()
