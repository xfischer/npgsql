...
using static EnterpriseDB.EDBClient.Tests.Support.MockState;
...
public class MultipleHostsTests : TestBase
{
    ...
    public async Task Valid_host_not_found(TargetSessionAttributes targetSessionAttributes, MockState[] servers)
    {
        ...
        var exception = Assert.ThrowsAsync<EDBException>(async () => await dataSource.OpenConnectionAsync(targetSessionAttributes))!;
    ...
    public void All_hosts_are_down()
    {
        ...
        using var dataSource = new EDBDataSourceBuilder(connectionString).BuildMultiHost();

        var exception = Assert.ThrowsAsync<EDBException>(async () => await dataSource.OpenConnectionAsync(TargetSessionAttributes.Any))!;
        ...
        {
            Assert.That(aggregateException.InnerExceptions[i], Is.TypeOf<EDBException>()
        ...
        [Values(PostgresErrorCodes.InvalidCatalogName, PostgresErrorCodes.CannotConnectNow)] string errorCode)
    {
        ...
        await using var dataSource = new EDBDataSourceBuilder(builder.ConnectionString).BuildMultiHost();
    ...
    public async Task First_host_is_down()
    {
        ...
        await using var dataSource = new EDBDataSourceBuilder(connectionString).BuildMultiHost();
    ...
    public async Task TargetSessionAttributes_with_single_host(string targetSessionAttributes)
    {
        ...
        {
            Assert.That(() => new EDBConnection(connectionString), Throws.Exception.TypeOf<NotSupportedException>());
        }
    }

    [Test]
    public void TargetSessionAttributes_default_is_null()
        => Assert.That(new EDBConnectionStringBuilder().TargetSessionAttributes, Is.Null);
    ...
    public async Task TargetSessionAttributes_uses_environment_variable()
    {
        ...
        await using var dataSource = new EDBDataSourceBuilder(builder.ConnectionString)
            .BuildMultiHost();
    ...
    public async Task Connect_with_load_balancing()
    {
        ...
        var defaultCsb = new EDBConnectionStringBuilder
        {
            Host = MultipleHosts(primaryPostmaster, standbyPostmaster),
            ServerCompatibilityMode = ServerCompatibilityMode.NoTypeLoading,
            MaxPoolSize = 1,
        ...
        await using var dataSource = new EDBDataSourceBuilder(defaultCsb.ConnectionString)
            .BuildMultiHost();

        EDBConnector firstConnector;
        EDBConnector secondConnector;
    ...
    public async Task Offline_state_on_connection_failure()
    {
        ...
        var state = conn.EDBDataSource.GetDatabaseState();
    ...
    public async Task Offline_state_on_query_execution_pg_critical_failure()
    {
        ...
        var state = conn.EDBDataSource.GetDatabaseState();
        Assert.That(state, Is.EqualTo(DatabaseState.Unknown));
        Assert.That(conn.EDBDataSource.Statistics.Total, Is.EqualTo(2));
        ...
        state = conn.EDBDataSource.GetDatabaseState();
        Assert.That(state, Is.EqualTo(DatabaseState.Offline));
    ...
    public async Task Offline_state_on_query_execution_IOException()
    {
        ...
        var ex = Assert.ThrowsAsync<EDBException>(() => conn.ExecuteNonQueryAsync("SELECT 1"))!;
    ...
    public async Task Unknown_state_on_query_execution_TimeoutException_with_cancellation_failure()
    {
        ...
        var ex = Assert.ThrowsAsync<EDBException>(async () => await query)!;
        Assert.That(ex.InnerException, Is.TypeOf<TimeoutException>());
        Assert.That(conn.State, Is.EqualTo(ConnectionState.Open));
    ...
    public async Task Clear_pool_one_host_only_on_admin_shutdown()
    {
        ...
        Assert.That(primaryConn.EDBDataSource.Statistics.Total, Is.EqualTo(3));
        ...
        Assert.That(standbyDataSource.GetDatabaseState(), Is.EqualTo(DatabaseState.Standby));
        Assert.That(primaryConn.EDBDataSource.Statistics.Total, Is.EqualTo(1));
    ...
    public async Task Primary_host_failover_can_connect()
    {
        ...
        var noHostFoundEx = Assert.ThrowsAsync<EDBException>(async () => await conn.OpenAsync())!;
    ...
    public void IntegrationTest([Values] bool loadBalancing, [Values] bool alwaysCheckHostState)
    {
        ...
        async Task Query(EDBDataSource dataSource)
        {
            await using var conn = dataSource.CreateConnection();
            for (var i = 0; i < 5; i++)
            {
    ...
    public async Task DataSource_with_wrappers()
    {
        ...
        var builder = new EDBDataSourceBuilder
        {
            ConnectionStringBuilder =
            {
    ...
    class DisposableWrapper : IAsyncDisposable
    {
        ...
        readonly IEnumerable<IDisposable> _disposablesSync;

        public DisposableWrapper(IEnumerable<IAsyncDisposable> disposables) => _disposables = disposables;

        public DisposableWrapper(IEnumerable<IDisposable> disposables) => _disposablesSync = disposables;
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            if (_disposablesSync is not null)
                foreach (var disposable in _disposablesSync)
                    disposable.Dispose();
        }
    }
#else
