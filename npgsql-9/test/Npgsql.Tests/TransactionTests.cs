...
public class TransactionTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task Commit([Values(PrepareOrNot.NotPrepared, PrepareOrNot.Prepared)] PrepareOrNot prepare)
    {
        ...
        await using (tx)
        {
            var cmd = new EDBCommand($"INSERT INTO {table} (name) VALUES ('X')", conn, tx);
        ...
        // With multiplexing we can't assume that disposed EDBTransaction will throw ObjectDisposedException
        // Because disposed EDBTransaction might be reused by another thread
        if (!IsMultiplexing)
    ...
    public async Task Failed_transaction_cannot_rollback_to_savepoint_with_custom_timeout()
    {
        ...
        using var cmd = new EDBCommand("SELECT unknown_thing", conn);
        cmd.CommandTimeout = 1;
        try
        {
            cmd.ExecuteScalar();
    ...
    public async Task Failed_transaction_on_close_with_custom_timeout()
    {
        ...
        using (var badCmd = new EDBCommand("SEL", conn))
        {
            badCmd.CommandTimeout = EDBCommand.DefaultTimeout + 1;
    ...
    public async Task Transaction_on_recycled_connection()
    {
        ...
        await using var conn = new EDBConnection(ConnectionString + $";Application Name={GetUniqueIdentifier(nameof(Transaction_on_recycled_connection))}");
        conn.Open();
    ...
    class NoTransactionDatabaseInfo : PostgresDatabaseInfo
    {
        ...
        internal NoTransactionDatabaseInfo(EDBConnector conn) : base(conn) {}
    }
    ...
    public void Bug184_Rollback_fails_on_aborted_transaction()
    {
        ...
        try {
            var command = new EDBCommand("select count(*) from dta", connTimeoutChanged, t);
