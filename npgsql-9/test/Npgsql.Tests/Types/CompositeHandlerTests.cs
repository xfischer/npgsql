...
{
    Task<EDBDataSource> OpenAndMapComposite<T>(T composite, string? schema, string nameSuffix, out string nameQualified)
        where T : IComposite
    {
        var nameTranslator = new EDBSnakeCaseNameTranslator();
        ...
        async Task<EDBDataSource> OpenAndMapCompositeCore(string nameQualified)
        {
