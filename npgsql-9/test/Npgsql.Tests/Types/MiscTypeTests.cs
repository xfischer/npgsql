...
class MiscTypeTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    {
        await AssertType(true, "true", "boolean", EDBDbType.Boolean, DbType.Boolean, skipArrayCheck: true);
        await AssertType(false, "false", "boolean", EDBDbType.Boolean, DbType.Boolean, skipArrayCheck: true);

        // The literal representations for bools inside array are different ({t,f} instead of true/false, so we check separately.
        await AssertType(new[] { true, false }, "{t,f}", "boolean[]", EDBDbType.Boolean | EDBDbType.Array);
    }
    ...
    public Task Uuid()
        => AssertType(
            ...
            "uuid", EDBDbType.Uuid, DbType.Guid);
    ...
    public async Task Read_unknown()
    {
        ...
        await using var cmd = new EDBCommand($"SELECT '{expected}'", conn);
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using (var cmd = new EDBCommand("SELECT @p1::TEXT, @p2::TEXT, @p3::TEXT", conn))
        {
            cmd.Parameters.AddWithValue("p1", DBNull.Value);
            cmd.Parameters.Add(new EDBParameter<string?>("p2", null));
            cmd.Parameters.Add(new EDBParameter<object>("p3", DBNull.Value));
        ...
        // Setting non-generic EDBParameter.Value to null is not allowed, only DBNull.Value
        await using (var cmd = new EDBCommand("SELECT @p4::TEXT", conn))
        {
        ...
        // Setting generic EDBParameter<object>.Value to null is not allowed, only DBNull.Value
        await using (var cmd = new EDBCommand("SELECT @p4::TEXT", conn))
        {
            cmd.Parameters.Add(new EDBParameter<object>("p4", EDBDbType.Text) { Value = null! });
            Assert.That(async () => await cmd.ExecuteReaderAsync(), Throws.Exception.TypeOf<InvalidOperationException>());
        }
    }
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT TRUE", conn);
    ...
    public async Task UnknownResultTypeList()
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT TRUE, 8", conn);
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT 8", conn);
        cmd.AllResultTypesAreUnknown = true;
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT @p::TEXT", conn);
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT @p::INT4", conn);
...
#if NETFRAMEWORK
        var unixEpoch = NetFrameworkShims.UnixEpoch();
#else
        var unixEpoch = DateTime.UnixEpoch;
#endif
        await AssertTypeWrite(dataSource, new object?[] { unixEpoch, null, DBNull.Value, unixEpoch.AddDays(1) }, "{\"1970-01-01 01:00:00+01\",NULL,NULL,\"1970-01-02 01:00:00+01\"}", "timestamp with time zone[]", EDBDbType.TimestampTz | EDBDbType.Array, isDefault: false);
        Assert.ThrowsAsync<ArgumentException>(() => AssertTypeWrite(dataSource, new object?[]
            {
                DateTime.Now, null, DBNull.Value, unixEpoch.AddDays(1)
            }, "{\"1970-01-01 01:00:00+01\",NULL,NULL,\"1970-01-02 01:00:00+01\"}", "timestamp with time zone[]",
            EDBDbType.TimestampTz | EDBDbType.Array, isDefault: false));
    }

    [Test]
    public Task Int2Vector()
        => AssertType(new short[] { 4, 5, 6 }, "4 5 6", "int2vector", EDBDbType.Int2Vector, isDefault: false);

    [Test]
    public Task Oidvector()
        => AssertType(new uint[] { 4, 5, 6 }, "4 5 6", "oidvector", EDBDbType.Oidvector, isDefault: false);
