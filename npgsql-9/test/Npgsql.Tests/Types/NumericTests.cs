...
public class NumericTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    {
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand("SELECT @p, @p = " + query, conn);
    ...
    {
        await AssertType(5.5m, "5.5", "numeric", EDBDbType.Numeric, DbType.Decimal);
        await AssertTypeWrite(5.5m, "5.5", "numeric", EDBDbType.Numeric, DbType.VarNumeric, inferredDbType: DbType.Decimal);

        await AssertType((short)8, "8", "numeric", EDBDbType.Numeric, DbType.Decimal, isDefault: false);
        await AssertType(8,        "8", "numeric", EDBDbType.Numeric, DbType.Decimal, isDefault: false);
        await AssertType((byte)8,  "8", "numeric", EDBDbType.Numeric, DbType.Decimal, isDefault: false);
        await AssertType(8F,       "8", "numeric", EDBDbType.Numeric, DbType.Decimal, isDefault: false);
        await AssertType(8D,       "8", "numeric", EDBDbType.Numeric, DbType.Decimal, isDefault: false);
        await AssertType(8M,       "8", "numeric", EDBDbType.Numeric, DbType.Decimal, isDefault: false);
    }
    ...
    public async Task Read_overflow_is_safe()
    {
        ...
        using var cmd = new EDBCommand(@"SELECT (0.20285714285714285714285714285)::numeric, generate_series FROM generate_series(1, 2)", conn);
    ...
    public async Task BigInteger_large()
    {
        ...
        using var conn = await OpenConnectionAsync();
        using var cmd = new EDBCommand("SELECT '0.1'::numeric, @p", conn);
    ...
    public async Task NumericZero_WithScale()
    {
        ...
        var param = new EDBParameter("p", DbType.Decimal, 10, null, ParameterDirection.Input, false, 10, 2, DataRowVersion.Default, 0.00M);
