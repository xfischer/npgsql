...
""");
    ...
    private static bool IsDoesNotExist(Exception exception)
        => exception switch
        {
            ...
            EDBException { InnerException: AggregateException ae } when ae.InnerExceptions.Any(ie => ie is PostgresException { SqlState: "3D000" })
                => true,

            // Pretty awful hack around #104
            EDBException { InnerException: IOException { InnerException: SocketException { SocketErrorCode: SocketError.ConnectionReset } } }
                => true,
    ...
    public override void CreateTables()
    {
        ...
        if (reloadTypes && connection.DbConnection is EDBConnection npgsqlConnection)
        {
    ...
    private void ClearPool()
        => EDBConnection.ClearPool((EDBConnection)connection.DbConnection);
}
