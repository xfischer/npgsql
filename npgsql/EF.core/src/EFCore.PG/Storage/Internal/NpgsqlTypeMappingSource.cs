...
public class NpgsqlTypeMappingSource : RelationalTypeMappingSource
{
    ...
    private readonly EDBCidrTypeMapping _cidrAsIPNetwork = EDBCidrTypeMapping.Default;
    private readonly EDBLegacyCidrTypeMapping _cidrAsNpgsqlCidr = EDBLegacyCidrTypeMapping.Default;

    // Built-in geometric types
    private readonly EDBPointTypeMapping _point = EDBPointTypeMapping.Default;
    private readonly EDBBoxTypeMapping _box = EDBBoxTypeMapping.Default;
    private readonly EDBLineTypeMapping _line = EDBLineTypeMapping.Default;
    private readonly EDBLineSegmentTypeMapping _lseg = EDBLineSegmentTypeMapping.Default;
    private readonly EDBPathTypeMapping _path = EDBPathTypeMapping.Default;
    private readonly EDBPolygonTypeMapping _polygon = EDBPolygonTypeMapping.Default;
    private readonly EDBCircleTypeMapping _circle = EDBCircleTypeMapping.Default;

    // uint/ulong mappings
    private readonly NpgsqlUIntTypeMapping _xid = new("xid", EDBDbType.Xid);
    private readonly NpgsqlULongTypeMapping _xid8 = new("xid8", EDBDbType.Xid8);
        ...
        : base(dependencies, relationalDependencies)
    {
        ...
        _int4range = NpgsqlRangeTypeMapping.CreatBuiltInRangeMapping(
            "int4range", typeof(EDBRange<int>), EDBDbType.IntegerRange, _int4);
        _int8range = NpgsqlRangeTypeMapping.CreatBuiltInRangeMapping(
            "int8range", typeof(EDBRange<long>), EDBDbType.BigIntRange, _int8);
        _numrange = NpgsqlRangeTypeMapping.CreatBuiltInRangeMapping(
            "numrange", typeof(EDBRange<decimal>), EDBDbType.NumericRange, _numeric);
        ...
        _dateTimeDaterange = NpgsqlRangeTypeMapping.CreatBuiltInRangeMapping(
            "daterange", typeof(EDBRange<DateTime>), EDBDbType.DateRange, _dateDateTime);
...
#pragma warning restore CS0618
    ...
    protected virtual RelationalTypeMapping? FindBaseMapping(in RelationalTypeMappingInfo mappingInfo)
    {
        ...
        if (storeTypeName is not null)
        {
            if (StoreTypeMappings.TryGetValue(storeTypeName, out var mappings))
            {
                ...
                {
                    return FindCollectionMapping(
                        mappingInfo,
                        mappingInfo.ClrType,
                        providerType: null,
                        elementMapping: mappingInfo.CoreTypeMappingInfo.ElementTypeMapping)
                        ?? new NpgsqlJsonTypeMapping(storeTypeName, clrType);
                }
        ...
        CoreTypeMapping? elementMapping)
    {
        ...
        switch (storeType)
        {
            case null:
            {
                ...
                if (_supportsMultiranges)
                {
                    if (relationalElementMapping is NpgsqlRangeTypeMapping rangeMapping)
                    {
                        ...
                        return new NpgsqlMultirangeTypeMapping(multirangeStoreType, modelClrType, rangeMapping);
                    }
                    ...
                    if (relationalElementMapping.GetType() is
                        { Name: "IntervalRangeMapping", Namespace: "EnterpriseDB.EDBClient.EntityFrameworkCore.PostgreSQL.Storage.Internal" } type1)
                    {
                        return (RelationalTypeMapping)Activator.CreateInstance(
                            type1.Assembly.GetType(
                                "EnterpriseDB.EDBClient.EntityFrameworkCore.PostgreSQL.Storage.Internal.IntervalMultirangeMapping")!,
                            modelClrType,
                            relationalElementMapping)!;
                    }

                    if (relationalElementMapping.GetType() is
                        { Name: "DateIntervalRangeMapping", Namespace: "EnterpriseDB.EDBClient.EntityFrameworkCore.PostgreSQL.Storage.Internal" } type2)
                    {
                        return (RelationalTypeMapping)Activator.CreateInstance(
                            type2.Assembly.GetType(
                                "EnterpriseDB.EDBClient.EntityFrameworkCore.PostgreSQL.Storage.Internal.DateIntervalMultirangeMapping")!,
                            modelClrType,
                            relationalElementMapping)!;
                    }
                }
                ...
                return (NpgsqlArrayTypeMapping)Activator.CreateInstance(
                    typeof(NpgsqlArrayTypeMapping<,,>).MakeGenericType(modelClrType, concreteCollectionType, elementType),
                    relationalElementMapping)!;
            }
            ...
            case var _ when IsMultirange(storeType, out var rangeStoreType) && _supportsMultiranges:
            {
                // Note that we ignore the elementMapping argument here (but not in the CLR type-only path above).
                // This is because the user-provided storeType for the array should take precedence over the element type mapping that gets
                // calculated purely based on the element's CLR type in base.FindMappingWithConversion.
                ...
                if (relationalElementMapping is NpgsqlRangeTypeMapping rangeMapping
                    ...
                    && (relationalElementMapping.Converter is null || modelClrType is null || modelClrType.IsArrayOrGenericList()))
                {
                    return new NpgsqlMultirangeTypeMapping(
                        storeType, modelClrType ?? typeof(List<>).MakeGenericType(relationalElementMapping.ClrType), rangeMapping);
                }
                    ...
                    { Name: "IntervalRangeMapping", Namespace: "EnterpriseDB.EDBClient.EntityFrameworkCore.PostgreSQL.Storage.Internal" } type1)
                {
                    return (RelationalTypeMapping)Activator.CreateInstance(
                        type1.Assembly.GetType(
                            "EnterpriseDB.EDBClient.EntityFrameworkCore.PostgreSQL.Storage.Internal.IntervalMultirangeMapping")!,
                        modelClrType ?? relationalElementMapping.ClrType.MakeArrayType(),
                        relationalElementMapping)!;
                }
...
#pragma warning disable EF1001 // SelectExpression constructors are pubternal
            // This is a scalar collection mapped to JSON, which - unlike with other relational providers - is not the default: in PG we
            // map to PG arrays instead.
...
#pragma warning restore EF1001 // SelectExpression constructors are pubternal
    ...
    protected virtual RelationalTypeMapping? FindUserRangeMapping(in RelationalTypeMappingInfo mappingInfo)
    {
        ...
        if (rangeClrType is not null && (!rangeClrType.IsGenericType || rangeClrType.GetGenericTypeDefinition() != typeof(EDBRange<>)))
        {
            return null;
        }
    ...
    public override RelationalTypeMapping? FindMapping(IProperty property)
    {
        ...
        if (typeMapping is NpgsqlArrayTypeMapping arrayMapping
            && property.DeclaringType.IsMappedToJson())
        {
            return FindCollectionMapping(
                // TODO: Unfortunately GetContainerColumnType seems to return null at this point
                property.DeclaringType.GetContainerColumnType() ?? "jsonb",
                property.ClrType,
