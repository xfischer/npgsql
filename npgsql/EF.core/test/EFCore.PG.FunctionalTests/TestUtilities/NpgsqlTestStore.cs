...
public class NpgsqlTestStore : RelationalTestStore
{
    ...
    private static EDBConnection CreateConnection(string name, string? connectionStringOptions)
    ...
    {
        await using var master = new EDBConnection(CreateAdminConnectionString());
        ...
        await WaitForExistsAsync((EDBConnection)Connection);

        return true;
    }

    private static async Task WaitForExistsAsync(EDBConnection connection)
    {
        var retryCount = 0;
        while (true)
        {
