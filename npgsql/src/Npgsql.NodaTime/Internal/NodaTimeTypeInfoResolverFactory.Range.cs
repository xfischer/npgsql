...
sealed partial class NodaTimeTypeInfoResolverFactory
{
    ...
    class RangeResolver : IPgTypeInfoResolver
    {
        ...
        static TypeInfoMappingCollection AddMappings(TypeInfoMappingCollection mappings)
        {
            ...
            mappings.AddStructType<EDBRange<ZonedDateTime>>(TimestampTzRangeDataTypeName,
                static (options, mapping, _) => mapping.CreateInfo(options,
                    CreateRangeConverter(new ZonedDateTimeConverter(options.EnableDateTimeInfinityConversions), options)));
            mappings.AddStructType<EDBRange<OffsetDateTime>>(TimestampTzRangeDataTypeName,
                static (options, mapping, _) => mapping.CreateInfo(options,
                    CreateRangeConverter(new OffsetDateTimeConverter(options.EnableDateTimeInfinityConversions), options)));

            // tsrange
            mappings.AddStructType<EDBRange<LocalDateTime>>(TimestampRangeDataTypeName,
                static (options, mapping, _) =>
                    mapping.CreateInfo(options,
            ...
            mappings.AddStructType<EDBRange<LocalDate>>(DateRangeDataTypeName,
                static (options, mapping, _) => mapping.CreateInfo(options,
                    CreateRangeConverter(new LocalDateConverter(options.EnableDateTimeInfinityConversions), options)));
    ...
    sealed class RangeArrayResolver : RangeResolver, IPgTypeInfoResolver
    {
        ...
        static TypeInfoMappingCollection AddMappings(TypeInfoMappingCollection mappings)
        {
            ...
            mappings.AddStructArrayType<EDBRange<Instant>>(TimestampTzRangeDataTypeName);
            mappings.AddStructArrayType<EDBRange<ZonedDateTime>>(TimestampTzRangeDataTypeName);
            mappings.AddStructArrayType<EDBRange<OffsetDateTime>>(TimestampTzRangeDataTypeName);

            // tsrange
            mappings.AddStructArrayType<EDBRange<LocalDateTime>>(TimestampRangeDataTypeName);

            // daterange
            mappings.AddArrayType<DateInterval>(DateRangeDataTypeName);
            mappings.AddStructArrayType<EDBRange<LocalDate>>(DateRangeDataTypeName);
