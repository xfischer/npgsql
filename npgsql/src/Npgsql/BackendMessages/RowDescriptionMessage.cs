...
sealed class RowDescriptionMessage : IBackendMessage
{
    ...
    internal RowDescriptionMessage Load(EDBReadBuffer buf, PgSerializerOptions options, bool isOutDescription /* EnterpriseDB Team */)
    {
        ...
        else /* EnterpriseDB Team */
        {
            for (var i = 0; i != numFields; ++i)
            {
                ...
                if (!_nameIndex.ContainsKey(field.Name))
                {
                    ...
                    if (!_insensitiveIndex.ContainsKey(field.Name))
                    {
                        _insensitiveIndex.Add(field.Name, i);
                    }
                }
            }
        }
...
#pragma warning restore CS8618
    ...
    internal void PopulateCallableStmt(
       PgSerializerOptions serializerOptions, string name, short retIndex, uint typoid,
        short typeSize, int typeModifier, DataFormat dataFormat
   )
    {
        _serializerOptions = serializerOptions;
        Name = name;
