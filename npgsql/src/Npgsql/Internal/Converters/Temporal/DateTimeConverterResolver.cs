...
sealed class DateTimeConverterResolver<T> : PgConverterResolver<T>
{
    ...
    public PgConverterResolution? Get(DateTime value, PgTypeId? expectedPgTypeId, bool validateOnly = false)
    {
        if (value.Kind is DateTimeKind.Utc)
        {
            ...
            return expectedPgTypeId == _timestamp
                ? throw new ArgumentException(
                    string.Format(EDBStrings.TimestampNoDateTimeUtc, _options.GetDataTypeName(_timestamp).DisplayName, _options.GetDataTypeName(_timestampTz).DisplayName), nameof(value))
        ...
        {
            throw new ArgumentException(
                string.Format(EDBStrings.TimestampTzNoDateTimeUnspecified, value.Kind, _options.GetDataTypeName(_timestampTz).DisplayName), nameof(value));
        }
...
sealed class DateTimeConverterResolver
{
    ...
    public static DateTimeConverterResolver<EDBRange<DateTime>> CreateRangeResolver(PgSerializerOptions options, PgTypeId timestampTz, PgTypeId timestamp, bool dateTimeInfinityConversions)
    ...
    {
        if (typeof(TElement) != typeof(EDBRange<DateTime>))
        ...
        return new DateTimeConverterResolver<T>(options, static (resolver, value, expectedPgTypeId) =>
        {
            ...
            foreach (var element in (IList<EDBRange<DateTime>>)value)
            {
