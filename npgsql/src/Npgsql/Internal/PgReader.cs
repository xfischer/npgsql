...
public class PgReader
{
    ...
    byte[]? _pooledArray;
    EDBReadBuffer.ColumnStream? _userActiveStream;
    ...
    internal PgReader(EDBReadBuffer buffer)
    {
        _buffer = buffer;
    ...
    // TODO refactor out
    internal long GetFieldStartPos(EDBNestedDataReader nestedDataReader) => _fieldStartPos;
    // TODO refactor out
    internal int GetFieldOffset(EDBNestedDataReader nestedDataReader) => FieldOffset;
    ...
    {
        if (EDBReadBuffer.BufferBoundsChecks)
            Core(count);
    ...
    EDBReadBuffer.ColumnStream GetColumnStream(bool canSeek = false, int? length = null)
    {
