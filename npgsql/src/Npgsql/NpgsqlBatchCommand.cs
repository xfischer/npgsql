...
public sealed class EDBBatchCommand : DbBatchCommand
{
    ...
    /// <inheritdoc/>
    public override EDBParameter CreateParameter()
#else
    /// <summary>
...
#else
    /// <summary>
    /// Returns whether the <see cref="EDBBatchCommand.CreateParameter"/> method is implemented.
    /// </summary>
    ...
    /// <summary>
    /// The SQL as it will be sent to PostgreSQL, after any rewriting performed by EDB (e.g. named to positional parameter
    /// placeholders).
    /// </summary>
    ...
    /// backing <see cref="EDBCommand.Parameters" /> (in non-batching) mode. If the user provided named parameters, this is a
    ...
    internal List<EDBParameter> CurrentParametersReadOnly => HasParameters ? PositionalParameters : EmptyParameters;

    List<EDBParameter>? _ownedInputParameters;
    List<EDBParameter>? _inputParameters;
    ...
    /// <summary>
    /// Initializes a new <see cref="EDBBatchCommand"/>.
    /// </summary>
    public EDBBatchCommand() : this(string.Empty) {}

    /// <summary>
    /// Initializes a new <see cref="EDBBatchCommand"/>.
    /// </summary>
    /// <param name="commandText">The text of the <see cref="EDBBatchCommand"/>.</param>
    public EDBBatchCommand(string commandText)
        => _commandText = commandText;

    internal bool ExplicitPrepare(EDBConnector connector)
    {
        if (!IsPrepared)
        {
    ...
    internal bool TryAutoPrepare(EDBConnector connector)
    {
    ...
    internal void Reset()
    {
        ...
        if (ReferenceEquals(_inputParameters, _ownedInputParameters))
            ...
            _inputParameters = null; // We're pointing at a user's EDBParameterCollection
    ...
    internal void PopulateOutputParameters(EDBDataReader reader, ILogger logger)
    {
        ...
        case PlaceholderType.Named:
        {
            ...
            for (var ordinal = 0; ordinal < fieldCount; ordinal++)
            {
                ...
                if (i is not -1 && parameters[i] is { IsOutputDirection: true } parameter)
                {
                    ...
                    parameterIndices.Array![ordinal] = i;
                }
                else
                {
                    parameterIndices.Array![ordinal] = -1;
        ...
        static void SetValue(EDBDataReader reader, ILogger logger, EDBParameter p, int ordinal, int index)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                logger.LogDebug(ex, "Failed to set value on output parameter instance '{ParameterNameOrIndex}' for output parameter {OutputName}",
                    p.ParameterName is EDBParameter.PositionalName ? index : p.ParameterName, reader.GetName(ordinal));
                throw;
            }
        }
    }
