...
public sealed class EDBBinaryExporter : ICancelable
{
    ...
    async Task ReadHeader(bool async)
    {
        ...
        foreach (var t in EDBRawCopyStream.BinarySignature)
            if (_buf.ReadByte() != t)
    ...
    /// <returns>The value of the column</returns>
    public T Read<T>(EDBDbType type)
        => Read<T>((EDBDbType?)type);
    ...
    public ValueTask<T> ReadAsync<T>(EDBDbType type, CancellationToken cancellationToken = default)
        => ReadAsync<T>((EDBDbType?)type, cancellationToken);

    T Read<T>(EDBDbType? type)
    {
        ThrowIfNotOnRow();
    ...
    async ValueTask<T> ReadAsync<T>(EDBDbType? type, CancellationToken cancellationToken)
    {
        ThrowIfNotOnRow();
    ...
    PgConverterInfo GetInfo(Type type, EDBDbType? npgsqlDbType, out bool asObject)
    {
    ...
    PgConverterInfo CreateConverterInfo(Type type, EDBDbType? npgsqlDbType = null)
    {
