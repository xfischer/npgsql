...
public sealed class EDBConnection : DbConnection, ICloneable, IComponent
{
    #region Fields
    ...
    /// </summary>
    internal EDBConnectionStringBuilder Settings { get; private set; } = DefaultSettings;

    static readonly EDBConnectionStringBuilder DefaultSettings = new();

    EDBDataSource? _dataSource;

    internal EDBDataSource EDBDataSource
    {
        get
        {
    ...
    public static IEDBTypeMapper GlobalTypeMapper => TypeMapping.GlobalTypeMapper.Instance;

    /// <summary>
    /// Connection-level type mapping is no longer supported. See the 7.0 release notes for configuring type mapping on EDBDataSource.
    /// </summary>
    [Obsolete("Connection-level type mapping is no longer supported. See the 7.0 release notes for configuring type mapping on EDBDataSource.", true)]
    public IEDBTypeMapper TypeMapper
    ...
    internal EDBConnection(EDBDataSource dataSource, EDBConnector connector) : this()
    {
    ...
    internal static EDBConnection FromDataSource(EDBDataSource dataSource)
        => new()
        {
            _dataSource = dataSource,
            Settings = dataSource.Settings,
    ...
    void SetupDataSource()
    {
        ...
        var settings = new EDBConnectionStringBuilder(_connectionString);
        ...
        if (PoolManager.Pools.TryGetValue(canonical, out _dataSource))
        {
            ...
            if (_dataSource is EDBMultiHostDataSource multiHostDataSource && settings.TargetSessionAttributesParsed.HasValue)
        ...
        var dataSourceBuilder = new EDBDataSourceBuilder(canonical);
        dataSourceBuilder.UseLoggerFactory(EDBLoggingConfiguration.GlobalLoggerFactory);
        dataSourceBuilder.EnableParameterLogging(EDBLoggingConfiguration.GlobalIsParameterLoggingEnabled);
        ...
        // If this is a multi-host data source and the user specified a TargetSessionAttributes, create a wrapper in front of the
        // MultiHostDataSource with that TargetSessionAttributes.
        if (_dataSource is EDBMultiHostDataSource multiHostDataSource2 && settings.TargetSessionAttributesParsed.HasValue)
    ...
    internal Task Open(bool async, CancellationToken cancellationToken)
    {
        ...
        async Task OpenAsync(bool async, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var timeout = new EDBTimeout(connectionTimeout);
    ...
    /// </p>
    /// <p>
    /// The <see cref="EDBConnectionStringBuilder.Password"/> and <see cref="EDBConnectionStringBuilder.Passfile"/> connection
    ...
    /// </p>
    /// </remarks>
    [Obsolete("Use EDBDataSourceBuilder.UsePeriodicPasswordProvider or inject passwords directly into EDBDataSource.Password")]
    ...
    public ConnectionState FullState
    {
        ...
        get
        {
            ...
            case ConnectorState.Closed:
                ThrowHelper.ThrowInvalidOperationException("Internal EnterpriseDB.EDBClient bug: connection is in state Open but connector is in state Closed");
                return ConnectionState.Broken;
            default:
                ThrowHelper.ThrowInvalidOperationException($"Internal EnterpriseDB.EDBClient bug: unexpected value {{0}} of enum {nameof(ConnectorState)}. Please file a bug.", Connector.State);
                return ConnectionState.Broken;
            }
        }
        internal set
    ...
    /// </summary>
    internal EDBCommand? CachedCommand { get; set; }
    ...
    /// <summary>
    /// Creates and returns a <see cref="EDBCommand"/> object associated with the <see cref="EDBConnection"/>.
    /// </summary>
    ...
    public new EDBCommand CreateCommand()
    {
        ...
        return EDBCommand.CreateCachedCommand(this);
    }
    ...
    public new EDBBatch CreateBatch()
    {
        ...
        return EDBBatch.CreateCachedBatch(this);
    }
#else
    /// <summary>
    /// Creates and returns a <see cref="EDBBatch"/> object associated with the <see cref="EDBConnection"/>.
    /// </summary>
    /// <returns>A <see cref="EDBBatch"/> object.</returns>
    public EDBBatch CreateBatch() => new(this);
#endif
    ...
    /// </summary>
    /// <returns>A <see cref="EDBTransaction"/> object representing the new transaction.</returns>
    /// <remarks>
    ...
    async ValueTask<EDBTransaction> BeginTransaction(bool async, IsolationLevel level, CancellationToken cancellationToken)
    {
        ...
        var connector = ConnectorBindingScope == ConnectorBindingScope.Transaction
            ? Connector
            : await StartBindingScope(ConnectorBindingScope.Transaction, EDBTimeout.Infinite, async, cancellationToken).ConfigureAwait(false);
        ...
        try
        {
            ...
            connector.Transaction ??= new EDBTransaction(connector);
    ...
    internal void OnNotice(PostgresNotice e)
    {
        try
        {
            Notice?.Invoke(this, new EDBNoticeEventArgs(e));
        }
        catch (Exception ex)
        {
    ...
    internal void OnNotification(EDBNotificationEventArgs e)
    {
        try
        {
    ...
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public Task<EDBBinaryImporter> BeginBinaryImportAsync(string copyFromCommand, CancellationToken cancellationToken = default)
        => BeginBinaryImport(async: true, copyFromCommand, cancellationToken);

    async Task<EDBBinaryImporter> BeginBinaryImport(bool async, string copyFromCommand, CancellationToken cancellationToken = default)
    {
    ...
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public Task<EDBBinaryExporter> BeginBinaryExportAsync(string copyToCommand, CancellationToken cancellationToken = default)
        => BeginBinaryExport(async: true, copyToCommand, cancellationToken);

    async Task<EDBBinaryExporter> BeginBinaryExport(bool async, string copyToCommand, CancellationToken cancellationToken = default)
    {
    ...
    /// See https://www.postgresql.org/docs/current/static/sql-copy.html.
    /// </remarks>
    public Task<EDBRawCopyStream> BeginRawBinaryCopyAsync(string copyCommand, CancellationToken cancellationToken = default)
        => BeginRawBinaryCopy(async: true, copyCommand, cancellationToken);

    async Task<EDBRawCopyStream> BeginRawBinaryCopy(bool async, string copyCommand, CancellationToken cancellationToken = default)
    {
        if (copyCommand == null)
    ...
    {
        if (_disposed)
            ThrowHelper.ThrowObjectDisposedException(nameof(EDBConnection));
    }
    ...
    /// </summary>
    internal bool TryGetBoundConnector([NotNullWhen(true)] out EDBConnector? connector)
    {
        if (ConnectorBindingScope == ConnectorBindingScope.None)
        {
    ...
    /// </summary>
    internal ValueTask<EDBConnector> StartBindingScope(
        ConnectorBindingScope scope, EDBTimeout timeout, bool async, CancellationToken cancellationToken)
    {
        ...
        async ValueTask<EDBConnector> StartBindingScopeAsync()
        {
            try
            {
                Debug.Assert(Settings.Multiplexing);
    ...
    internal EDBConnector StartBindingScope(ConnectorBindingScope scope)
        => StartBindingScope(scope, EDBTimeout.Infinite, async: false, CancellationToken.None)
            .GetAwaiter().GetResult();

    internal EndScopeDisposable StartTemporaryBindingScope(out EDBConnector connector)
    {
        connector = StartBindingScope(ConnectorBindingScope.Temporary);
        return new EndScopeDisposable(this);
    }

    internal T CheckOpenAndRunInTemporaryScope<T>(Func<EDBConnector, T> func)
    {
        CheckOpen();
    ...
    public override DataTable GetSchema(string? collectionName, string?[]? restrictions)
        => EDBSchema.GetSchema(async: false, this, collectionName, restrictions).GetAwaiter().GetResult();
...
#endif
    {
     	return EDBSchema.GetSchema(async: true, this, collectionName, restrictions, cancellationToken);
    }
    ...
    object ICloneable.Clone()
    {
        ...
        // `new EDBConnection(_connectionString)`, since that would bring in the default data source builder, and with it various
...
#pragma warning restore CS0618
    ...
    /// <summary>
    /// This event is unsupported by EnterpriseDB.EDBClient. Use <see cref="DbConnection.StateChange"/> instead.
    /// </summary>
