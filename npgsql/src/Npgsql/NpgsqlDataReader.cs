...
#pragma warning restore CA1010
{
    ...
    /// </summary>
    List<EDBBatchCommand> _statements = default!;
    ...
    int ColumnCount => RowDescription?.Count ?? 0;  // EnterpriseDB (defaults to 0)
    ...
    internal EDBNestedDataReader? CachedFreeNestedDataReader;
    ...
    internal void Init(
        ...
        List<EDBBatchCommand> statements,
    ...
    ValueTask<IBackendMessage> ReadMessage(bool async)
    {
        ...
        static async ValueTask<IBackendMessage> ReadMessageSequential(EDBConnector connector, bool async)
        {
    ...
    async Task<bool> NextResult(bool async, bool isConsuming = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _hasRows = false;
            _isInMemoryReader = false; // EnterpriseDB Team
            ...
            for (statementIndex = ++StatementIndex; statementIndex < statements.Count; statementIndex = ++StatementIndex)
            {
                ...
                else // Non-prepared/preparing flow
                {
                    ...
                    Expect<ParseCompleteMessage>(await Connector.ReadMessage(async, checkDataAvailable: true).ConfigureAwait(false), Connector); // EnterpriseDB (additional param)
                    ...
                    RowDescription = statement.Description = msg.Code switch
                    {
                        ...
                        BackendMessageCode.RowDescription or BackendMessageCode.OutDescription => preparedStatement == null // EnterpriseDB (added OutDescription case)
                ...
                if (Command.CommandType != CommandType.StoredProcedure) // EnterpriseDB ZK: Connector._isCallableStmt != true
                {
                    ...
                    else
                    {
                        ...
                        case BackendMessageCode.CopyInResponse:
                            throw Connector.Break(new NotSupportedException(
                                "COPY isn't supported in regular command execution - see https://www.npgsql.org/doc/copy.html for documentation on COPY with EnterpriseDB.EDBClient. " +
                ...
                //
                var hasOutParameters = Command._parameters?.HasOutputParameters ?? false;
                var hasReturnParam = Command._parameters?._hasReturnParam ?? false;
                if (Command.WrappingBatch is null && StatementIndex == 0 && (hasOutParameters || hasReturnParam))
                {
                ...
                else if (Command.CommandType == CommandType.StoredProcedure && Command.Parameters.Count > 0) // EnterpriseDB ZK :Connector._isCallableStmt == true
                {
                    msg = await Connector.ReadMessage(async).ConfigureAwait(false);
                ...
                {
                    msg = await ReadMessage(async).ConfigureAwait(false); // EnterpriseDB ZK: Connector._isCallableStmt == true
                ...
                if (msg.Code == BackendMessageCode.DataRow) Connector.State = ConnectorState.Fetching;
                ...
                if (RowDescription == null && Command.CommandType != CommandType.StoredProcedure) //ZK :Connector._isCallableStmt != true
                {
                    ...
                    switch (msg.Code)
                    {
                    // EnterpriseDB state changed to Fetching above, not returning anymore
                    //case BackendMessageCode.DataRow:
                ...
                case BackendMessageCode.OutDescription:
                    ...
                    return true;
                case BackendMessageCode.DataRow: // EnterpriseDB
                    break;
                case BackendMessageCode.CommandComplete:
                    if (statement.AppendErrorBarrier ?? Command.EnableErrorBarriers)
                        Expect<ReadyForQueryMessage>(await Connector.ReadMessage(async).ConfigureAwait(false), Connector);
    ...
    async Task PopulateOutputParameters(bool async, EDBParameterCollection parameters) // EnterpriseDB (renamed)
    {
        ...
        while (done != true)
        {
            ...
            if (msg.Code == BackendMessageCode.CommandComplete && paramdata != true)
                continue;
            else
                switch (msg.Code)
                {
    ...
    /// a statement-by-statement basis, unlike <see cref="EDBDataReader.RecordsAffected"/>
    /// which exposes an aggregation across all statements.
    /// </remarks>
    [Obsolete("Use the new DbBatch API")]
    public IReadOnlyList<EDBBatchCommand> Statements
    {
        get
        {
            ThrowIfClosedOrDisposed();
    ...
    async Task Consume(bool async, Exception? firstException = null)
    {
        ...
        while (Connector.IsConnected)
        {
            try
            {
                if (Command.CommandType != CommandType.StoredProcedure) // EnterpriseDB (check and else part added)
                {
                    if (!(_isSchemaOnly
                ...
                else /*EnterpriseDB Team */
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            var msg = await Connector.SkipUntil(BackendMessageCode.CommandComplete, BackendMessageCode.ReadyForQuery, async).ConfigureAwait(false);
                            switch (msg.Code)
                            {
                            case BackendMessageCode.CommandComplete:
                                ProcessMessage(msg);
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (!(ex is PostgresException ||
                  ex is EDBException { InnerException: AggregateException aggregateException } &&
    ...
    internal async Task Cleanup(bool async, bool connectionClosing = false, bool isDisposing = false)
    {
        ...
        EDBEventSource.Log.CommandStop();
    ...
    /// The column type must be a record or a to EDB known composite type, or an array thereof.
    /// Currently only supported in non-sequential mode.
    /// </summary>
    /// <param name="ordinal">The zero-based column ordinal.</param>
    /// <returns>A data reader.</returns>
    public new EDBNestedDataReader GetData(int ordinal)
    {
        ThrowIfNotInResult();
        ...
        {
            reader = new EDBNestedDataReader(this, null, 1, compositeType);
        }
        if (isArray)
            reader.InitArray();
        else
    ...
    public object GetEDBValue(int ordinal) // // EnterpriseDB Team, used only from PopulateOutputParameters
    {
        ...
        if (Command.CommandType == CommandType.StoredProcedure)
            columnLength = SeekToColumnStart(ordinal, false);
        ...
        catch
        {
            ...
            {
                var writtenBytes = Buffer.ReadPosition;
                var remainingBytes = columnLength - writtenBytes;
                if (remainingBytes > 0)
                    Buffer.Skip(remainingBytes, false);
            }
            throw;
        }
    ...
    {
        if (_isRowBuffered || _isInMemoryReader) // EnterpriseDB _isInMemoryReader
    ...
    [UnconditionalSuppressMessage("ILLink", "IL2093",
        Justification = "Members are only dynamically accessed by EnterpriseDB.EDBClient via GetFieldType by GetSchema, and only in certain cases. " +
    ...
    async Task<DataTable?> GetSchemaTable(bool async, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var column in await GetColumnSchema<EDBDbColumn>(async, cancellationToken).ConfigureAwait(false))
        {
            ...
            row["ProviderType"] = column.EDBDbType ?? EDBDbType.Unknown;
    ...
    static void ThrowInvalidState(ReaderState state)
    {
        ...
        case ReaderState.Closed:
            ...
            break;
        case ReaderState.Disposed:
            ThrowHelper.ThrowObjectDisposedException(nameof(EDBDataReader));
            break;
        default:
            ThrowHelper.ThrowInvalidOperationException("No resultset is currently being traversed");
            break;
    ...
    internal void MutateToInMemoryReader(EDBCommand command)
    {
        ...
        paramsByName = new(capacity: command.Parameters.InternalList.Count);

        var i = 0;
        foreach (var param in command.Parameters.InternalList)
        {
