...
/// </summary>
[Obsolete("EDBLargeObjectManager allows manipulating PostgreSQL large objects via publicly available PostgreSQL functions (lo_read, lo_write); call these yourself directly.")]
public class EDBLargeObjectManager
{
    const int InvWrite = 0x00020000;
    const int InvRead = 0x00040000;

    internal EDBConnection Connection { get; }
    ...
    internal async Task<T> ExecuteFunction<T>(bool async, string function, CancellationToken cancellationToken, params object[] arguments)
    {
        ...
        for (var i = 0; i < arguments.Length; i++)
        {
            ...
            command.Parameters.Add(new EDBParameter { Value = arguments[i] });
        }
    ...
    /// </param>
    /// <returns>An EDBLargeObjectStream</returns>
    public Task<EDBLargeObjectStream> OpenReadAsync(uint oid, CancellationToken cancellationToken = default)
        => OpenRead(async: true, oid, cancellationToken);

    async Task<EDBLargeObjectStream> OpenRead(bool async, uint oid, CancellationToken cancellationToken = default)
    {
    ...
    /// </param>
    /// <returns>An EDBLargeObjectStream</returns>
    public Task<EDBLargeObjectStream> OpenReadWriteAsync(uint oid, CancellationToken cancellationToken = default)
        => OpenReadWrite(async: true, oid, cancellationToken);

    async Task<EDBLargeObjectStream> OpenReadWrite(bool async, uint oid, CancellationToken cancellationToken = default)
    {
