...
/// </summary>
[Obsolete("EDBLargeObjectStream allows manipulating PostgreSQL large objects via publicly available PostgreSQL functions (lo_read, lo_write); call these yourself directly.")]
public sealed class EDBLargeObjectStream : Stream
{
    readonly EDBLargeObjectManager _manager;
    readonly int _fd;
    long _pos;
    readonly bool _writeable;
    bool _disposed;

    internal EDBLargeObjectStream(EDBLargeObjectManager manager, int fd, bool writeable)
    {
        _manager = manager;
        _fd = fd;
        _pos = 0;
