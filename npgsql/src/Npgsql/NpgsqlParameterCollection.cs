...
public sealed class EDBParameterCollection : DbParameterCollection, IList<EDBParameter>
{
    ...
    internal void ProcessParameters(PgSerializerOptions options, bool validateValues, CommandType commandType)
    {
        ...
        for (var i = 0; i < list.Count; i++)
        {
            ...
            {
                LogMessages.TryEDBTrace(null, $"ResolveTypeInfo Error: {e.Message}");
            ...
            if (validateValues && p.TypeInfo is not null) // EnterpriseDB : add typeinfo check
            {
                p.Bind(out _, out _);
            }
        }
    ...
    {
        var castedValue = value as EDBParameter;
