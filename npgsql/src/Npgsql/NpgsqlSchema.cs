...
{
    public static Task<DataTable> GetSchema(bool async, EDBConnection conn, string? collectionName, string?[]? restrictions, CancellationToken cancellationToken = default)
    {
    ...
    static Task<DataTable> GetDatabases(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetSchemata(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetTables(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetColumns(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetViews(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetMaterializedViews(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
    ...
    static Task<DataTable> GetUsers(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
    ...
    static Task<DataTable> GetIndexes(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetIndexColumns(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
...
""");
    ...
    static Task<DataTable> GetConstraints(EDBConnection conn, string?[]? restrictions, string? constraintType, bool async, CancellationToken cancellationToken = default)
    {
        var sql = new StringBuilder(
            """
SELECT
...
""");
    ...
    static Task<DataTable> GetConstraintColumns(EDBConnection conn, string?[]? restrictions, bool async, CancellationToken cancellationToken = default)
    {
        var sql = new StringBuilder(
            """
SELECT current_database(),
...
""");
    ...
    static DataTable GetDataTypes(EDBConnection conn)
    {
        ...
        try
        {
            ...
            foreach (var baseType in types)
            {
                ...
                if (baseType.DataTypeName.ToEDBDbType() is { } npgsqlDbType)
            ...
            foreach (var arrayType in connector.DatabaseInfo.ArrayTypes)
            {
                ...
                row["CreateFormat"] += "[]";
                row["DataType"] = info.Type.FullName;
                if (arrayType.DataTypeName.ToEDBDbType() is { } npgsqlDbType)
                    row["ProviderDbType"] = (int)npgsqlDbType;
            }
            ...
            foreach (var rangeType in connector.DatabaseInfo.RangeTypes)
            {
                ...
                row["DataType"] = info.Type.FullName;
                if (rangeType.DataTypeName.ToEDBDbType() is { } npgsqlDbType)
                    row["ProviderDbType"] = (int)npgsqlDbType;
            }
            ...
            foreach (var multirangeType in connector.DatabaseInfo.MultirangeTypes)
            {
                ...
                row["DataType"] = info.Type.FullName;
                if (multirangeType.DataTypeName.ToEDBDbType() is { } npgsqlDbType)
                    row["ProviderDbType"] = (int)npgsqlDbType;
            }
            ...
            foreach (var domainType in connector.DatabaseInfo.DomainTypes)
            {
                ...
                row["DataType"] = info.Type.FullName;
                if (representationalType.DataTypeName.ToEDBDbType() is { } npgsqlDbType)
                    row["ProviderDbType"] = (int)npgsqlDbType;
            }
        }
        finally
    ...
    static async Task<DataTable> ParseResults(bool async, EDBCommand command, DataTable dataTable, Action<EDBDataReader, DataRow> populateRow, CancellationToken cancellationToken)
    {
    ...
    static object GetFieldValueOrDBNull<T>(EDBDataReader reader, int ordinal)
