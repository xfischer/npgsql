...
/// </remarks>
public readonly struct EDBCube : IEquatable<EDBCube>
{
    ...
    /// </exception>
    internal EDBCube(double[] lowerLeft, double[] upperRight)
    {
    ...
    public EDBCube(double lowerLeft, double upperRight)
    {
    ...
    public EDBCube(IEnumerable<double> coords)
    {
    ...
    /// </exception>
    public EDBCube(IEnumerable<double> lowerLeft, IEnumerable<double> upperRight) :
    ...
    public EDBCube(EDBCube cube, double coord)
    {
    ...
    /// <param name="lowerLeft">The lower left value.</param>
    /// <param name="upperRight">The upper right value.</param>
    public EDBCube(EDBCube cube, double lowerLeft, double upperRight)
    {
    ...
    /// <example>
    /// <code>
    /// var cube = new EDBCube(new[] { 1, 3, 5 }, new[] { 6, 7, 8 }); // '(1,3,5),(6,7,8)'
    /// cube.ToSubset(1); // '(3),(7)'
    /// cube.ToSubset(2, 1, 0, 0); // '(5,3,1,1),(8,7,6,6)'
    /// </code>
    /// </example>
    public EDBCube ToSubset(params int[] indexes)
    {
    ...
    /// <inheritdoc />
    public bool Equals(EDBCube other) => Dimensions == other.Dimensions
                                            && _lowerLeft.SequenceEqual(other._lowerLeft)
                                            && _upperRight.SequenceEqual(other._upperRight);

    /// <inheritdoc />
    public override bool Equals(object? obj) => obj is EDBCube other && Equals(other);

    /// <inheritdoc cref="IEquatable{T}" />
    public static bool operator ==(EDBCube x, EDBCube y) => x.Equals(y);

    /// <inheritdoc cref="IEquatable{T}" />
    public static bool operator !=(EDBCube x, EDBCube y) => !(x == y);
...
#else
            leftBuilder.Append(string.Format(CultureInfo.InvariantCulture, $"{_lowerLeft[i]:G17}"));
            rightBuilder.Append(string.Format(CultureInfo.InvariantCulture, $"{_upperRight[i]:G17}"));
#endif
