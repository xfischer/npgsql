...
/// </summary>
public sealed class EDBTsVector : IEnumerable<EDBTsVector.Lexeme>, IEquatable<EDBTsVector>
{
    /// <summary>
    /// Represents an empty tsvector.
    /// </summary>
    public static readonly EDBTsVector Empty = new EDBTsVector([], noCheck: true);

    readonly List<Lexeme> _lexemes;

    internal EDBTsVector(List<Lexeme> lexemes, bool noCheck = false)
    {
        if (noCheck)
        {
            _lexemes = lexemes;
    ...
    /// <returns></returns>
    [Obsolete("Client-side parsing of EDBTsVector is unreliable and cannot fully duplicate the PostgreSQL logic. Use PG functions instead (e.g. to_tsvector)")]
    public static EDBTsVector Parse(string value)
    {
        ...
        Finish:
        return new EDBTsVector(lexemes);
    }
    ...
    /// <inheritdoc />
    public bool Equals(EDBTsVector? other)
    {
    ...
    public override bool Equals(object? obj)
        => obj is EDBTsVector other && Equals(other);
