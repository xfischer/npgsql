...
static class PregeneratedMessages
{
    static PregeneratedMessages()
    {
        ...
        using var buf = new EDBWriteBuffer(null, new MemoryStream(), null, EDBWriteBuffer.MinimumSize, Encoding.ASCII);
    ...
    internal static byte[] Generate(EDBWriteBuffer buf, string query)
    {
        EDBWriteBuffer.AssertASCIIOnly(query);
