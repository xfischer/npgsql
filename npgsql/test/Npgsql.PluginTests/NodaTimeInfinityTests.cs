...
public class NodaTimeInfinityTests : TestBase, IDisposable
{
    ...
    public async Task Timestamptz_read_values()
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd =
            new EDBCommand("SELECT 'infinity'::timestamp with time zone, '-infinity'::timestamp with time zone", conn);
    ...
    public async Task Timestamptz_write_values()
    {
        ...
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT $1::text, $2::text, $3::text, $4::text", conn)
        {
            Parameters =
            {
    ...
    public async Task Timestamptz_write()
    {
        ...
        {
            Parameters = { new() { Value = Instant.MinValue, EDBDbType = EDBDbType.TimestampTz } }
        };
        ...
        await using var cmd2 = new EDBCommand("SELECT ($1 AT TIME ZONE 'UTC')::text", conn)
        {
            Parameters = { new() { Value = Instant.MaxValue, EDBDbType = EDBDbType.TimestampTz } }
        };
    ...
    public async Task Timestamp_write()
    {
        ...
        {
            Parameters = { new() { Value = LocalDateTime.MinIsoValue, EDBDbType = EDBDbType.Timestamp } }
        };
        ...
        await using var cmd2 = new EDBCommand("SELECT $1::text", conn)
        {
            Parameters = { new() { Value = LocalDateTime.MaxIsoValue, EDBDbType = EDBDbType.Timestamp } }
        };
    ...
    public async Task Date_write()
    {
        ...
        await using var cmd = new EDBCommand("SELECT $1::text", conn)
        {
            Parameters = { new() { Value = LocalDate.MinIsoValue, EDBDbType = EDBDbType.Date } }
        };
    ...
    public async Task Interval_write()
    {
        ...
        await using var cmd = new EDBCommand("SELECT $1::text", conn)
        {
            Parameters = { new() { Value = Period.MinValue, EDBDbType = EDBDbType.Interval } }
        };
    ...
    public async Task Interval_read()
    {
        ...
        await using var cmd = new EDBCommand("SELECT '-infinity'::interval, 'infinity'::interval", conn);
    ...
    public async Task Interval_convert_infinity()
    {
        ...
        {
            cmd.Parameters.AddWithValue("p1", EDBDbType.Interval, Period.MaxValue);
            cmd.Parameters.AddWithValue("p2", EDBDbType.Interval, Period.MinValue);
            await cmd.ExecuteNonQueryAsync();
        }

        using (var cmd = new EDBCommand("SELECT i1::TEXT, i2::TEXT, i1, i2 FROM data", conn))
    ...
    public NodaTimeInfinityTests(bool disableDateTimeInfinityConversions)
    {
        ...
        {
            Assert.Ignore(
                "NodaTimeInfinityTests rely on the EnterpriseDB.EDBClient.DisableDateTimeInfinityConversions AppContext switch and can only be run in DEBUG builds");
        }
#endif
