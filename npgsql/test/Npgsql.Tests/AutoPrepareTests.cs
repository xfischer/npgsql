...
public class AutoPrepareTests : TestBase
{
    ...
    public void Basic()
    {
        ...
        using var checkCmd = new EDBCommand(CountPreparedStatements, conn);
        checkCmd.Prepare();
    ...
    public void Recycle()
    {
        ...
        var cmd2 = new EDBCommand("SELECT 2", conn);
        ...
        using (var getTextCmd = new EDBCommand("SELECT statement FROM pg_prepared_statements WHERE statement NOT LIKE '%COUNT%' ORDER BY statement", conn))
    ...
    public async Task Positional_parameter()
    {
        ...
        await using var checkCmd = new EDBCommand(CountPreparedStatements, conn);
    ...
    public void Promote_auto_to_explicit()
    {
        ...
        using var cmd3 = new EDBCommand("SELECT 2", conn);
    ...
    public void One_command_same_sql_twice()
    {
        ...
        using var conn = dataSource.OpenConnection();
        using var cmd = new EDBCommand("SELECT 1; SELECT 1; SELECT 1; SELECT 1", conn);
        //cmd.Prepare();
        //Assert.That(cmd.IsPrepared, Is.True);
        cmd.ExecuteNonQuery();
    ...
    public void Overloaded_sql()
    {
        ...
        {
            cmd.Parameters.AddWithValue("p", EDBDbType.Integer, 8);
            cmd.ExecuteNonQuery();
            cmd.ExecuteNonQuery();
            Assert.That(cmd.IsPrepared, Is.True);
        }
    ...
    public void Row_description_properly_cloned()
    {
        ...
        using var conn = dataSource.OpenConnection();
        conn.UnprepareAll();
        using var cmd1 = new EDBCommand("SELECT 1 AS foo", conn);
        using var cmd2 = new EDBCommand("SELECT 1 AS bar", conn);
    ...
    // Exclude some internal EDB .NET Connector queries which include pg_type as well as the count statement itself
...
""";
    ...
    public async Task Auto_prepared_command_failure()
    {
        ...
        await using (var command = new EDBCommand($"INSERT INTO {tableName} (id) VALUES (1)", conn))
        {
            await command.ExecuteNonQueryAsync();
    ...
    public void Replace_with_bad_sql()
    {
        ...
        // Since PreparedStatementManager.NumPrepared is 0, EDB .NET Connector will now send DISCARD ALL, but our internal state thinks
...
""");
    ...
    {
        using var cmd = new EDBCommand("SELECT name,statement FROM pg_prepared_statements", conn);
