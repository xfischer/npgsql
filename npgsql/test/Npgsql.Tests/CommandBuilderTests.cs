...
class CommandBuilderTests : TestBase
{
    ...
    public async Task DeriveParameters_text_prepared_statement()
    {
        ...
        await using var cmd = new EDBCommand(query, conn);
...
CREATE DOMAIN {domainArrayType} AS int[] CHECK(array_length(VALUE, 1) = 2);");
        ...
        var cmd = new EDBCommand($"SELECT :a::{domainType}, :b::{domainType}[], :c::{domainArrayType}", conn);
        var val = 23;
    ...
    public async Task DeriveParameters_text_unmapped_enum()
    {
        ...
        var cmd = new EDBCommand($"SELECT :x::{type}", conn);
    ...
    public async Task DeriveParameters_text_mapped_enum()
    {
        ...
        var cmd = new EDBCommand($"SELECT :x::{type}, :y::{type}[]", connection);
    ...
    public async Task DeriveParameters_text_mapped_composite()
    {
        ...
        await using var cmd = new EDBCommand($"SELECT @p1::{type}, @p2::{type}[]", connection);
        EDBCommandBuilder.DeriveParameters(cmd);
        Assert.That(cmd.Parameters, Has.Count.EqualTo(2));
...
INSERT INTO {table} VALUES('key1', 'description', '2018-07-03', '2018-07-03 07:02:00', 123, 123.4, 1234.5, B'1', 'note')");
        ...
        Assert.That(daDataAdapter.UpdateCommand.Parameters[2].EDBDbType, Is.EqualTo(timeStampType));
        ...
        Assert.That(daDataAdapter.UpdateCommand.Parameters[15].EDBDbType, Is.EqualTo(timeStampType));
    ...
    public async Task Get_update_command_with_column_aliases()
    {
        ...
        using var cmd = new EDBCommand($"SELECT Cod as CodAlias, Descr as DescrAlias, Data as DataAlias FROM {table}", conn);
    ...
    public async Task Get_update_command_with_array_column_type()
    {
        ...
        using var daDataAdapter = new EDBDataAdapter($"SELECT cod, vettore FROM {table} ORDER By cod", conn);
        using var cbCommandBuilder = new EDBCommandBuilder(daDataAdapter);
        var dtTable = new DataTable();
    ...
    public async Task Get_update_command_with_domain_column_type()
    {
        ...
        using var adapter = new EDBDataAdapter($"select * from {tableName}", conn);

        var builder = new EDBCommandBuilder(adapter)
        {
    ...
    public async Task Fill_datatable_with_array_column_type()
    {
        ...
        using var adapter = new EDBDataAdapter($"select * from {tableName}", connection);
