...
public class CommandParameterTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task Generic_parameter()
    {
        ...
        cmd.Parameters.Add(new EDBParameter<char[]>("p4", ['f', 'o', 'o']));
        using var reader = await cmd.ExecuteReaderAsync();
        reader.Read();
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT @p1::TEXT", conn);
        cmd.Parameters.Add(
            genericParam
                ? new EDBParameter<object?>("p1", null)
                : new EDBParameter("p1", null)
        );

        Assert.That(async () => await cmd.ExecuteReaderAsync(),
            Throws.Exception
                .TypeOf<InvalidOperationException>()
                .With.Message.EqualTo("Parameter 'p1' must have either its EDBDbType or its DataTypeName or its Value set."));
    }

    [Test]
    public async Task Object_generic_param_does_runtime_lookup()
    {
        await AssertTypeWrite<object>(1, "1", "integer", EDBDbType.Integer, DbType.Int32, DbType.Int32, isDefault: false,
            isEDBDbTypeInferredFromClrType: true, skipArrayCheck: true);
        await AssertTypeWrite<object>(new[] {1, 1}, "{1,1}", "integer[]", EDBDbType.Integer | EDBDbType.Array, isDefault: false,
            isEDBDbTypeInferredFromClrType: true, skipArrayCheck: true);
    }
    ...
    public async Task Object_generic_parameter_works()
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT $1", conn);
        cmd.Parameters.Add(new EDBParameter<object> { EDBDbType = EDBDbType.Integer, Value = 8 });
        Assert.That(await cmd.ExecuteScalarAsync(), Is.EqualTo(8));
    }
}
