...
public class DataSourceTests : TestBase
{
    ...
    {
        if (string.IsNullOrEmpty(new EDBConnectionStringBuilder(ConnectionString).Password))
            Assert.Fail("No password in default connection string, test cannot run");

        using var dataSource = EDBDataSource.Create(ConnectionString);
        var parsedConnectionString = new EDBConnectionStringBuilder(dataSource.ConnectionString);
    ...
    {
        var dataSource = EDBDataSource.Create(ConnectionString);
    ...
    {
        var connectionStringBuilder = new EDBConnectionStringBuilder(ConnectionString) { Pooling = false };
        var dataSource = EDBDataSource.Create(connectionStringBuilder);
    ...
    public async Task Executing_command_on_disposed_datasource([Values] bool multiplexing)
    {
        ...
        DbDataSource dataSource = EDBDataSource.Create(csb.ConnectionString);
#if !NETFRAMEWORK
        await
#endif
    ...
    public async Task Multiplexing_connectionless_command_open_connection()
    {
        ...
        await using var dataSource = EDBDataSource.Create(csb.ConnectionString);
