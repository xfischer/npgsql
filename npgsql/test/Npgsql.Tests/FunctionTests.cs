...
public class FunctionTests : TestBase
{
    ...
    public async Task Resultset()
    {
        ...
        await using var cmd = new EDBCommand(function, conn) { CommandType = CommandType.StoredProcedure };
...
$$ LANGUAGE plpgsql");
        await using var cmd = new EDBCommand(function, conn);
        cmd.CommandType = CommandType.StoredProcedure;
        var outParam = new EDBParameter("param", DbType.Int32)
        {
    ...
    public async Task Named_parameters()
    {
        ...
        await using var command = new EDBCommand("make_timestamp", conn);
        command.CommandType = CommandType.StoredProcedure;
    ...
    {
        await using var conn = await OpenConnectionAsync();
        var command = new EDBCommand("VALUES (4,5), (6,7)", conn);
        command.Parameters.Add(new EDBParameter("a", DbType.Int32)
        {
            Direction = ParameterDirection.Output,
            Value = -1
        });
        command.Parameters.Add(new EDBParameter("b", DbType.Int32)
        {
            Direction = ParameterDirection.Output,
            Value = -1
        });
        command.Parameters.Add(new EDBParameter("c", DbType.Int32)
        {
            Direction = ParameterDirection.Output,
            Value = -1
        });
...
SET search_path TO {schema2};");
        await using var command = new EDBCommand("redundantfunc", conn) { CommandType = CommandType.StoredProcedure };
        EDBCommandBuilder.DeriveParameters(command);
...
RESET search_path;");
        await using var command = new EDBCommand("schema1func", conn) { CommandType = CommandType.StoredProcedure };
...
#if DEBUG
    [OneTimeSetUp]
    public void OneTimeSetup() => EDBCommand.EnableStoredProcedureCompatMode = true;

    [OneTimeTearDown]
    public void OneTimeTeardown() => EDBCommand.EnableStoredProcedureCompatMode = false;
#else
    [OneTimeSetUp]
