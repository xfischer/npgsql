...
#endif
public class EDBParameterCollectionTests
{
    readonly CompatMode _compatMode;
    const int LookupThreshold = EDBParameterCollection.LookupThreshold;

    [Test]
    public void Can_only_add_EDBParameter()
    {
    ...
    public void Remove_duplicate_parameter([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        {
            command.Parameters.AddWithValue(string.Format("p{0:00}", i + 1), EDBDbType.Text,
        ...
        command.Parameters.AddWithValue("P02", EDBDbType.Text, "String parameter value 2");
    ...
    public void Remove_case_differing_parameter([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        using var command = new EDBCommand();
        command.Parameters.Add(new EDBParameter("PP0", 1));
        command.Parameters.Add(new EDBParameter("Pp0", 1));
        for (var i = 0; i < count - 2; i++)
            command.Parameters.Add(new EDBParameter($"pp{i}", i));
    ...
    public void Correct_index_returned_for_duplicate_ParameterName([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            command.Parameters.AddWithValue(string.Format("parameter{0:00}", i + 1), EDBDbType.Text, string.Format("String parameter value {0}", i + 1));
        }
        ...
        command.Parameters.AddWithValue("Parameter02", EDBDbType.Text, "String parameter value 2");

        // Insert another case insensitive before the original.
        command.Parameters.Insert(0, new EDBParameter("ParameteR02", EDBDbType.Text) { Value = "String parameter value 2" });
    ...
    public void Finds_case_insensitive_lookups([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        for (var i = 0; i < count; i++)
            parameters.Add(new EDBParameter($"p{i}", i));
    ...
    public void Throws_on_indexer_mismatch([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        Assert.DoesNotThrow(() =>
        {
            command.Parameters["p1"] = new EDBParameter("p1", 1);
            command.Parameters["p1"] = new EDBParameter("P1", 1);
        });

        Assert.Throws<ArgumentException>(() =>
        {
            command.Parameters["p1"] = new EDBParameter("p2", 1);
        });
    }
    ...
    public void Positional_parameter_lookup_returns_first_match([Values(LookupThreshold, LookupThreshold - 2)] int count)
    {
        ...
        var parameters = command.Parameters;
        for (var i = 0; i < count; i++)
            parameters.Add(new EDBParameter(EDBParameter.PositionalName, i));
    ...
    {
        using var cmd = new EDBCommand("SELECT $1, $2");
        var p = new EDBParameter("", "Hello world");
    ...
    public void IndexOf_falls_back_to_first_insensitive_match([Values] bool manyParams)
    {
        ...
        parameters.Add(new EDBParameter("foo", 8));
        parameters.Add(new EDBParameter("bar", 8));
        parameters.Add(new EDBParameter("BAR", 8));
    ...
    public void IndexOf_matches_all_parameter_syntaxes()
    {
        ...
        parameters.Add(new EDBParameter("@foo0", 8));
        parameters.Add(new EDBParameter(":foo1", 8));
        parameters.Add(new EDBParameter("foo2", 8));
    ...
    public void Clean_name()
    {
        ...
        Assert.That(param.ParameterName, Is.EqualTo(EDBParameter.PositionalName));
    }
    ...
    public void Clone_sets_correct_collection()
    {
        ...
        cmd.Parameters.Add(new EDBParameter<int> { TypedValue = 42 });
    ...
    public EDBParameterCollectionTests(CompatMode compatMode)
    {
        _compatMode = compatMode;

#if DEBUG
        EDBParameterCollection.TwoPassCompatMode = compatMode == CompatMode.TwoPass;
#else
        if (compatMode == CompatMode.TwoPass)
            Assert.Ignore("Cannot test case-insensitive EDBParameterCollection behavior in RELEASE");
#endif
    }
