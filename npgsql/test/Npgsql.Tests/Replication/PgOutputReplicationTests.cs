...
using TruncateOptions = EnterpriseDB.EDBClient.Replication.PgOutput.Messages.TruncateMessage.TruncateOptions;
using ReplicaIdentitySetting = EnterpriseDB.EDBClient.Replication.PgOutput.Messages.RelationMessage.ReplicaIdentitySetting;
    ...
    public Task CreatePgOutputReplicationSlot()
    {
        ...
        return SafeReplicationTest(
            async (slotName, _) =>
            {
                ...
                using var cmd =
                    new EDBCommand($"SELECT * FROM pg_replication_slots WHERE slot_name = '{options.Name}'",
                        c);
...
");
    ...
    public Task Stream()
    {
        ...
        return SafePgOutputReplicationTest(
            async (slotName, tableName, publicationName) =>
            {
                ...
                using (var command = new EDBCommand($"INSERT INTO {tableName} VALUES ($1)", c))
                {
    ...
    [EDBTypes.PgName("descriptor")]
    public class Descriptor
    {
        [EDBTypes.PgName("id")]
        public long Id { get; set; }

        [EDBTypes.PgName("name")]
...
CREATE PUBLICATION {publicationName} FOR TABLE {tableName};");
                ...
                EDBConnection.GlobalTypeMapper.MapComposite<Descriptor>("descriptor");
