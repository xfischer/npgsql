...
public class PhysicalReplicationTests : SafeReplicationTestBase<PhysicalReplicationConnection>
{
    ...
    public Task CreateReplicationSlot([Values]bool temporary, [Values]bool reserveWal)
        => SafeReplicationTest(
            async (slotName, _) =>
            {
                ...
                using var cmd =
                    new EDBCommand($"SELECT * FROM pg_replication_slots WHERE slot_name = '{slot.Name}'",
                        c);
    ...
    public Task ReadReplicationSlot(bool createSlot, bool reserveWal)
        => SafeReplicationTest(
            async (slotName, _) =>
            {
                ...
                using var cmd =
                    new EDBCommand($@"SELECT slot_name, substring(pg_walfile_name(restart_lsn), 1, 8)::bigint AS timeline_id, restart_lsn
                ...
                var expectedRestartLsn = createSlot ? reader.GetFieldValue<EDBLogSequenceNumber?>(reader.GetOrdinal("restart_lsn")) : null;
                Assert.That(reader.Read, Is.False);
                await using var rc = await OpenReplicationConnectionAsync();
    ...
    public Task Replication_with_slot()
        => SafeReplicationTest(
            ...
            {
                // var messages = new ConcurrentQueue<(EDBLogSequenceNumber WalStart, EDBLogSequenceNumber WalEnd, byte[] data)>();
                await using var rc = await OpenReplicationConnectionAsync();
