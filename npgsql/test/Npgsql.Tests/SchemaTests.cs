...
public class SchemaTests(SyncOrAsync syncOrAsync) : SyncOrAsyncTestBase(syncOrAsync)
{
    ...
    public async Task DataSourceInformation()
    {
        ...
        Assert.That(row["DataSourceProductName"], Is.EqualTo("edb-dotnet"));
...
CREATE DOMAIN {domainType} AS TEXT");
        ...
        Assert.That(intRow["ProviderDbType"], Is.EqualTo((int)EDBDbType.Integer));
        ...
        Assert.That(textRow["ProviderDbType"], Is.EqualTo((int)EDBDbType.Text));
        ...
        Assert.That(numericRow["ProviderDbType"], Is.EqualTo((int)EDBDbType.Numeric));
        ...
        Assert.That(intArrayRow["ProviderDbType"], Is.EqualTo((int)(EDBDbType.Integer | EDBDbType.Array)));
        ...
        Assert.That(intRangeRow["DataType"], Does.StartWith("EDBTypes.EDBRange`1[[System.Int32"));
        Assert.That(intRangeRow["ProviderDbType"], Is.EqualTo((int)(EDBDbType.Integer | EDBDbType.Range)));
        ...
        Assert.That(enumRow["DataType"], Is.EqualTo("EnterpriseDB.EDBClient.Tests.SchemaTests+TestEnum"));
        ...
        Assert.That(compositeRow["DataType"], Is.EqualTo("EnterpriseDB.EDBClient.Tests.SchemaTests+TestComposite"));
        ...
        Assert.That(domainRow["ProviderDbType"], Is.EqualTo((int)EDBDbType.Text));
    ...
    public async Task ParameterMarkerFormat()
    {
        ...
        command.Parameters.Add(new EDBParameter(parameterName, 4));
...
CREATE TABLE {table} (color {schema}.{enumName});");
    ...
    {
        await using var dataSource = new EDBSlimDataSourceBuilder(ConnectionString).Build();
        await using var conn = await dataSource.OpenConnectionAsync();
        Assert.That(() => GetSchema(conn, DbMetaDataCollectionNames.DataTypes), Throws.Nothing);
    }

    // ReSharper disable MethodHasAsyncOverload
    async Task<DataTable> GetSchema(EDBConnection conn)
        => IsAsync ? await conn.GetSchemaAsync() : conn.GetSchema();

    async Task<DataTable> GetSchema(EDBConnection conn, string collectionName)
        => IsAsync ? await conn.GetSchemaAsync(collectionName) : conn.GetSchema(collectionName);

    async Task<DataTable> GetSchema(EDBConnection conn, string collectionName, string?[] restrictions)
