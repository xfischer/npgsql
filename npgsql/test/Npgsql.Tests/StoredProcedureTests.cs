...
END$$");
    ...
    public async Task Batch_positional_parameters_works()
    {
        ...
        await using var batch = new EDBBatch(connection, transaction)
        {
            BatchCommands =
            {
                new(tempname)
    ...
    public async Task Batch_StoredProcedure_output_parameters_works()
    {
        ...
        await using var batch = new EDBBatch(connection, transaction)
        {
            BatchCommands =
            {
                new(sproc)
                {
                    ...
                    {
                        new() { Value = "" },
                        new() { EDBDbType = EDBDbType.Bigint, Direction = ParameterDirection.Output }
                    }
                },
                new(sproc)
                {
        ...
        Assert.That(batch.BatchCommands[0].Parameters[1].Value, Is.EqualTo(1));
        Assert.That(batch.BatchCommands[1].Parameters[1].Value, Is.EqualTo(1));
    }
...
$$ LANGUAGE plpgsql");
        ...
        await using var command = new EDBCommand(sproc, conn) { CommandType = CommandType.StoredProcedure };
        ...
        Assert.That(command.Parameters[2].EDBDbType, Is.EqualTo(EDBDbType.Integer));
    ...
    public async Task DeriveParameters_procedure_in_only()
    {
        ...
        await using var cmd = new EDBCommand(sproc, conn) { CommandType = CommandType.StoredProcedure };
    ...
    public async Task DeriveParameters_procedure_with_case_sensitive_name()
    {
        ...
        try
        {
            await using var command = new EDBCommand(@"""ProcedureCaseSensitive""", conn) { CommandType = CommandType.StoredProcedure };
            EDBCommandBuilder.DeriveParameters(command);
            Assert.That(command.Parameters[0].EDBDbType, Is.EqualTo(EDBDbType.Integer));
            Assert.That(command.Parameters[1].EDBDbType, Is.EqualTo(EDBDbType.Text));
        }
    ...
    public async Task DeriveParameters_dot_character_in_function_name()
    {
        ...
        try
        {
            await using var command = new EDBCommand(@"""My.Dotted.Procedure""", conn) { CommandType = CommandType.StoredProcedure };
            EDBCommandBuilder.DeriveParameters(command);
            Assert.That(command.Parameters[0].EDBDbType, Is.EqualTo(EDBDbType.Integer));
            Assert.That(command.Parameters[1].EDBDbType, Is.EqualTo(EDBDbType.Text));
        }
    ...
    {
        await using var conn = await OpenConnectionAsync();
        var invalidCommandName = new EDBCommand("invalidprocedurename", conn) { CommandType = CommandType.StoredProcedure };
...
SET search_path TO {schema2};");
        await using var command = new EDBCommand("redundantsproc", conn) { CommandType = CommandType.StoredProcedure };
        EDBCommandBuilder.DeriveParameters(command);
...
RESET search_path;");
        await using var command = new EDBCommand("schema1sproc", conn) { CommandType = CommandType.StoredProcedure };
...
SET search_path TO {schema1}, {schema2};");
        var command = new EDBCommand("redundantsproc", conn) { CommandType = CommandType.StoredProcedure };
        Assert.That(() => EDBCommandBuilder.DeriveParameters(command),
            Throws.Exception.TypeOf<PostgresException>()
