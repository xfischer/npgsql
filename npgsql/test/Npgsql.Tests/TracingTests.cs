...
public class TracingTests(MultiplexingMode multiplexingMode, bool async) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task PhysicalOpen()
    {
        ...
        static void ValidateActivity(Activity activity, EDBConnection conn, bool isMultiplexing)
        {
    ...
    public async Task CommandExecute_error([Values] bool batch)
    {
        ...
        Assert.That(exceptionTags["exception.type"], Is.EqualTo("EnterpriseDB.EDBClient.PostgresException"));
    ...
    public async Task BinaryImport()
    {
        ...
        if (async)
        {
            ...
            await writer.WriteAsync((short)8, EDBDbType.Smallint);
    ...
    public async Task TextExport()
    {
        ...
        if (async)
        {
            ...
            _ = await reader.ReadAsync(chars, 0, chars.Length);
        }
        else
        {
    ...
    static ActivityListener StartListener(out List<Activity> activities)
    {
        ...
        {
            ShouldListenTo = source => source.Name == "edb-dotnet",
    ...
    static async Task<object?> ExecuteScalar(EDBConnection connection, bool async, bool isBatch, string query, bool prepare = false)
    {
        if (isBatch)
        {
