...
public class TypeMapperTests : TestBase
{
    ...
    public async Task String_to_citext()
    {
        ...
        await using var command = new EDBCommand("SELECT @p = 'hello'::citext", connection);
    ...
    public async Task String_to_citext_with_db_type_string()
    {
        ...
        var dataSourceBuilder = CreateDataSourceBuilder();
        ((IEDBTypeMapper)dataSourceBuilder).AddDbTypeResolverFactory(new ForceStringToCitextResolverFactory());
        await using var dataSource = dataSourceBuilder.Build();
        await using var connection = await dataSource.OpenConnectionAsync();

        await using var command = new EDBCommand("SELECT @p = 'hello'::citext", connection);
        var parameter = new EDBParameter("p", DbType.String)
        {
            Value = "HeLLo"
        };
        ...
        Assert.That(parameter.EDBDbType, Is.EqualTo(EDBDbType.Citext));
    ...
    public async Task Guid_to_custom_type()
    {
        ...
        ((IEDBTypeMapper)dataSourceBuilder).AddDbTypeResolverFactory(new GuidTextDbTypeResolverFactory(type));
        await using var dataSource = dataSourceBuilder.Build();
        await using var connection = await dataSource.OpenConnectionAsync();
        ...
        await using var command = new EDBCommand($"SELECT @p::text = '{guid}'", connection);
        var parameter = new EDBParameter("p", DbType.Guid)
        {
            Value = guid
        };
        command.Parameters.Add(parameter);
        ...
        Assert.That(parameter.EDBDbType, Is.EqualTo(EDBDbType.Unknown));
...
CREATE EXTENSION citext SCHEMA ""{schemaName}""");
    ...
    {
        public override IDbTypeResolver CreateDbTypeResolver(EDBDatabaseInfo databaseInfo) => new DbTypeResolver();
    ...
    {
        public override IDbTypeResolver CreateDbTypeResolver(EDBDatabaseInfo databaseInfo) => new DbTypeResolver(typeName);
