...
public class ArrayTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public async Task NullableInts()
    {
        ...
        var dataSourceBuilder = new EDBDataSourceBuilder(connectionStringBuilder.ToString());
        await using var dataSource = dataSourceBuilder.Build();

        await AssertType(dataSource, new int?[] { 1, 2, null, 3 }, "{1,2,NULL,3}", "integer[]", EDBDbType.Integer | EDBDbType.Array);
    }
...
""", conn);
    ...
    public async Task Write_empty_multidimensional_array()
        => await AssertTypeWrite(new int[0, 0], "{}", "integer[]", EDBDbType.Integer | EDBDbType.Array);

    [Test]
    public async Task Generic_List()
        => await AssertType(
            new List<int> { 1, 2, 3 }, "{1,2,3}", "integer[]", EDBDbType.Integer | EDBDbType.Array, isDefaultForReading: false);

    [Test]
    public async Task Write_IList_implementation()
        => await AssertTypeWrite(
            ImmutableArray.Create(1, 2, 3), "{1,2,3}", "integer[]", EDBDbType.Integer | EDBDbType.Array);
    ...
    public async Task Generic_IList()
    {
        ...
        cmd.Parameters.Add(new EDBParameter<IList<int>>("p1", EDBDbType.Array | EDBDbType.Integer) { TypedValue = expected });
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT ARRAY[[1], [2]]", conn);
    ...
    public async Task Array_of_domain()
    {
        ...
        await using var cmd = new EDBCommand("SELECT @p1::posint[], @p2::posint[][]", conn);
...
""");
        await conn.ReloadTypesAsync();
        await using var cmd = new EDBCommand("SELECT @p1::int_array_1d, @p2::int_array_2d", conn);
        var oneDim = new[] { 1, 3, 5, 9 };
        var twoDim = new[,] { { 1, 3 }, { 5, 9 } };
        cmd.Parameters.AddWithValue("p1", EDBDbType.Integer | EDBDbType.Array, oneDim);
        cmd.Parameters.AddWithValue("p2", EDBDbType.Integer | EDBDbType.Array, twoDim);
    ...
    {
        await using var conn = await OpenConnectionAsync();
        await using var cmd = new EDBCommand("SELECT '{}'::INT[], '{}'::INT[]", conn);
        await using var reader = await cmd.ExecuteReaderAsync();
    ...
    [Test]
    public async Task Arrays_not_supported_by_default_on_EDBSlimSourceBuilder()
    {
