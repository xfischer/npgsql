...
{
    async Task Write<T>(Action<EDBDataReader, T> assert, string? schema = null)
    ...
    async Task Write<T>(T composite, Action<EDBDataReader, T>? assert = null, string? schema = null)
        where T : IComposite
    {
        await using var dataSource = await OpenAndMapComposite(composite, schema, nameof(Write), out var _);
        await using var connection = await dataSource.OpenConnectionAsync();
        {
            await using var command = new EDBCommand("SELECT (@c).*", connection);
        ...
        {
            await using var command = new EDBCommand("SELECT (@arrayc)[1].*", connection);
