...
public class TextTests(MultiplexingMode multiplexingMode) : MultiplexingTestBase(multiplexingMode)
{
    ...
    public Task Text_as_ReadOnlyMemory_of_bytes()
        => AssertTypeWrite(new ReadOnlyMemory<byte>(Encoding.UTF8.GetBytes("foo")), "foo", "text", EDBDbType.Text, DbType.String,
            isDefault: false);
    ...
    public Task Text_as_MemoryStream()
        => AssertTypeWrite(() => new MemoryStream("foo"u8.ToArray()), "foo", "text", EDBDbType.Text, DbType.String, isDefault: false);
