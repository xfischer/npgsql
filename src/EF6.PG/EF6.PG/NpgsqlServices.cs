...
namespace EnterpriseDB.EDBClient
{
    ...
    {
        public static EDBServices Instance { get; } = new EDBServices();

        public EDBServices()
        {
            AddDependencyResolver(new SingletonDependencyResolver<Func<MigrationSqlGenerator>>(
                () => new EDBMigrationSqlGenerator(), nameof(EnterpriseDB.EDBClient)));
        }
        ...
        protected override DbCommandDefinition CreateDbCommandDefinition([NotNull] DbProviderManifest providerManifest, [NotNull] DbCommandTree commandTree)
            => CreateCommandDefinition(CreateDbCommand(((EDBProviderManifest)providerManifest).Version, commandTree));
        ...
        internal DbCommand CreateDbCommand(Version serverVersion, DbCommandTree commandTree)
        {
            ...
            {
                var dbParameter = new EDBParameter
                {
                    ParameterName = parameter.Key,
                    EDBDbType = EDBProviderManifest.GetEDBDbType(((PrimitiveType)parameter.Value.EdmType).PrimitiveTypeKind)
                };
        ...
        // EnterpriseDB.EDBClient > 4.0 does strict type checks on integral values and fails with enums passed with numeric DbType.
        ...
        internal void TranslateCommandTree(Version serverVersion, DbCommandTree commandTree, DbCommand command, bool createParametersForNonSelect = true)
        {
            ...
            sqlGenerator.Command = (EDBCommand)command;
        ...
        protected override bool DbDatabaseExists([NotNull] DbConnection connection, int? commandTimeout, [NotNull] StoreItemCollection storeItemCollection)
        {
            ...
            UsingPostgresDbConnection((EDBConnection)connection, conn =>
            {
                using (var command = new EDBCommand("select count(*) from pg_catalog.pg_database where datname = '" + connection.Database + "';", conn))
        ...
        protected override void DbCreateDatabase([NotNull] DbConnection connection, int? commandTimeout, [NotNull] StoreItemCollection storeItemCollection)
        {
            UsingPostgresDbConnection((EDBConnection)connection, conn =>
            {
                ...
                using (var  command = new EDBCommand(sb.ToString(), conn))
        ...
        protected override void DbDeleteDatabase([NotNull] DbConnection connection, int? commandTimeout, [NotNull] StoreItemCollection storeItemCollection)
        {
            UsingPostgresDbConnection((EDBConnection)connection, conn =>
            {
                ...
                using (var command = new EDBCommand("DROP DATABASE \"" + connection.Database + "\";", conn))
                    command.ExecuteNonQuery();
            });
        }

        static void UsingPostgresDbConnection(EDBConnection connection, Action<EDBConnection> action)
        {
