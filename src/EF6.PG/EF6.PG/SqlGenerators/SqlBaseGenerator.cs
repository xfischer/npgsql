...
namespace EnterpriseDB.EDBClient.SqlGenerators
{
    internal abstract class SqlBaseGenerator : DbExpressionVisitor<VisitedExpression>
    {
        ...
        public override VisitedExpression Visit([NotNull] DbConstantExpression expression)
        {
            if (CreateParametersForConstants)
            {
                var parameter = new EDBParameter
                {
                    ...
                    EDBDbType = EDBProviderManifest.GetEDBDbType(((PrimitiveType)expression.ResultType.EdmType).PrimitiveTypeKind),
        ...
        VisitedExpression VisitFunction(EdmFunction function, IList<DbExpression> args, TypeUsage resultType)
        {
            ...
            if (function.NamespaceName == "EnterpriseDB.EDBClient")
            {
                ...
                else if (functionName == "setweight")
                {
                    ...
                    var weightLabel = (EDBWeightLabel)weightLabelExpression.Value;
                    if (!Enum.IsDefined(typeof(EDBWeightLabel), weightLabelExpression.Value))
