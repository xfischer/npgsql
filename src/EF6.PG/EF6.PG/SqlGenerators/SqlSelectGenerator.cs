...
namespace EnterpriseDB.EDBClient.SqlGenerators
{
    internal class SqlSelectGenerator : SqlBaseGenerator
    {
        ...
        {
            Debug.Assert(command is EDBCommand);
            ...
            ((EDBCommand)command).UnknownResultTypeList = pe.Projection.Arguments.Select(a => ((PrimitiveType)((ColumnExpression)a).ColumnType.EdmType).PrimitiveTypeKind == PrimitiveTypeKind.String).ToArray();
            ...
            {
                ((EDBCommand)command).ObjectResultTypes = pe.Projection.Arguments.Select(a =>
                {
                    var kind = ((PrimitiveType)((ColumnExpression)a).ColumnType.EdmType).PrimitiveTypeKind;
                    switch (kind)
                    {
