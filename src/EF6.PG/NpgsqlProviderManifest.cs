...
namespace EnterpriseDB.EDBClient
{
    internal class EDBProviderManifest : DbXmlEnabledProviderManifest
    {
        ...
        public EDBProviderManifest(string serverVersion)
            : base(CreateXmlReaderForResource("EnterpriseDB.EDBClient.Resources.NpgsqlProviderManifest.Manifest.xml"))
        {
        ...
        protected override XmlReader GetDbInformation([NotNull] string informationType)
        {
            if (informationType == StoreSchemaDefinition)
                return CreateXmlReaderForResource("EnterpriseDB.EDBClient.Resources.NpgsqlSchema.ssdl");
            if (informationType == StoreSchemaDefinitionVersion3)
                return CreateXmlReaderForResource("EnterpriseDB.EDBClient.Resources.NpgsqlSchemaV3.ssdl");
            if (informationType == StoreSchemaMapping)
                return CreateXmlReaderForResource("EnterpriseDB.EDBClient.Resources.NpgsqlSchema.msl");
        ...
        internal static EDBDbType GetEDBDbType(PrimitiveTypeKind primitiveType)
        {
        ...
        {
            var stream = Assembly.GetAssembly(typeof(EDBProviderManifest)).GetManifestResourceStream(resourceName);
            if (stream == null)
        ...
        public override ReadOnlyCollection<EdmFunction> GetStoreFunctions()
            => new[] { typeof(EDBTextFunctions).GetTypeInfo(), typeof(EDBTypeFunctions) }
