...
using EDBNunit.Tests;
using EDBTypes;

using NUnit.Framework;

namespace EDBNunit.Tests
{
    [TestFixture("9.4")]
    [TestFixture("9.3")]
    [TestFixture("9.2")]
    ...
    public abstract class TestBase
    {
        ...
        /// </summary>
        internal EDBConnection Conn { get; set; }
        ...
        protected virtual string ConnectionStringEF
        {
            get
            {
                if (connectionStringEF == null)
                {
                    ...
                    var connectionSB = new EDBConnectionStringBuilder(ConnectionString);
		...
		/// test database.
		/// </summary>
		private const string DEFAULT_CONNECTION_STRING = "Server=localhost;User ID=enterprisedb;Password=edb.123;Database=efdotnet;port=5445";
        ...
        protected virtual void SetupLogging()
        {
            ...
            if (!_loggingSetUp)
            {
				EDBLogManager.Provider = new ConsoleLoggingProvider();
                _loggingSetUp = true;
            }
        }
        ...
        protected int ExecuteNonQuery(string sql, EDBConnection conn = null, EDBTransaction tx = null)
        {
            if (conn == null)
                conn = Conn;
        ...
        protected object ExecuteScalar(string sql, EDBConnection conn = null, EDBTransaction tx = null)
        {
            if (conn == null)
                conn = Conn;
            var cmd = tx == null ? new EDBCommand(sql, conn) : new EDBCommand(sql, conn, tx);
...
#if !NET40
        protected async Task<int> ExecuteNonQueryAsync(string sql, EDBConnection conn = null, EDBTransaction tx = null)
        {
            if (conn == null)
                conn = Conn;
            var cmd = tx == null ? new EDBCommand(sql, conn) : new EDBCommand(sql, conn, tx);
        ...
        protected async Task<object> ExecuteScalarAsync(string sql, EDBConnection conn = null, EDBTransaction tx = null)
        {
            if (conn == null)
                conn = Conn;
            var cmd = tx == null ? new EDBCommand(sql, conn) : new EDBCommand(sql, conn, tx);
        ...
        /// <returns></returns>
        protected static EDBCommand CreateSleepCommand(EDBConnection conn, int seconds)
        {
