...
using BclTimestampHandler = EnterpriseDB.EDBClient.TypeHandlers.DateTimeHandlers.TimestampHandler;

#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member

namespace EnterpriseDB.EDBClient.NodaTime
{
    public class TimestampHandlerFactory : EDBTypeHandlerFactory<Instant>
    {
    ...
    sealed class TimestampHandler : EDBSimpleTypeHandler<Instant>, IEDBSimpleTypeHandler<LocalDateTime>, IEDBSimpleTypeHandler<DateTime>
    {
        ...
        LocalDateTime IEDBSimpleTypeHandler<LocalDateTime>.Read(EDBReadBuffer buf, int len, FieldDescription? fieldDescription)
        {
            ...
            if (value == long.MaxValue || value == long.MinValue)
                throw new EDBSafeReadException(new NotSupportedException("Infinity values not supported when reading LocalDateTime, read as Instant instead"));
        ...
        internal static void WriteInteger(Instant instant, EDBWriteBuffer buf)
        {
        ...
        // This is legacy support for PostgreSQL's old floating-point timestamp encoding - finally removed in PG 10 and not used for a long
        // time. Unfortunately CrateDB seems to use this for some reason.
        internal static void WriteDouble(Instant instant, EDBWriteBuffer buf)
        {
