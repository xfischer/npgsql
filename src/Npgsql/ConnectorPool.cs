...
namespace EnterpriseDB.EDBClient
{
    ...
    sealed class ConnectorPool : IDisposable
    {
        ...
        readonly EDBConnector?[] _idle;
        readonly EDBConnector?[] _open;

        readonly ConcurrentQueue<(TaskCompletionSource<EDBConnector?> TaskCompletionSource, bool IsAsync)> _waiting;
        ...
        internal bool TryAllocateFast(EDBConnection conn, [NotNullWhen(true)] out EDBConnector? connector)
        {
        ...
        internal async ValueTask<EDBConnector> AllocateLong(EDBConnection conn, EDBTimeout timeout, bool async, CancellationToken cancellationToken)
        {
            ...
            while (true)
            {
                ...
                if (busyCount == _max)
                {
                    ...
                    var tcs = new TaskCompletionSource<EDBConnector?>(TaskCreationOptions.RunContinuationsAsynchronously);
                    ...
                    try
                    {
                        if (async)
                        {
                            ...
                            {
                                using (cancellationToken.Register(s => ((TaskCompletionSource<EDBConnector?>)s!).SetCanceled(), tcs))
                                    await tcs.Task;
                            }
                        }
                        else
        ...
        internal void Release(EDBConnector connector)
        {
        ...
        void CloseConnector(EDBConnector connector, bool wasIdle)
        {
            try
            {
                connector.Close();
        ...
        {
            if (state.Open > _max)
                throw new EDBException($"Pool is over capacity (Total={state.Open}, Max={_max})");
            if (state.Open < 0)
                throw new EDBException("Open is negative");
            if (state.Idle < 0)
                throw new EDBException("Idle is negative");
            if (state.Open - state.Idle < 0)
                throw new EDBException("Busy is negative");
        }
