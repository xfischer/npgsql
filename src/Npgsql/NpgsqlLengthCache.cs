...
namespace EnterpriseDB.EDBClient{
    ...
    /// </summary>
    public sealed class EDBLengthCache
    {
        internal bool IsPopulated;
        internal int Position;
        internal List<int> Lengths;

        internal EDBLengthCache() => Lengths = new List<int>();

        internal EDBLengthCache(int capacity) => Lengths = new List<int>(capacity);

        /// <summary>
        /// Stores a length value in the cache, to be fetched later via <see cref="Get"/>.
        /// Called at the <see cref="EDBTypeHandler.ValidateAndGetLength{TAny}"/> phase.
        /// </summary>
