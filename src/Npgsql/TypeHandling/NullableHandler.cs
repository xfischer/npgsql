...
namespace EnterpriseDB.EDBClient.TypeHandling
{
    ...
    delegate Task WriteAsyncDelegate<T>(EDBTypeHandler handler, T value, EDBWriteBuffer buffer, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async);
    ...
    static class NullableHandler
    {
        ...
        static Task WriteAsync<T>(EDBTypeHandler handler, T? value, EDBWriteBuffer buffer, EDBLengthCache? lengthCache, EDBParameter? parameter, bool async)
            where T : struct
            => value.HasValue
