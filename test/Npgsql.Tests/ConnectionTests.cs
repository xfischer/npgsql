...
namespace EnterpriseDB.EDBClient.Tests
{
    public class ConnectionTests : TestBase
    {
        ...
        public void ProvidePasswordCallbackDelegateIsUsed()
        {
            ...
            using (var conn = new EDBConnection(connString.ToString()) { ProvidePasswordCallback = ProvidePasswordCallback })
            {
                conn.Open();
...
#pragma warning restore CS8603 // Possible null reference return.
        ...
        {
            var connString = new EDBConnectionStringBuilder(ConnectionString) { Pooling = false };
            var goodPassword = connString.Password;
            connString.Password = null;
...
#pragma warning restore CS8603 // Possible null reference return.
        ...
        {
            var csbWithKeepAlive = new EDBConnectionStringBuilder(ConnectionString) { KeepAlive = 1 };
        ...
        public void BreakConnectorInPool(bool keepAlive)
        {
            ...
            using (var conn = new EDBConnection(csb.ToString()))
            {
                ...
                else
                {
                    ...
                    Assert.That(() => conn.ExecuteScalar("SELECT 1"), Throws.Exception.TypeOf<EDBException>());
                }
            }
        }
        ...
        {
            using (var conn = OpenConnection(new EDBConnectionStringBuilder(ConnectionString) { SearchPath = "foo" }))
        ...
        public void PoolByPassword()
        {
            ...
            try
            {
                ...
                using (goodConn = new EDBConnection(csb.ToString()))
                    goodConn.Open();
            ...
            {
                if (goodConn != null)
                    EDBConnection.ClearPool(goodConn);
            }
        }
        ...
        public void NonUTF8Encoding()
        {
            using (var adminConn = OpenConnection())
            {
                ...
                try
                {
                    ...
                    var goodCsb = new EDBConnectionStringBuilder(ConnectionString)
                    {
                        Database = "sqlascii",
                    ...
                    var badCsb = new EDBConnectionStringBuilder(ConnectionString)
                    {
                        Database = "sqlascii",
                        Pooling = false
                    };
        ...
        [Test, Ignore("MERGE_NEED_TO_EXPLORE"), Explicit, Description("Turns on TCP keepalive and sleeps forever, good for wiresharking")]
