...
#nullable restore           
    ...
    public class EDB_EC_1113_Tests : TestBase
    {
        ...
        {
            con.ReloadTypes();
            EDBCommand Command = new EDBCommand("TEST_PROC_TYPE_ARRAY", con);
    ...
    public class EDB_EC_1134_Tests : TestBase
    {
        ...
        public void Init()
        {
            ...
            //Insert into table.
            using (EDBCommand insertCommand = new EDBCommand("", con))
            {
    ...
    public class EDB_EC_1084_Tests : TestBase
    {
        ...
        public void Init()
        {
            ...
            //Create Schema.
            CreateDropSchema("CREATE SCHEMA ca", true);

            //Create table and insert data.
            CreateTable("CREATE TABLE ca.epas_test_table2" +
                        "(" +
                            "id numeric not null," +
                        ...
                        "WITH(" +
                            "OIDS = FALSE" +
                        ")");
            InsertIntoTable("insert into ca.epas_test_table2 (id, title) values ( 1, 'row 1')");
        }
        ...
        public void Dispose()
        {
            ...
            //Drop Schema
            CreateDropSchema("DROP SCHEMA ca", false);

            TestUtil.closeDB(con);
        }

        private void CreateDropSchema(string schameString, bool create)
        {
            //Create Table.
            EDBCommand createTableCommand = new EDBCommand("", con);
            createTableCommand.CommandText = schameString;
            int count = createTableCommand.ExecuteNonQuery();

            if (create)
                Assert.IsTrue(count != 0, "Schema was not created");
        }
        ...
        private void CreateTable(string tableString)
        {
            ...
            Assert.IsTrue(count != 0, "Table was not created");
        }

        private void InsertIntoTable(string insertString)
        {
            //Insert into table.
            EDBCommand insertCommand = new EDBCommand("", con);
        ...
        private void CreateDropProcedure(string procString, bool create)
        {
            //Create Procedure
            EDBCommand createProcCommand = new EDBCommand("", con);
            createProcCommand.CommandType = CommandType.Text;
        ...
        private void DoSupressTest(string pkg, string pkgBody, string pkgNameCall, string pkgNameDelete, bool shouldThrow, string shouldThrowThis)
        {
            ...
            if (pkg != null)
                CreateDropProcedure(pkg, true);

            CreateDropProcedure(pkgBody, true);
            ...
            {
                //Drop package body.
                CreateDropProcedure("DROP PACKAGE BODY " + pkgNameDelete, false);

                //Drop package.
                CreateDropProcedure("DROP PACKAGE " + pkgNameDelete, false);
            }
            else
                CreateDropProcedure("DROP PROCEDURE " + pkgNameCall, false);

            Assert.AreEqual(shouldThrow, isThrown, "Test result is not as expected.");
            if (shouldThrow)
                Assert.AreEqual(shouldThrowThis, exceptionMessage, "Exception message is not as expected");
