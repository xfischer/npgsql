    ...
    public class ReaderTests : TestBase
    {
        ...
        public void NoResultSet()
        {
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("INSERT INTO data VALUES (8)", conn))
                using (var reader = cmd.ExecuteReader(Behavior))
                {
                ...
                using (var cmd = new EDBCommand("SELECT 1; INSERT INTO data VALUES (8)", conn))
                using (var reader = cmd.ExecuteReader(Behavior))
                {
                    reader.NextResult();
                    Assert.That(() => reader.GetOrdinal("foo"), Throws.Exception.TypeOf<InvalidOperationException>());
        ...
        public void Statements()
        {
            ...
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("SELECT name FROM data; DELETE FROM data", conn))
                using (var reader = cmd.ExecuteReader(Behavior))
                {
        ...
        public void GetStringWithParameter()
        {
            using (var conn = OpenConnection())
            {
                ...
                var command = new EDBCommand("SELECT name FROM data WHERE name = :value;", conn);
...
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
        ...
        public void ExceptionThrownFromExecuteQuery([Values(PrepareOrNot.Prepared, PrepareOrNot.NotPrepared)] PrepareOrNot prepare)
        {
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("SELECT pg_temp.emit_exception()", conn))
                {
        ...
        public void ExceptionThrownFromNextResult([Values(PrepareOrNot.Prepared, PrepareOrNot.NotPrepared)] PrepareOrNot prepare)
        {
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("SELECT 1; SELECT pg_temp.emit_exception()", conn))
                {
                    if (prepare == PrepareOrNot.Prepared)
                        cmd.Prepare();
                    using (var reader = cmd.ExecuteReader(Behavior))
...
LANGUAGE plpgsql VOLATILE";
        ...
        public async Task GetStream([Values(true, false)] bool isAsync)
        {
            ...
            using (var conn = OpenConnection())
            {
                ...
                byte[] expected = { 1, 2, 3, 4, 5 };
                var actual = new byte[expected.Length];
                using (var cmd = new EDBCommand($@"SELECT {TestUtil.EncodeByteaHex(expected)}::bytea, {TestUtil.EncodeByteaHex(expected)}::bytea", conn))
                using (var reader = cmd.ExecuteReader(Behavior))
                {
                    reader.Read();
        ...
        public void GetBytesWithNull([Values(true, false)] bool isAsync)
        {
            ...
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("SELECT bytes FROM data", conn))
                using (var reader = cmd.ExecuteReader(Behavior))
                {
                    reader.Read();
    ...
    class ExplodingTypeHandler : EDBSimpleTypeHandler<int>
    {
        ...
        public override int Read(EDBReadBuffer buf, int len, FieldDescription? fieldDescription = null)
        {
            buf.ReadInt32();
            throw _safe
                ? new EDBSafeReadException(new Exception("Safe read exception as requested"))
