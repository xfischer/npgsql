...
#pragma warning restore CS8618
        ...
        public void UnmappedComposite()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                using (var cmd = new EDBCommand("SELECT @scalar1, @scalar2, @scalar3, @scalar4, @array1, @array2", conn))
                {
                    ...
                    cmd.Parameters.Add(new EDBParameter<SomeComposite>
                    {
                        ParameterName = "scalar3",
                        TypedValue = composite,
                        DataTypeName = $"{tempSchema}.unmapped_comp"
                    });
                    cmd.Parameters.Add(new EDBParameter<dynamic>
                    {
        ...
        public void CompositePostgresType()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                using (var cmd = new EDBCommand("SELECT ROW(ROW(8, 'foo')::comp1, ARRAY[ROW(9, 'bar')::comp1, ROW(10, 'baz')::comp1])::comp2", conn))
                {
                    using (var reader = cmd.ExecuteReader())
                    {
                        reader.Read();
        ...
        public void CompositeTypeResolutionWithGlobalMapping()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                EDBConnection.GlobalTypeMapper.MapComposite<SomeComposite>("composite1");
                try
                {
                    conn.ReloadTypes();
                    ...
                    using (var cmd = new EDBCommand("SELECT @p", conn))
                    {
                        cmd.Parameters.Add(new EDBParameter { ParameterName = "p", Value = new SomeComposite { X = 8, SomeText = "foo" }});
                        using (var reader = cmd.ExecuteReader())
                        {
                            reader.Read();
                            Assert.That(reader.GetDataTypeName(0), Does.StartWith("pg_temp").And.EndWith(".composite1"));
                    ...
                    conn.ReloadTypes();
                    using (var cmd = new EDBCommand("SELECT ROW(1, 'foo')::COMPOSITE1", conn))
                    using (var reader = cmd.ExecuteReader())
                    {
                        reader.Read();
                        Assert.That(reader.GetDataTypeName(0), Does.StartWith("pg_temp").And.EndWith(".composite1"));
                ...
                {
                    EDBConnection.GlobalTypeMapper.UnmapComposite<SomeComposite>("composite1");
                }
            }
        }
        ...
        public void CompositeTypeResolutionWithLateMapping()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                conn.ReloadTypes();
                conn.TypeMapper.MapComposite<SomeComposite>("composite2");
                using (var cmd = new EDBCommand("SELECT ROW(1, 'foo')::COMPOSITE2", conn))
                using (var reader = cmd.ExecuteReader())
                {
                    reader.Read();
                    Assert.That(reader.GetDataTypeName(0), Does.StartWith("pg_temp").And.EndWith(".composite2"));
        ...
        public void LateMapping()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                using (var cmd = new EDBCommand("SELECT @p1::composite3, @p2::composite3", conn))
                {
                    cmd.Parameters.Add(new EDBParameter
                    {
                        ParameterName = "p1",
        ...
        public void GlobalMapping()
        {
            ...
            try
            {
                using (var conn = OpenConnection(csb))
                {
                    ...
                    EDBConnection.GlobalTypeMapper.MapComposite<SomeComposite>("composite4");
                    conn.ReloadTypes();
                ...
                // Unmap
                EDBConnection.GlobalTypeMapper.UnmapComposite<SomeComposite>("composite4");
        ...
        public void Recursive()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                using (var cmd = new EDBCommand("SELECT @p::composite_container", conn))
                {
                    cmd.Parameters.AddWithValue("p", expected);
                    using (var reader = cmd.ExecuteReader())
                    {
        ...
        public void Struct()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                using (var cmd = new EDBCommand("SELECT @p::composite_struct", conn))
                {
                    cmd.Parameters.AddWithValue("p", expected);
                    using (var reader = cmd.ExecuteReader())
                    {
        ...
        public void Array()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                using (var cmd = new EDBCommand("SELECT @p1::composite5[], @p2::composite5[]", conn))
                {
                    cmd.Parameters.Add(new EDBParameter
                    {
                        ParameterName = "p1",
...
(
    ...
    postal_code us_postal_code
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                using (var cmd = new EDBCommand(@"SELECT @p", conn))
                {
                    cmd.Parameters.Add(new EDBParameter { ParameterName = "p", Value=expected });
                    using (var reader = cmd.ExecuteReader())
                    {
                        reader.Read();
        ...
        public void LocalMappingDontLeak()
        {
            ...
            EDBConnection.GlobalTypeMapper.MapComposite<Composite2>("composite");
            try
            {
                using (var conn = OpenConnection(csb))
                {
            ...
            {
                EDBConnection.GlobalTypeMapper.UnmapComposite<Composite2>("composite");
                using (var conn = OpenConnection(csb))
                {
