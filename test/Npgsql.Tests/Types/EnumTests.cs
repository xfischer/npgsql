    ...
    class EnumTests : TestBase
    {
        ...
        public void EnumTypeResolutionWithGlobalMapping()
        {
            ...
            using (var conn = OpenConnection(csb))
            {
                ...
                try
                {
                    ...
                    conn.ReloadTypes();
                    using (var cmd = new EDBCommand("SELECT 'happy'::MOOD1", conn))
                    using (var reader = cmd.ExecuteReader())
                    {
                        reader.Read();
                        Assert.That(reader.GetDataTypeName(0), Does.StartWith("pg_temp").And.EndWith(".mood1"));
        ...
        public void ReadUnmappedEnumsAsString()
        {
            using (var conn = new EDBConnection(ConnectionString))
            {
                ...
                conn.ReloadTypes();
                using (var cmd = new EDBCommand("SELECT 'Sad'::MOOD7, ARRAY['Ok', 'Happy']::MOOD7[]", conn))
                using (var reader = cmd.ExecuteReader())
                {
                    reader.Read();
        ...
        public void WriteStringToBackendEnum()
        {
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("insert into pg_temp.test_fruit(id, value1, value2) values(default, @p1, @p2);", conn))
                {
        ...
        public void WriteEnumAsDbUnknwown()
        {
            using (var conn = OpenConnection())
            {
                ...
                using (var cmd = new EDBCommand("insert into pg_temp.test_mood_writes(value1) values(@p1);", conn))
                {
